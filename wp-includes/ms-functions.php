...
function get_admin_users_for_domain($sitedomain = '', $path = '') {
    ...
    if (!$sitedomain) $site_id = $wpdb->siteid; else
        $site_id = $wpdb->get_var($wpdb->prepare("SELECT id FROM $wpdb->site WHERE domain = %s AND path = %s", $sitedomain, $path));

    if ($site_id) return $wpdb->get_results($wpdb->prepare("SELECT u.ID, u.user_login, u.user_pass FROM $wpdb->users AS u, $wpdb->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $site_id), ARRAY_A);
...
function get_active_blog_for_user($user_id) {
    ...
    if (empty($blogs)) return null;

    if (!is_multisite()) return $blogs[$wpdb->blogid];
    ...
    if ((!is_object($primary)) || ($primary->archived == 1 || $primary->spam == 1 || $primary->deleted == 1)) {
        ...
        if (is_array($blogs) && count($blogs) > 0) {
            foreach ((array)$blogs as $blog_id => $blog) {
                if ($blog->site_id != $wpdb->siteid) continue;
                $details = get_blog_details($blog_id);
                if (is_object($details) && $details->archived == 0 && $details->spam == 0 && $details->deleted == 0) {
                    $ret = $blog;
                    if (get_user_meta($user_id, 'primary_blog', true) != $blog_id) update_user_meta($user_id, 'primary_blog', $blog_id);
                    if (!get_user_meta($user_id, 'source_domain', true)) update_user_meta($user_id, 'source_domain', $blog->domain);
                    break;
                }
            }
        } else {
...
function get_blog_count($network_id = 0) {
    if (func_num_args()) _deprecated_argument(__FUNCTION__, '3.1');
...
function get_blog_id_from_url($domain, $path = '/') {
    ...
    if ($id == -1) // blog does not exist
        return 0; elseif ($id) return (int)$id;
...
function is_email_address_unsafe($user_email) {
    ...
    if ($banned_names && !is_array($banned_names)) $banned_names = explode("\n", $banned_names);
...
function wpmu_validate_user_signup($user_name, $user_email) {
    ...
    if (empty($user_name)) $errors->add('user_name', __('Please enter a username.'));
    ...
    if (in_array($user_name, $illegal_names) == true) $errors->add('user_name', __('That username is not allowed.'));

    if (is_email_address_unsafe($user_email)) $errors->add('user_email', __('You cannot use that email address to signup. We are having problems with them blocking some of our email. Please use another email provider.'));

    if (strlen($user_name) < 4) $errors->add('user_name', __('Username must be at least 4 characters.'));

    if (strpos(' ' . $user_name, '_') != false) $errors->add('user_name', __('Sorry, usernames may not contain the character &#8220;_&#8221;!'));
    ...
    if (is_array($limited_email_domains) && empty($limited_email_domains) == false) {
        ...
        if (in_array($emaildomain, $limited_email_domains) == false) $errors->add('user_email', __('Sorry, that email address is not allowed!'));
    }
    ...
    if ($signup != null) {
        ...
        if ($diff > 2 * DAY_IN_SECONDS) $wpdb->delete($wpdb->signups, array('user_login' => $user_name)); else
    ...
    if ($signup != null) {
        ...
        // If registered more than two days ago, cancel registration and let this signup go through.
        if ($diff > 2 * DAY_IN_SECONDS) $wpdb->delete($wpdb->signups, array('user_email' => $user_email)); else
...
function wpmu_validate_blog_signup($blogname, $blog_title, $user = '') {
    ...
    if (!is_subdomain_install()) {
        $illegal_names = array_merge($illegal_names, /**
    ...
    if (empty($blogname)) $errors->add('blogname', __('Please enter a site name.'));

    if (preg_match('/[^a-z0-9]+/', $blogname)) $errors->add('blogname', __('Only lowercase letters (a-z) and numbers are allowed.'));

    if (in_array($blogname, $illegal_names) == true) $errors->add('blogname', __('That name is not allowed.'));

    if (strlen($blogname) < 4 && !is_super_admin()) $errors->add('blogname', __('Site name must be at least 4 characters.'));

    if (strpos($blogname, '_') !== false) $errors->add('blogname', __('Sorry, site names may not contain the character &#8220;_&#8221;!'));

    // do not allow users to create a blog that conflicts with a page on the main blog.
    if (!is_subdomain_install() && $wpdb->get_var($wpdb->prepare("SELECT post_name FROM " . $wpdb->get_blog_prefix($current_site->blog_id) . "posts WHERE post_type = 'page' AND post_name = %s", $blogname))) $errors->add('blogname', __('Sorry, you may not use that site name.'));

    // all numeric?
    if (preg_match('/^[0-9]*$/', $blogname)) $errors->add('blogname', __('Sorry, site names must have letters too!'));
    ...
    if (empty($blog_title)) $errors->add('blog_title', __('Please enter a site title.'));
    ...
    if (domain_exists($mydomain, $path, $current_site->id)) $errors->add('blogname', __('Sorry, that site already exists!'));

    if (username_exists($blogname)) {
        if (is_object($user) == false || (is_object($user) && ($user->user_login != $blogname))) $errors->add('blogname', __('Sorry, that site is reserved!'));
    }
    ...
    if (!empty($signup)) {
        $diff = current_time('timestamp', true) - mysql2date('U', $signup->registered);
        // If registered more than two days ago, cancel registration and let this signup go through.
        if ($diff > 2 * DAY_IN_SECONDS) $wpdb->delete($wpdb->signups, array('domain' => $mydomain, 'path' => $path)); else
...
function wpmu_signup_blog_notification($domain, $path, $title, $user, $user_email, $key, $meta = array()) {
    ...
    if (!is_subdomain_install() || get_current_site()->id != 1) $activate_url = network_site_url("wp-activate.php?key=$key"); else
     ...
     * @param array $meta By default, contains the requested privacy setting and lang_id.
     */
        apply_filters('wpmu_signup_blog_notification_subject', __('[%1$s] Activate %2$s'), $domain, $path, $title, $user, $user_email, $key, $meta), $from_name, esc_url('http://' . $domain . $path));
...
function wpmu_signup_user_notification($user, $user_email, $key, $meta = array()) {
     ...
     * @param array $meta Signup meta data.
     */
        apply_filters('wpmu_signup_user_notification_subject', __('[%1$s] Activate %2$s'), $user, $user_email, $key, $meta), $from_name, $user);
    wp_mail($user_email, wp_specialchars_decode($subject), $message, $message_headers);
    return true;
}
...
function wpmu_activate_signup($key) {
    ...
    if (empty($signup)) return new WP_Error('invalid_key', __('Invalid activation key.'));

    if ($signup->active) {
        if (empty($signup->domain)) return new WP_Error('already_active', __('The user is already active.'), $signup); else
    ...
    if (!$user_id) $user_id = wpmu_create_user($signup->user_login, $password, $signup->user_email); else
        $user_already_exists = true;

    if (!$user_id) return new WP_Error('create_user', __('Could not create user'), $signup);
    ...
    if (empty($signup->domain)) {
        ...
        if (isset($user_already_exists)) return new WP_Error('user_already_exists', __('That username is already activated.'), $signup);
...
function wpmu_create_blog($domain, $path, $title, $user_id, $meta = array(), $site_id = 1) {
    ...
    if (is_subdomain_install()) $domain = str_replace('@', '', $domain);
    ...
    // Check if the domain has been used already. We should return an error message.
    if (domain_exists($domain, $path, $site_id)) return new WP_Error('blog_taken', __('Sorry, that site already exists!'));

    if (!defined('WP_INSTALLING')) define('WP_INSTALLING', true);

    if (!$blog_id = insert_blog($domain, $path, $site_id)) return new WP_Error('insert_blog', __('Could not create site.'));
    ...
    foreach ($meta as $key => $value) {
        if (in_array($key, array('public', 'archived', 'mature', 'spam', 'deleted', 'lang_id'))) update_blog_status($blog_id, $key, $value); else
    ...
    if (!is_super_admin($user_id) && !get_user_meta($user_id, 'primary_blog', true)) update_user_meta($user_id, 'primary_blog', $blog_id);
...
function newblog_notify_siteadmin($blog_id, $deprecated = '') {
    if (get_site_option('registrationnotification') != 'yes') return false;
...
function install_blog($blog_id, $blog_title = '') {
    ...
    if ($wpdb->get_results("DESCRIBE {$wpdb->posts}")) die('<h1>' . __('Already Installed') . '</h1><p>' . __('You appear to have already installed WordPress. To reinstall please clear your old database tables first.') . '</p></body></html>');
    ...
    if (get_site_option('ms_files_rewriting')) update_option('upload_path', UPLOADBLOGSDIR . "/$blog_id/files"); else
...
function wpmu_welcome_notification($blog_id, $user_id, $password, $title, $meta = array()) {
    ...
    if ($welcome_email == false) $welcome_email = __('Howdy USERNAME,
...
function get_dirsize($directory) {
    ...
    if (is_array($dirsize) && isset($dirsize[$directory]['size'])) return $dirsize[$directory]['size'];

    if (false == is_array($dirsize)) $dirsize = array();
...
function check_upload_mimes($mimes) {
    ...
    foreach ($site_exts as $ext) {
        foreach ($mimes as $ext_pattern => $mime) {
            if ($ext != '' && strpos($ext_pattern, $ext) !== false) $site_mimes[$ext_pattern] = $mime;
        }
    }
    return $site_mimes;
}
...
function wpmu_log_new_registrations($blog_id, $user_id) {
    ...
    if ($user) $wpdb->insert($wpdb->registration_log, array('email' => $user->user_email, 'IP' => preg_replace('/[^0-9., ]/', '', wp_unslash($_SERVER['REMOTE_ADDR'])), 'blog_id' => $blog_id, 'date_registered' => current_time('mysql')));
}
...
function global_terms($term_id, $deprecated = '') {
    ...
    if (!global_terms_enabled()) return $term_id;
    ...
    if ($global_id != $term_id) {
        if (get_option('default_category') == $term_id) update_option('default_category', $global_id);
...
function upload_is_file_too_big($upload) {
    if (is_array($upload) == false || defined('WP_IMPORTING') || get_site_option('upload_space_check_disabled')) return $upload;

    if (strlen($upload['bits']) > (1024 * get_site_option('fileupload_maxk', 1500))) return sprintf(__('This file is too big. Files must be less than %d KB in size.') . '<br />', get_site_option('fileupload_maxk', 1500));
...
function signup_nonce_check($result) {
    if (!strpos($_SERVER['PHP_SELF'], 'wp-signup.php')) return $result;

    if (wp_create_nonce('signup_form_' . $_POST['signup_form_id']) != $_POST['_signup_form']) wp_die(__('Please try again.'));
...
function maybe_redirect_404() {
    ...
    if (is_main_site() && is_404() && defined('NOBLOGREDIRECT') && ($destination = apply_filters('blog_redirect_404', NOBLOGREDIRECT))) {
        if ($destination == '%siteurl%') $destination = network_home_url();
...
function maybe_add_existing_user_to_blog() {
    if (false === strpos($_SERVER['REQUEST_URI'], '/newbloguser/')) return false;
    ...
    if (!empty($details)) delete_option('new_user_' . $key);

    if (empty($details) || is_wp_error(add_existing_user_to_blog($details))) wp_die(sprintf(__('An error occurred adding you to this site. Back to the <a href="%s">homepage</a>.'), home_url()));
...
function is_user_option_local($key, $user_id = 0, $blog_id = 0) {
    ...
    if (isset($current_user->$local_key)) return true;
...
function users_can_register_signup_filter() {
    ...
    if ($registration == 'all' || $registration == 'user') return true;
...
function filter_SSL($url) {
    if (!is_string($url)) return get_bloginfo('url'); // Return home blog url with proper scheme

    if (force_ssl_content() && is_ssl()) $url = set_url_scheme($url, 'https');
...
function wp_schedule_update_network_counts() {
    ...
    if (!wp_next_scheduled('update_network_counts') && !defined('WP_INSTALLING')) wp_schedule_event(time(), 'twicedaily', 'update_network_counts');
}
...
function wp_maybe_update_network_site_counts() {
     ...
     */
    if (!apply_filters('enable_live_network_counts', $is_small_network, 'sites')) return;
...
function wp_maybe_update_network_user_counts() {
    ...
    if (!apply_filters('enable_live_network_counts', $is_small_network, 'users')) return;
...
function get_space_allowed() {
    ...
    if (!is_numeric($space_allowed)) $space_allowed = get_site_option('blog_upload_space');

    if (empty($space_allowed) || !is_numeric($space_allowed)) $space_allowed = 100;
...
function get_upload_space_available() {
    ...
    if (get_site_option('upload_space_check_disabled')) return $space_allowed;
...
function is_upload_space_available() {
    if (get_site_option('upload_space_check_disabled')) return true;
...
function upload_size_limit_filter($size) {
    ...
    if (get_site_option('upload_space_check_disabled')) return min($size, $fileupload_maxk);
...
function wp_get_sites($args = array()) {
    ...
    if (wp_is_large_network()) return array();
    ...
    if (isset($args['public'])) $query .= $wpdb->prepare("AND public = %d ", $args['public']);

    if (isset($args['archived'])) $query .= $wpdb->prepare("AND archived = %d ", $args['archived']);

    if (isset($args['mature'])) $query .= $wpdb->prepare("AND mature = %d ", $args['mature']);

    if (isset($args['spam'])) $query .= $wpdb->prepare("AND spam = %d ", $args['spam']);

    if (isset($args['deleted'])) $query .= $wpdb->prepare("AND deleted = %d ", $args['deleted']);

    if (isset($args['limit']) && $args['limit']) {
        if (isset($args['offset']) && $args['offset']) $query .= $wpdb->prepare("LIMIT %d , %d ", $args['offset'], $args['limit']); else
