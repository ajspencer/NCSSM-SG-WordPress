...
function get_sidebar($name = null) {
    ...
    // Backward compat code will be removed in a future release
    if ('' == locate_template($templates, true)) load_template(ABSPATH . WPINC . '/theme-compat/sidebar.php');
}
...
function wp_loginout($redirect = '', $echo = true) {
    if (!is_user_logged_in()) $link = '<a href="' . esc_url(wp_login_url($redirect)) . '">' . __('Log in') . '</a>'; else
...
function wp_login_form($args = array()) {
    $defaults = array('echo' => true, 'redirect' => (is_ssl() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], // Default redirect is back to the current page
...
function wp_register($before = '<li>', $after = '</li>', $echo = true) {
    ...
    if (!is_user_logged_in()) {
        if (get_option('users_can_register')) $link = $before . '<a href="' . esc_url(wp_registration_url()) . '">' . __('Register') . '</a>' . $after; else
            $link = '';
    } else {
...
function get_bloginfo($show = '', $filter = 'raw') {
    ...
    switch ($show) {
        ...
        case 'siteurl' : // DEPRECATED
            _deprecated_argument(__FUNCTION__, '2.2', sprintf(/* translators: 1: 'siteurl'/'home' argument, 2: bloginfo() function name, 3: 'url' argument */
        ...
        case 'language':
            ...
            break;
        case 'text_direction':
            _deprecated_argument(__FUNCTION__, '2.2', sprintf(/* translators: 1: 'text_direction' argument, 2: bloginfo() function name, 3: is_rtl() function name */
    ...
    $url = true;
    if (strpos($show, 'url') === false && strpos($show, 'directory') === false && strpos($show, 'home') === false) $url = false;
...
function wp_title($sep = '&raquo;', $display = true, $seplocation = '') {
    ...
    if (is_post_type_archive()) {
        ...
        if (!$post_type_object->has_archive) $title = post_type_archive_title('', false);
    }
    ...
    if (is_archive() && !empty($year)) {
        $title = $year;
        if (!empty($monthnum)) $title .= $t_sep . $wp_locale->get_month($monthnum);
        if (!empty($day)) $title .= $t_sep . zeroise($day, 2);
    }
...
function single_post_title($prefix = '', $display = true) {
    ...
    if ($display) echo $prefix . $title; else
        return $prefix . $title;
}
...
function single_term_title($prefix = '', $display = true) {
    ...
    if ($display) echo $prefix . $term_name; else
...
function get_calendar($initial = true, $echo = true) {
    ...
    if (!empty($monthnum) && !empty($year)) {
        ...
        $thisyear = '' . intval(substr($m, 0, 4));
        if (strlen($m) < 6) $thismonth = '01'; else
    ...
    if (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false || stripos($_SERVER['HTTP_USER_AGENT'], 'camino') !== false || stripos($_SERVER['HTTP_USER_AGENT'], 'safari') !== false) $ak_title_separator = "\n"; else
        $ak_title_separator = ', ';
    ...
    if ($ak_post_titles) {
        foreach ((array)$ak_post_titles as $ak_post_title) {
            ...
            if (empty($ak_titles_for_day['day_' . $ak_post_title->dom])) $ak_titles_for_day['day_' . $ak_post_title->dom] = '';
            if (empty($ak_titles_for_day["$ak_post_title->dom"])) // first one
                $ak_titles_for_day["$ak_post_title->dom"] = $post_title; else
    ...
    for ($day = 1; $day <= $daysinmonth; ++$day) {
        ...
        if ($day == gmdate('j', current_time('timestamp')) && $thismonth == gmdate('m', current_time('timestamp')) && $thisyear == gmdate('Y', current_time('timestamp'))) $calendar_output .= '<td id="today">'; else
            $calendar_output .= '<td>';

        if (in_array($day, $daywithpost)) // any posts today?
            $calendar_output .= '<a href="' . get_day_link($thisyear, $thismonth, $day) . '" title="' . esc_attr($ak_titles_for_day[$day]) . "\">$day</a>"; else
            $calendar_output .= $day;
        $calendar_output .= '</td>';

        if (6 == calendar_week_mod(date('w', mktime(0, 0, 0, $thismonth, $day, $thisyear)) - $week_begins)) $newrow = true;
    }
...
function the_modified_date($d = '', $before = '', $after = '', $echo = true) {
    ...
    if ($echo) echo $the_modified_date; else
...
function get_the_modified_date($d = '') {
    if ('' == $d) $the_time = get_post_modified_time(get_option('date_format'), null, null, true); else
...
function get_the_time($d = '', $post = null) {
    ...
    if ('' == $d) $the_time = get_post_time(get_option('time_format'), false, $post, true); else
...
function get_post_time($d = 'U', $gmt = false, $post = null, $translate = false) {
    ...
    if ($gmt) $time = $post->post_date_gmt; else
        $time = $post->post_date;
...
function get_the_modified_time($d = '') {
    if ('' == $d) $the_time = get_post_modified_time(get_option('time_format'), null, null, true); else
        $the_time = get_post_modified_time($d, null, null, true);
...
function get_post_modified_time($d = 'U', $gmt = false, $post = null, $translate = false) {
    ...
    if ($gmt) $time = $post->post_modified_gmt; else
...
function feed_links($args = array()) {
    ...
    $defaults = array(/* translators: Separator between blog name and feed type in feed links */
        'separator' => _x('&raquo;', 'feed link'), /* translators: 1: blog title, 2: separator (raquo) */
        'feedtitle' => __('%1$s %2$s Feed'), /* translators: 1: blog title, 2: separator (raquo) */
...
function feed_links_extra($args = array()) {
    $defaults = array(/* translators: Separator between blog name and feed type in feed links */
        'separator' => _x('&raquo;', 'feed link'), /* translators: 1: blog name, 2: separator(raquo), 3: post title */
        'singletitle' => __('%1$s %2$s %3$s Comments Feed'), /* translators: 1: blog name, 2: separator(raquo), 3: category name */
        'cattitle' => __('%1$s %2$s %3$s Category Feed'), /* translators: 1: blog name, 2: separator(raquo), 3: tag name */
        'tagtitle' => __('%1$s %2$s %3$s Tag Feed'), /* translators: 1: blog name, 2: separator(raquo), 3: author name  */
        'authortitle' => __('%1$s %2$s Posts by %3$s Feed'), /* translators: 1: blog name, 2: separator(raquo), 3: search phrase */
        'searchtitle' => __('%1$s %2$s Search Results for &#8220;%3$s&#8221; Feed'), /* translators: 1: blog name, 2: separator(raquo), 3: post type name */
    ...
    if (is_singular()) {
        ...
        if ($post_type_obj) $href = get_post_type_archive_feed_link($post_type_obj->name);
    }

    if (isset($title) && isset($href)) echo '<link rel="alternate" type="' . feed_content_type() . '" title="' . esc_attr($title) . '" href="' . esc_url($href) . '" />' . "\n";
}
...
function noindex() {
    ...
    if ('0' == get_option('blog_public')) wp_no_robots();
}
...
function get_search_query($escaped = true) {
    ...
    if ($escaped) $query = esc_attr($query);
    return $query;
}
...
function language_attributes($doctype = 'html') {
    ...
    if ($lang = get_bloginfo('language')) {
        if (get_option('html_type') == 'text/html' || $doctype == 'html') $attributes[] = "lang=\"$lang\"";
...
function paginate_links($args = '') {
    ...
    $defaults = array('base' => $pagenum_link, // http://example.com/all_posts.php%_% : %_% is replaced by format (below)
        'format' => $format, // ?page=%#% : %#% is replaced by the page number
        'total' => $total, 'current' => $current, 'show_all' => false, 'prev_next' => true, 'prev_text' => __('&laquo; Previous'), 'next_text' => __('Next &raquo;'), 'end_size' => 1, 'mid_size' => 2, 'type' => 'plain', 'add_args' => array(), // array of query args to add
...
function wp_admin_css_color($key, $name, $url, $colors = array(), $icons = array()) {
    ...
    if (!isset($_wp_admin_css_colors)) $_wp_admin_css_colors = array();
...
function register_admin_color_schemes() {
    ...
    if (false !== strpos($GLOBALS['wp_version'], '-src')) return;
...
function wp_admin_css($file = 'wp-admin', $force_echo = false) {
    ...
    if ($wp_styles->query($handle)) {
        if ($force_echo || did_action('wp_print_styles')) // we already printed the style queue. Print this one immediately
            wp_print_styles($handle); else // Add to style queue
...
function add_thickbox() {
    ...
    if (is_network_admin()) add_action('admin_head', '_thickbox_path_admin_subfolder');
}
...
function get_the_generator($type = '') {
    if (empty($type)) {
        ...
        if (empty($current_filter)) return;
...
function __checked_selected_helper($helper, $current, $echo, $type) {
    if ((string)$helper === (string)$current) $result = " $type='$type'"; else
        $result = '';
