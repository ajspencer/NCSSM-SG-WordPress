...
function get_category_parents($id, $link = false, $separator = '/', $nicename = false, $visited = array()) {
    ...
    if ($nicename) $name = $parent->slug; else
        $name = $parent->name;
    ...
    if ($link) $chain .= '<a href="' . esc_url(get_category_link($parent->term_id)) . '">' . $name . '</a>' . $separator; else
...
function _usort_terms_by_ID($a, $b) {
    if ($a->term_id > $b->term_id) return 1; elseif ($a->term_id < $b->term_id) return -1;
    else
        return 0;
}
...
function wp_list_categories($args = '') {
    ...
    if ($r['title_li'] && 'list' == $r['style']) $output .= '</ul></li>';
...
function wp_tag_cloud($args = '') {
    ...
    foreach ($tags as $key => $tag) {
        if ('edit' == $args['link']) $link = get_edit_term_link($tag->term_id, $tag->taxonomy, $args['post_type']); else
    ...
    if ('array' == $args['format'] || empty($args['echo'])) return $return;
...
function wp_generate_tag_cloud($tags, $args = '') {
    ...
    if ($args['number'] > 0) $tags = array_slice($tags, 0, $args['number']);
    ...
    foreach ($tags as $key => $tag) {
        ...
        $a[] = "<a href='$tag_link' class='tag-link-$tag_id' title='" . esc_attr($title_attribute) . "' style='font-size: " . str_replace(',', '.', ($args['smallest'] + (($count - $min_count) * $font_step))) . $args['unit'] . ";'>$tag_name</a>";
    }
...
function walk_category_dropdown_tree() {
    $args = func_get_args();
    // the user's options are the third parameter
    if (empty($args[2]['walker']) || !is_a($args[2]['walker'], 'Walker')) $walker = new Walker_CategoryDropdown; else
        $walker = $args[2]['walker'];
...
class Walker_CategoryDropdown extends Walker {
    ...
    public function start_el(&$output, $category, $depth = 0, $args = array(), $id = 0) {
        ...
        $output .= '>';
        $output .= $pad . $cat_name;
        if ($args['show_count']) $output .= '&nbsp;&nbsp;(' . number_format_i18n($category->count) . ')';
        $output .= "</option>\n";
    }
}
...
function get_tag_link($tag) {
    if (!is_object($tag)) $tag = (int)$tag;

    $tag = get_term_link($tag, 'post_tag');

    if (is_wp_error($tag)) return '';
