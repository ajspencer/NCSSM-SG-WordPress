...
if (!isset($wp_filter)) $wp_filter = array();

if (!isset($wp_actions)) $wp_actions = array();

if (!isset($merged_filters)) $merged_filters = array();

if (!isset($wp_current_filter)) $wp_current_filter = array();
...
function has_filter($tag, $function_to_check = false) {
    ...
    if (false === $function_to_check || false == $has) return $has;

    if (!$idx = _wp_filter_build_unique_id($tag, $function_to_check, false)) return false;

    foreach ((array)array_keys($wp_filter[$tag]) as $priority) {
        if (isset($wp_filter[$tag][$priority][$idx])) return $priority;
    }
...
function apply_filters($tag, $value) {
    ...
    if (!isset($wp_filter[$tag])) {
        if (isset($wp_filter['all'])) array_pop($wp_current_filter);
        return $value;
    }

    if (!isset($wp_filter['all'])) $wp_current_filter[] = $tag;
    ...
    do {
        foreach ((array)current($wp_filter[$tag]) as $the_) if (!is_null($the_['function'])) {
            $args[1] = $value;
...
function apply_filters_ref_array($tag, $args) {
    ...
    do {
        foreach ((array)current($wp_filter[$tag]) as $the_) if (!is_null($the_['function'])) $args[0] = call_user_func_array($the_['function'], array_slice($args, 0, (int)$the_['accepted_args']));
...
function do_action($tag, $arg = '') {
    ...
    if (!isset($wp_actions[$tag])) $wp_actions[$tag] = 1; else
        ++$wp_actions[$tag];
    ...
    do {
        foreach ((array)current($wp_filter[$tag]) as $the_) if (!is_null($the_['function'])) call_user_func_array($the_['function'], array_slice($args, 0, (int)$the_['accepted_args']));
...
function did_action($tag) {
    ...
    if (!isset($wp_actions[$tag])) return 0;
...
function _wp_call_all_hook($args) {
    ...
    reset($wp_filter['all']);
    do {
        foreach ((array)current($wp_filter['all']) as $the_) if (!is_null($the_['function'])) call_user_func_array($the_['function'], $args);
    ...
    if (is_object($function[0])) {
        ...
        if (function_exists('spl_object_hash')) {
            ...
            if (!isset($function[0]->wp_filter_id)) {
                if (false === $priority) return false;
