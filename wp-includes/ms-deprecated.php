...
function get_dashboard_blog() {
    ...
    if ($blog = get_site_option('dashboard_blog')) return get_blog_details($blog);
...
function wpmu_admin_do_redirect($url = '') {
    ...
    if (isset($_GET['redirect'])) {
        if (substr($_GET['redirect'], 0, 2) == 's_') $url .= '&action=blogs&s=' . esc_html(substr($_GET['redirect'], 2));
...
function wpmu_admin_redirect_add_updated_param($url = '') {
    ...
    if (strpos($url, 'updated=true') === false) {
        if (strpos($url, '?') === false) return $url . '?updated=true'; else
...
function get_user_id_from_string($string) {
    ...
    if (is_email($string)) $user = get_user_by('email', $string); elseif (is_numeric($string)) return $string;
    else
