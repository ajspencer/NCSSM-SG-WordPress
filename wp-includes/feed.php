...
function get_the_category_rss($type = null) {
    ...
    foreach ($cat_names as $cat_name) {
        if ('rdf' == $type) $the_list .= "\t\t<dc:subject><![CDATA[$cat_name]]></dc:subject>\n"; elseif ('atom' == $type) $the_list .= sprintf('<category scheme="%1$s" term="%2$s" />', esc_attr(get_bloginfo_rss('url')), esc_attr($cat_name));
        else
...
function html_type_rss() {
    ...
    if (strpos($type, 'xhtml') !== false) $type = 'xhtml'; else
        $type = 'html';
    echo $type;
}
