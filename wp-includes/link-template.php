...
function user_trailingslashit($string, $type_of_url = '') {
    global $wp_rewrite;
    if ($wp_rewrite->use_trailing_slashes) $string = trailingslashit($string); else
...
function get_permalink($id = 0, $leavename = false) {
    ...
    if ($post->post_type == 'page') return get_page_link($post, $leavename, $sample); elseif ($post->post_type == 'attachment') return get_attachment_link($post, $leavename);
    elseif (in_array($post->post_type, get_post_types(array('_builtin' => false)))) return get_post_permalink($post, $leavename, $sample);
    ...
    if ('' != $permalink && !in_array($post->post_status, array('draft', 'pending', 'auto-draft', 'future'))) {
        ...
        if (strpos($permalink, '%category%') !== false) {
            ...
            if ($cats) {
                ...
                if ($parent = $category_object->parent) $category = get_category_parents($parent, false, '/', true) . $category;
            }
...
function get_post_permalink($id = 0, $leavename = false, $sample = false) {
    ...
    if (!empty($post_link) && (!$draft_or_pending || $sample)) {
        ...
        if ($post_type->query_var && (isset($post->post_status) && !$draft_or_pending)) $post_link = add_query_arg($post_type->query_var, $slug, ''); else
...
function post_permalink($post_id = 0, $deprecated = '') {
    if (!empty($deprecated)) _deprecated_argument(__FUNCTION__, '1.3');
...
function get_page_link($post = false, $leavename = false, $sample = false) {
    ...
    if ('page' == get_option('show_on_front') && $post->ID == get_option('page_on_front')) $link = home_url('/'); else
...
function get_attachment_link($post = null, $leavename = false) {
    ...
    if ($wp_rewrite->using_permalinks() && $parent) {
        if ('page' == $parent->post_type) $parentlink = _get_page_link($post->post_parent); // Ignores page_on_front
        else
            $parentlink = get_permalink($post->post_parent);

        if (is_numeric($post->post_name) || false !== strpos(get_option('permalink_structure'), '%category%')) $name = 'attachment/' . $post->post_name; // <permalink>/<int>/ is paged so we use the explicit attachment marker
        else
            $name = $post->post_name;

        if (strpos($parentlink, '?') === false) $link = user_trailingslashit(trailingslashit($parentlink) . '%postname%');

        if (!$leavename) $link = str_replace('%postname%', $name, $link);
    }

    if (!$link) $link = home_url('/?attachment_id=' . $post->ID);
...
function get_post_comments_feed_link($post_id = 0, $feed = '') {
    ...
    if ('' != get_option('permalink_structure')) {
        if ('page' == get_option('show_on_front') && $post_id == get_option('page_on_front')) $url = _get_page_link($post_id); else
            $url = get_permalink($post_id);

        $url = trailingslashit($url) . 'feed';
        if ($feed != get_default_feed()) $url .= "/$feed";
        $url = user_trailingslashit($url, 'single_feed');
    } else {
        $type = get_post_field('post_type', $post_id);
        if ('page' == $type) $url = add_query_arg(array('feed' => $feed, 'page_id' => $post_id), home_url('/')); else
...
function post_comments_feed_link($link_text = '', $post_id = '', $feed = '') {
    ...
    if (empty($link_text)) $link_text = __('Comments Feed');
...
function get_author_feed_link($author_id, $feed = '') {
    ...
    if ('' == $permalink_structure) {
        ...
        if ($feed == get_default_feed()) $feed_link = 'feed'; else
            $feed_link = "feed/$feed";
...
function edit_term_link($link = '', $before = '', $after = '', $term = null, $echo = true) {
    if (is_null($term)) $term = get_queried_object();
    ...
    if (empty($link)) $link = __('Edit This');
...
function get_search_link($query = '') {
    ...
    if (empty($query)) $search = get_search_query(false); else
...
function get_search_comments_feed_link($search_query = '', $feed = '') {
    ...
    if (empty($permastruct)) $link = add_query_arg('feed', 'comments-' . $feed, $link); else
...
function get_post_type_archive_link($post_type) {
    global $wp_rewrite;
    if (!$post_type_obj = get_post_type_object($post_type)) return false;

    if (!$post_type_obj->has_archive) return false;

    if (get_option('permalink_structure') && is_array($post_type_obj->rewrite)) {
        $struct = (true === $post_type_obj->has_archive) ? $post_type_obj->rewrite['slug'] : $post_type_obj->has_archive;
        if ($post_type_obj->rewrite['with_front']) $struct = $wp_rewrite->front . $struct; else
...
function get_post_type_archive_feed_link($post_type, $feed = '') {
    ...
    if (!$link = get_post_type_archive_link($post_type)) return false;
...
function get_edit_post_link($id = 0, $context = 'display') {
    ...
    if ('revision' === $post->post_type) $action = ''; elseif ('display' == $context) $action = '&amp;action=edit';
    else
        $action = '&action=edit';
...
function get_delete_post_link($id = 0, $deprecated = '', $force_delete = false) {
    if (!empty($deprecated)) _deprecated_argument(__FUNCTION__, '3.0');
    ...
    if (!current_user_can('delete_post', $post->ID)) return;
...
function get_edit_user_link($user_id = null) {
    ...
    if (empty($user_id) || !current_user_can('edit_user', $user_id)) return '';

    $user = get_userdata($user_id);

    if (!$user) return '';

    if (get_current_user_id() == $user->ID) $link = get_edit_profile_url($user->ID); else
...
function get_adjacent_post($in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category') {
    ...
    if ((!$post = get_post()) || !taxonomy_exists($taxonomy)) return null;
    ...
    if ($in_same_term || !empty($excluded_terms)) {
        ...
        if ($in_same_term) {
            if (!is_object_in_taxonomy($post->post_type, $taxonomy)) return '';
            ...
            if (!$term_array || is_wp_error($term_array)) return '';
    ...
    if (false !== $result) {
        if ($result) $result = get_post($result);
        return $result;
    }
...
function get_adjacent_post_rel_link($title = '%title', $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category') {
    if ($previous && is_attachment() && $post = get_post()) $post = get_post($post->post_parent); else
    ...
    if (empty($post_title)) $post_title = $previous ? __('Previous Post') : __('Next Post');
...
function get_boundary_post($in_same_term = false, $excluded_terms = '', $start = true, $taxonomy = 'category') {
    $post = get_post();
    if (!$post || !is_single() || is_attachment() || !taxonomy_exists($taxonomy)) return null;
    ...
    if (!is_array($excluded_terms)) {
        if (!empty($excluded_terms)) $excluded_terms = explode(',', $excluded_terms); else
            $excluded_terms = array();
    }

    if ($in_same_term || !empty($excluded_terms)) {
        if ($in_same_term) $term_array = wp_get_object_terms($post->ID, $taxonomy, array('fields' => 'ids'));
...
function get_adjacent_post_link($format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category') {
    if ($previous && is_attachment()) $post = get_post(get_post()->post_parent); else
        $post = get_adjacent_post($in_same_term, $excluded_terms, $previous, $taxonomy);
...
function get_pagenum_link($pagenum = 1, $escape = true) {
    ...
    if (!$wp_rewrite->using_permalinks() || is_admin()) {
        ...
        if ($wp_rewrite->using_index_permalinks() && ($pagenum > 1 || '' != $request)) $base .= $wp_rewrite->index . '/';
    ...
    if ($escape) return esc_url($result); else
...
function get_next_posts_page_link($max_page = 0) {
    ...
    if (!is_single()) {
        ...
        if (!$max_page || $max_page >= $nextpage) return get_pagenum_link($nextpage);
    }
}
...
function get_comments_pagenum_link($pagenum = 1, $max_page = 0) {
    ...
    if ('newest' == get_option('default_comments_page')) {
        if ($pagenum != $max_page) {
            if ($wp_rewrite->using_permalinks()) $result = user_trailingslashit(trailingslashit($result) . $wp_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged'); else
...
function get_next_comments_link($label = '', $max_page = 0) {
    ...
    if (empty($max_page)) $max_page = $wp_query->max_num_comment_pages;

    if (empty($max_page)) $max_page = get_comment_pages_count();

    if ($nextpage > $max_page) return;

    if (empty($label)) $label = __('Newer Comments &raquo;');
...
function get_previous_comments_link($label = '') {
    ...
    if (intval($page) <= 1) return;

    $prevpage = intval($page) - 1;

    if (empty($label)) $label = __('&laquo; Older Comments');
...
function paginate_comments_links($args = array()) {
    ...
    if ($wp_rewrite->using_permalinks()) $defaults['base'] = user_trailingslashit(trailingslashit(get_permalink()) . $wp_rewrite->comments_pagination_base . '-%#%', 'commentpaged');

    $args = wp_parse_args($args, $defaults);
    $page_links = paginate_links($args);

    if ($args['echo']) echo $page_links; else
        return $page_links;
}
...
function get_home_url($blog_id = null, $path = '', $scheme = null) {
    ...
    if (!in_array($scheme, array('http', 'https', 'relative'))) {
        if (is_ssl() && !is_admin() && 'wp-login.php' !== $GLOBALS['pagenow']) $scheme = 'https'; else
...
function plugins_url($path = '', $plugin = '') {
    ...
    if (!empty($plugin) && 0 === strpos($plugin, $mu_plugin_dir)) $url = WPMU_PLUGIN_URL; else
        $url = WP_PLUGIN_URL;
...
function network_site_url($path = '', $scheme = null) {
    ...
    if ('relative' == $scheme) $url = $current_site->path; else
...
function network_home_url($path = '', $scheme = null) {
    if (!is_multisite()) return home_url($path, $scheme);

    $current_site = get_current_site();
    $orig_scheme = $scheme;

    if (!in_array($scheme, array('http', 'https', 'relative'))) $scheme = is_ssl() && !is_admin() ? 'https' : 'http';
...
function self_admin_url($path = '', $scheme = 'admin') {
    if (is_network_admin()) return network_admin_url($path, $scheme); elseif (is_user_admin()) return user_admin_url($path, $scheme);
    else
...
function get_dashboard_url($user_id = 0, $path = '', $scheme = 'admin') {
    ...
    if (!is_super_admin() && empty($blogs)) {
        ...
        if ($current_blog && (is_super_admin($user_id) || in_array($current_blog, array_keys($blogs)))) {
            ...
            if ($active) $url = get_admin_url($active->blog_id, $path, $scheme); else
                $url = user_admin_url($path, $scheme);
        }
    }
...
function get_edit_profile_url($user_id = 0, $scheme = 'admin') {
    ...
    if (is_user_admin()) $url = user_admin_url('profile.php', $scheme); elseif (is_network_admin()) $url = network_admin_url('profile.php', $scheme);
    else
...
function the_shortlink($text = '', $title = '', $before = '', $after = '') {
    ...
    if (empty($text)) $text = __('This is the short link.');

    if (empty($title)) $title = the_title_attribute(array('echo' => false));
...
function get_avatar_data($id_or_email, $args = null) {
    $args = wp_parse_args($args, array('size' => 96, 'height' => null, 'width' => null, 'default' => get_option('avatar_default', 'mystery'), 'force_default' => false, 'rating' => get_option('avatar_rating'), 'scheme' => null, 'processed_args' => null, // if used, should be a reference
        'extra_attr' => '',));
