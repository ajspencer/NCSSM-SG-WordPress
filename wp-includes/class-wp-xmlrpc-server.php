...
class wp_xmlrpc_server extends IXR_Server {
    ...
    public function __construct() {
        $this->methods = array(// WordPress API
            ...
            'wp.getTags' => 'this:wp_getTags', 'wp.newCategory' => 'this:wp_newCategory', 'wp.deleteCategory' => 'this:wp_deleteCategory', 'wp.suggestCategories' => 'this:wp_suggestCategories', 'wp.uploadFile' => 'this:mw_newMediaObject',    // Alias
            'wp.deleteFile' => 'this:wp_deletePost',        // Alias
            'wp.getCommentCount' => 'this:wp_getCommentCount', 'wp.getPostStatusList' => 'this:wp_getPostStatusList', 'wp.getPageStatusList' => 'this:wp_getPageStatusList', 'wp.getPageTemplates' => 'this:wp_getPageTemplates', 'wp.getOptions' => 'this:wp_getOptions', 'wp.setOptions' => 'this:wp_setOptions', 'wp.getComment' => 'this:wp_getComment', 'wp.getComments' => 'this:wp_getComments', 'wp.deleteComment' => 'this:wp_deleteComment', 'wp.editComment' => 'this:wp_editComment', 'wp.newComment' => 'this:wp_newComment', 'wp.getCommentStatusList' => 'this:wp_getCommentStatusList', 'wp.getMediaItem' => 'this:wp_getMediaItem', 'wp.getMediaLibrary' => 'this:wp_getMediaLibrary', 'wp.getPostFormats' => 'this:wp_getPostFormats', 'wp.getPostType' => 'this:wp_getPostType', 'wp.getPostTypes' => 'this:wp_getPostTypes', 'wp.getRevisions' => 'this:wp_getRevisions', 'wp.restoreRevision' => 'this:wp_restoreRevision',

            // Blogger API
            'blogger.getUsersBlogs' => 'this:blogger_getUsersBlogs', 'blogger.getUserInfo' => 'this:blogger_getUserInfo', 'blogger.getPost' => 'this:blogger_getPost', 'blogger.getRecentPosts' => 'this:blogger_getRecentPosts', 'blogger.newPost' => 'this:blogger_newPost', 'blogger.editPost' => 'this:blogger_editPost', 'blogger.deletePost' => 'this:blogger_deletePost',

            // MetaWeblog API (with MT extensions to structs)
            'metaWeblog.newPost' => 'this:mw_newPost', 'metaWeblog.editPost' => 'this:mw_editPost', 'metaWeblog.getPost' => 'this:mw_getPost', 'metaWeblog.getRecentPosts' => 'this:mw_getRecentPosts', 'metaWeblog.getCategories' => 'this:mw_getCategories', 'metaWeblog.newMediaObject' => 'this:mw_newMediaObject',
    ...
    public function escape(&$data) {
        if (!is_array($data)) return wp_slash($data);

        foreach ($data as &$v) {
            if (is_array($v)) $this->escape($v); elseif (!is_object($v)) $v = wp_slash($v);
        }
    }
    ...
    public function get_custom_fields($post_id) {
        ...
        foreach ((array)has_meta($post_id) as $meta) {
            ...
            if (!current_user_can('edit_post_meta', $post_id, $meta['meta_key'])) continue;
        ...
        foreach ((array)$fields as $meta) {
            if (isset($meta['id'])) {
                ...
                if (isset($meta['key'])) {
                    ...
                    if (current_user_can('edit_post_meta', $post_id, $meta['key'])) update_metadata_by_mid('post', $meta['id'], $meta['value']);
    ...
    public function wp_getUsersBlogs($args) {
        ...
        foreach ($blogs as $blog) {
            ...
            if ($blog->site_id != get_current_site()->id) continue;
            ...
            $struct[] = array('isAdmin' => $is_admin, 'url' => home_url('/'), 'blogid' => (string)$blog_id, 'blogName' => get_option('blogname'), 'xmlrpc' => site_url('xmlrpc.php', 'rpc'),);
    ...
    protected function _prepare_taxonomy($taxonomy, $fields) {
        ...
        if (in_array('labels', $fields)) $_taxonomy['labels'] = (array)$taxonomy->labels;

        if (in_array('cap', $fields)) $_taxonomy['cap'] = (array)$taxonomy->cap;

        if (in_array('menu', $fields)) $_taxonomy['show_in_menu'] = (bool)$_taxonomy->show_in_menu;

        if (in_array('object_type', $fields)) $_taxonomy['object_type'] = array_unique((array)$taxonomy->object_type);
    ...
    protected function _prepare_post($post, $fields) {
        ...
        if ($post_fields['post_status'] === 'future') $post_fields['post_status'] = 'publish';
        ...
        if (in_array('custom_fields', $fields)) $_post['custom_fields'] = $this->get_custom_fields($post['ID']);
    ...
    protected function _prepare_post_type($post_type, $fields) {
        ...
        if (in_array('taxonomies', $fields)) $_post_type['taxonomies'] = get_object_taxonomies($post_type->name, 'names');
    ...
    protected function _prepare_media_item($media_item, $thumbnail_size = 'thumbnail') {
        ...
        if ($thumbnail_src) $_media_item['thumbnail'] = $thumbnail_src[0]; else
    ...
    protected function _prepare_user($user, $fields) {
        ...
        $user_fields = array('username' => $user->user_login, 'first_name' => $user->user_firstname, 'last_name' => $user->user_lastname, 'registered' => $this->_convert_date($user->user_registered), 'bio' => $user->user_description, 'email' => $user->user_email, 'nickname' => $user->nickname, 'nicename' => $user->user_nicename, 'url' => $user->user_url, 'display_name' => $user->display_name, 'roles' => $user->roles,);
    ...
    public function wp_newPost($args) {
        if (!$this->minimum_args($args, 4)) return $this->error;
    ...
    protected function _insert_post($user, $content_struct) {
        ...
        if (!$post_type) return new IXR_Error(403, __('Invalid post type'));

        $update = !empty($post_data['ID']);

        if ($update) {
            if (!get_post($post_data['ID'])) return new IXR_Error(401, __('Invalid post ID.'));
            if (!current_user_can('edit_post', $post_data['ID'])) return new IXR_Error(401, __('Sorry, you are not allowed to edit this post.'));
            if ($post_data['post_type'] != get_post_type($post_data['ID'])) return new IXR_Error(401, __('The post type may not be changed.'));
        } else {
            if (!current_user_can($post_type->cap->create_posts) || !current_user_can($post_type->cap->edit_posts)) return new IXR_Error(401, __('Sorry, you are not allowed to post on this site.'));
        }
        ...
        switch ($post_data['post_status']) {
            ...
            case 'pending':
                break;
            case 'private':
                if (!current_user_can($post_type->cap->publish_posts)) return new IXR_Error(401, __('Sorry, you are not allowed to create private posts in this post type'));
                break;
            case 'publish':
            case 'future':
                if (!current_user_can($post_type->cap->publish_posts)) return new IXR_Error(401, __('Sorry, you are not allowed to publish posts in this post type'));
                break;
            default:
                if (!get_post_status_object($post_data['post_status'])) $post_data['post_status'] = 'draft';
                break;
        }
        ...
        if (!empty($post_data['post_author']) && $post_data['post_author'] != $user->ID) {
            if (!current_user_can($post_type->cap->edit_others_posts)) return new IXR_Error(401, __('You are not allowed to create posts as this user.'));

            $author = get_userdata($post_data['post_author']);

            if (!$author) return new IXR_Error(404, __('Invalid author ID.'));
        } else {
            $post_data['post_author'] = $user->ID;
        }

        if (isset($post_data['comment_status']) && $post_data['comment_status'] != 'open' && $post_data['comment_status'] != 'closed') unset($post_data['comment_status']);

        if (isset($post_data['ping_status']) && $post_data['ping_status'] != 'open' && $post_data['ping_status'] != 'closed') unset($post_data['ping_status']);
        ...
        if (!isset($post_data['ID'])) $post_data['ID'] = get_default_post_to_edit($post_data['post_type'], true)->ID;
        ...
        if ($post_data['post_type'] == 'post') {
            ...
            if ($post_data['post_status'] == 'private' || !empty($post_data['post_password'])) {
                ...
                if (!empty($post_data['sticky'])) return new IXR_Error(401, __('Sorry, you cannot stick a private post.'));
                if ($update) unstick_post($post_ID);
                ...
                if ($post_data['sticky']) stick_post($post_ID); else
        ...
        if (isset($post_data['post_thumbnail'])) {
            ...
            if (!$post_data['post_thumbnail']) delete_post_thumbnail($post_ID); elseif (!get_post(absint($post_data['post_thumbnail']))) return new IXR_Error(404, __('Invalid attachment ID.'));
            set_post_thumbnail($post_ID, $post_data['post_thumbnail']);
            unset($content_struct['post_thumbnail']);
        }

        if (isset($post_data['custom_fields'])) $this->set_custom_fields($post_ID, $post_data['custom_fields']);
        ...
        if (isset($post_data['terms']) || isset($post_data['terms_names'])) {
            ...
            if (isset($post_data['terms']) && is_array($post_data['terms'])) {
                ...
                foreach ($taxonomies as $taxonomy) {
                    if (!array_key_exists($taxonomy, $post_type_taxonomies)) return new IXR_Error(401, __('Sorry, one of the given taxonomies is not supported by the post type.'));
            ...
            if (isset($post_data['terms_names']) && is_array($post_data['terms_names'])) {
                ...
                foreach ($taxonomies as $taxonomy) {
                    ...
                    foreach ($term_names as $term_name) {
                        if (in_array($term_name, $ambiguous_terms)) return new IXR_Error(401, __('Ambiguous term name used in a hierarchical taxonomy. Please use term ID instead.'));

                        $term = get_term_by('name', $term_name, $taxonomy);

                        if (!$term) {
                            // term doesn't exist, so check that the user is allowed to create new terms
                            if (!current_user_can($post_type_taxonomies[$taxonomy]->cap->edit_terms)) return new IXR_Error(401, __('Sorry, you are not allowed to add a term to one of the given taxonomies.'));

                            // create the new term
                            $term_info = wp_insert_term($term_name, $taxonomy);
                            if (is_wp_error($term_info)) return new IXR_Error(500, $term_info->get_error_message());
        ...
        if (isset($post_data['post_format'])) {
            ...
            if (is_wp_error($format)) return new IXR_Error(500, $format->get_error_message());
        ...
        if (is_wp_error($post_ID)) return new IXR_Error(500, $post_ID->get_error_message());

        if (!$post_ID) return new IXR_Error(401, __('Sorry, your entry could not be posted. Something wrong happened.'));
    ...
    public function wp_editPost($args) {
        if (!$this->minimum_args($args, 5)) return $this->error;
        ...
        // ignore the existing GMT date if it is empty or a non-GMT date was supplied in $content_struct,
        // since _insert_post will ignore the non-GMT date if the GMT date is set
        if ($post['post_date_gmt'] == '0000-00-00 00:00:00' || isset($content_struct['post_date'])) unset($post['post_date_gmt']); else
    ...
    public function wp_deletePost($args) {
        ...
        if (!$result) return new IXR_Error(500, __('The post cannot be deleted.'));
    ...
    public function wp_getPost($args) {
        ...
        if (!current_user_can('edit_post', $post_id)) return new IXR_Error(401, __('Sorry, you cannot edit this post.'));
    ...
    public function wp_getPosts($args) {
        ...
        if (isset($filter['post_type'])) {
            ...
            if (!((bool)$post_type)) return new IXR_Error(403, __('The post type specified is not valid'));
        } else {
        ...
        if (isset($filter['number'])) $query['numberposts'] = absint($filter['number']);

        if (isset($filter['offset'])) $query['offset'] = absint($filter['offset']);
    ...
    public function wp_newTerm($args) {
        ...
        if (!taxonomy_exists($content_struct['taxonomy'])) return new IXR_Error(403, __('Invalid taxonomy'));

        $taxonomy = get_taxonomy($content_struct['taxonomy']);

        if (!current_user_can($taxonomy->cap->manage_terms)) return new IXR_Error(401, __('You are not allowed to create terms in this taxonomy.'));
        ...
        if (isset($content_struct['parent'])) {
            if (!$taxonomy['hierarchical']) return new IXR_Error(403, __('This taxonomy is not hierarchical.'));

            $parent_term_id = (int)$content_struct['parent'];
            $parent_term = get_term($parent_term_id, $taxonomy['name']);

            if (is_wp_error($parent_term)) return new IXR_Error(500, $parent_term->get_error_message());

            if (!$parent_term) return new IXR_Error(403, __('Parent term does not exist.'));
        ...
        if (is_wp_error($term)) return new IXR_Error(500, $term->get_error_message());

        if (!$term) return new IXR_Error(500, __('Sorry, your term could not be created. Something wrong happened.'));
    ...
    public function wp_editTerm($args) {
        ...
        if (!current_user_can($taxonomy->cap->edit_terms)) return new IXR_Error(401, __('You are not allowed to edit terms in this taxonomy.'));
        ...
        if (!$term) return new IXR_Error(404, __('Invalid term ID'));
        ...
        if (isset($content_struct['parent'])) {
            if (!$taxonomy['hierarchical']) return new IXR_Error(403, __("This taxonomy is not hierarchical so you can't set a parent."));
        ...
        if (!$term) return new IXR_Error(500, __('Sorry, editing the term failed.'));
    ...
    public function wp_deleteTerm($args) {
        ...
        if (!current_user_can($taxonomy->cap->delete_terms)) return new IXR_Error(401, __('You are not allowed to delete terms in this taxonomy.'));
        ...
        if (is_wp_error($result)) return new IXR_Error(500, $term->get_error_message());

        if (!$result) return new IXR_Error(500, __('Sorry, deleting the term failed.'));
    ...
    public function wp_getTerm($args) {
        ...
        if (!current_user_can($taxonomy->cap->assign_terms)) return new IXR_Error(401, __('You are not allowed to assign terms in this taxonomy.'));
    ...
    public function wp_getTerms($args) {
        ...
        if (isset($filter['number'])) $query['number'] = absint($filter['number']);
        ...
        if (isset($filter['hide_empty'])) $query['hide_empty'] = $filter['hide_empty']; else
            $query['get'] = 'all';
        ...
        if (is_wp_error($terms)) return new IXR_Error(500, $terms->get_error_message());
    ...
    public function wp_getUser($args) {
        ...
        if (!current_user_can('edit_user', $user_id)) return new IXR_Error(401, __('Sorry, you cannot edit users.'));

        $user_data = get_userdata($user_id);

        if (!$user_data) return new IXR_Error(404, __('Invalid user ID'));
    ...
    public function wp_getUsers($args) {
        ...
        if (!current_user_can('list_users')) return new IXR_Error(401, __('Sorry, you cannot list users.'));
        ...
        if (isset($filter['role'])) {
            if (get_role($filter['role']) === null) return new IXR_Error(403, __('The role specified is not valid'));
    ...
    public function wp_getProfile($args) {
        ...
        if (!current_user_can('edit_user', $user->ID)) return new IXR_Error(401, __('Sorry, you cannot edit your profile.'));
    ...
    public function wp_editProfile($args) {
        ...
        if (isset($content_struct['first_name'])) $user_data['first_name'] = $content_struct['first_name'];

        if (isset($content_struct['last_name'])) $user_data['last_name'] = $content_struct['last_name'];

        if (isset($content_struct['url'])) $user_data['user_url'] = $content_struct['url'];

        if (isset($content_struct['display_name'])) $user_data['display_name'] = $content_struct['display_name'];

        if (isset($content_struct['nickname'])) $user_data['nickname'] = $content_struct['nickname'];

        if (isset($content_struct['nicename'])) $user_data['user_nicename'] = $content_struct['nicename'];

        if (isset($content_struct['bio'])) $user_data['description'] = $content_struct['bio'];

        $result = wp_update_user($user_data);

        if (is_wp_error($result)) return new IXR_Error(500, $result->get_error_message());

        if (!$result) return new IXR_Error(500, __('Sorry, the user cannot be updated.'));
    ...
    public function wp_getPage($args) {
        ...
        if (!$page) return new IXR_Error(404, __('Invalid post ID.'));

        if (!current_user_can('edit_page', $page_id)) return new IXR_Error(401, __('Sorry, you cannot edit this page.'));
    ...
    public function wp_getPages($args) {
        ...
        if (!current_user_can('edit_pages')) return new IXR_Error(401, __('Sorry, you cannot edit pages.'));
        ...
        if ($num_pages >= 1) {
            ...
            foreach ($pages as $page) {
                if (current_user_can('edit_page', $page->ID)) $pages_struct[] = $this->_prepare_page($page);
            }
    ...
    public function wp_deletePage($args) {
        ...
        if (!$actual_page || ($actual_page['post_type'] != 'page')) return new IXR_Error(404, __('Sorry, no such page.'));

        // Make sure the user can delete pages.
        if (!current_user_can('delete_page', $page_id)) return new IXR_Error(401, __('Sorry, you do not have the right to delete this page.'));

        // Attempt to delete the page.
        $result = wp_delete_post($page_id);
        if (!$result) return new IXR_Error(500, __('Failed to delete the page.'));
    ...
    public function wp_editPage($args) {
        ...
        if (!current_user_can('edit_page', $page_id)) return new IXR_Error(401, __('Sorry, you do not have the right to edit this page.'));
    ...
    public function wp_getAuthors($args) {
        ...
        if (!current_user_can('edit_posts')) return new IXR_Error(401, __('Sorry, you cannot edit posts on this site.'));
        ...
        foreach (get_users(array('fields' => array('ID', 'user_login', 'display_name'))) as $user) {
            $authors[] = array('user_id' => $user->ID, 'user_login' => $user->user_login, 'display_name' => $user->display_name);
        }
    ...
    public function wp_newCategory($args) {
        ...
        if (!current_user_can('manage_categories')) return new IXR_Error(401, __('Sorry, you do not have the right to add a category.'));
        ...
        if (is_wp_error($cat_id)) {
            if ('term_exists' == $cat_id->get_error_code()) return (int)$cat_id->get_error_data(); else
    ...
    public function wp_deleteCategory($args) {
        ...
        if (!current_user_can('manage_categories')) return new IXR_Error(401, __('Sorry, you do not have the right to delete a category.'));
    ...
    public function wp_suggestCategories($args) {
        ...
        foreach ((array)get_categories($args) as $cat) {
            $category_suggestions[] = array('category_id' => $cat->term_id, 'category_name' => $cat->name);
        }
    ...
    public function wp_getComment($args) {
        ...
        if (!current_user_can('moderate_comments')) return new IXR_Error(403, __('You are not allowed to moderate comments on this site.'));

        /** This action is documented in wp-includes/class-wp-xmlrpc-server.php */
        do_action('xmlrpc_call', 'wp.getComment');

        if (!$comment = get_comment($comment_id)) return new IXR_Error(404, __('Invalid comment ID.'));
    ...
    public function wp_getComments($args) {
        ...
        if (!current_user_can('moderate_comments')) return new IXR_Error(401, __('Sorry, you cannot edit comments.'));

        /** This action is documented in wp-includes/class-wp-xmlrpc-server.php */
        do_action('xmlrpc_call', 'wp.getComments');

        if (isset($struct['status'])) $status = $struct['status']; else
            $status = '';

        $post_id = '';
        if (isset($struct['post_id'])) $post_id = absint($struct['post_id']);

        $offset = 0;
        if (isset($struct['offset'])) $offset = absint($struct['offset']);

        $number = 10;
        if (isset($struct['number'])) $number = absint($struct['number']);
    ...
    public function wp_deleteComment($args) {
        ...
        if (!current_user_can('edit_comment', $comment_ID)) return new IXR_Error(403, __('You are not allowed to moderate comments on this site.'));
    ...
    public function wp_editComment($args) {
        ...
        if (isset($content_struct['status'])) {
            ...
            if (!in_array($content_struct['status'], $statuses)) return new IXR_Error(401, __('Invalid comment status.'));
        ...
        if (isset($content_struct['content'])) $comment_content = $content_struct['content'];

        if (isset($content_struct['author'])) $comment_author = $content_struct['author'];

        if (isset($content_struct['author_url'])) $comment_author_url = $content_struct['author_url'];

        if (isset($content_struct['author_email'])) $comment_author_email = $content_struct['author_email'];
        ...
        if (!$result) return new IXR_Error(500, __('Sorry, the comment could not be edited. Something wrong happened.'));
    ...
    public function wp_newComment($args) {
        ...
        if (is_numeric($post)) $post_id = absint($post); else
        ...
        if ($logged_in) {
            ...
            if (isset($content_struct['author'])) $comment['comment_author'] = $content_struct['author'];

            $comment['comment_author_email'] = '';
            if (isset($content_struct['author_email'])) $comment['comment_author_email'] = $content_struct['author_email'];

            $comment['comment_author_url'] = '';
            if (isset($content_struct['author_url'])) $comment['comment_author_url'] = $content_struct['author_url'];

            $comment['user_ID'] = 0;

            if (get_option('require_name_email')) {
                if (6 > strlen($comment['comment_author_email']) || '' == $comment['comment_author']) return new IXR_Error(403, __('Comment author name and email are required')); elseif (!is_email($comment['comment_author_email'])) return new IXR_Error(403, __('A valid email address is required'));
            }
        }
    ...
    public function wp_getCommentStatusList($args) {
        ...
        if (!current_user_can('moderate_comments')) return new IXR_Error(403, __('You are not allowed access to details about this site.'));
    ...
    public function wp_getCommentCount($args) {
        ...
        if (!current_user_can('edit_posts')) return new IXR_Error(403, __('You are not allowed access to details about comments.'));
    ...
    public function wp_getPostStatusList($args) {
        ...
        if (!current_user_can('edit_posts')) return new IXR_Error(403, __('You are not allowed access to details about this site.'));
    ...
    public function wp_getPageStatusList($args) {
        ...
        if (!current_user_can('edit_pages')) return new IXR_Error(403, __('You are not allowed access to details about this site.'));
    ...
    public function wp_setOptions($args) {
        ...
        if (!current_user_can('manage_options')) return new IXR_Error(403, __('You are not allowed to update options.'));
        ...
        foreach ($options as $o_name => $o_value) {
            ...
            if ($this->blog_options[$o_name]['readonly'] == true) continue;
    ...
    public function wp_getMediaItem($args) {
        ...
        if (!current_user_can('upload_files')) return new IXR_Error(403, __('You do not have permission to upload files.'));

        /** This action is documented in wp-includes/class-wp-xmlrpc-server.php */
        do_action('xmlrpc_call', 'wp.getMediaItem');

        if (!$attachment = get_post($attachment_id)) return new IXR_Error(404, __('Invalid attachment ID.'));
    ...
    public function wp_getMediaLibrary($args) {
        ...
        if (!current_user_can('upload_files')) return new IXR_Error(401, __('You do not have permission to upload files.'));
        ...
        foreach ($attachments as $attachment) $attachments_struct[] = $this->_prepare_media_item($attachment);
    ...
    public function wp_getPostType($args) {
        ...
        if (!post_type_exists($post_type_name)) return new IXR_Error(403, __('Invalid post type'));

        $post_type = get_post_type_object($post_type_name);

        if (!current_user_can($post_type->cap->edit_posts)) return new IXR_Error(401, __('Sorry, you are not allowed to edit this post type.'));
    ...
    public function wp_getPostTypes($args) {
        ...
        foreach ($post_types as $post_type) {
            if (!current_user_can($post_type->cap->edit_posts)) continue;
    ...
    public function wp_getRevisions($args) {
        ...
        if (!$post = get_post($post_id)) return new IXR_Error(404, __('Invalid post ID'));

        if (!current_user_can('edit_post', $post_id)) return new IXR_Error(401, __('Sorry, you are not allowed to edit posts.'));

        // Check if revisions are enabled.
        if (!wp_revisions_enabled($post)) return new IXR_Error(401, __('Sorry, revisions are disabled.'));

        $revisions = wp_get_post_revisions($post_id);

        if (!$revisions) return array();

        $struct = array();

        foreach ($revisions as $revision) {
            if (!current_user_can('read_post', $revision->ID)) continue;
    ...
    public function wp_restoreRevision($args) {
        ...
        if (!$revision = wp_get_post_revision($revision_id)) return new IXR_Error(404, __('Invalid post ID'));

        if (wp_is_post_autosave($revision)) return new IXR_Error(404, __('Invalid post ID'));

        if (!$post = get_post($revision->post_parent)) return new IXR_Error(404, __('Invalid post ID'));

        if (!current_user_can('edit_post', $revision->post_parent)) return new IXR_Error(401, __('Sorry, you cannot edit this post.'));
    ...
    protected function _multisite_getUsersBlogs($args) {
        ...
        if (isset($blogs['faultCode'])) return new IXR_Error($blogs['faultCode'], $blogs['faultString']);
    ...
    public function blogger_getUserInfo($args) {
        ...
        if (!current_user_can('edit_posts')) return new IXR_Error(401, __('Sorry, you do not have access to user data on this site.'));
    ...
    public function blogger_getPost($args) {
        ...
        if (!$post_data) return new IXR_Error(404, __('Invalid post ID.'));
        ...
        $struct = array('userid' => $post_data['post_author'], 'dateCreated' => $this->_convert_date($post_data['post_date']), 'content' => $content, 'postid' => (string)$post_data['ID']);
    ...
    public function blogger_getRecentPosts($args) {
        ...
        $username = $args[2];
        $password = $args[3];
        if (isset($args[4])) $query = array('numberposts' => absint($args[4])); else
            $query = array();
        ...
        foreach ($posts_list as $entry) {
            ...
            $recent_posts[] = array('userid' => $entry['post_author'], 'dateCreated' => $post_date, 'content' => $content, 'postid' => (string)$entry['ID'],);
        }
    ...
    public function blogger_newPost($args) {
        ...
        if (!current_user_can(get_post_type_object('post')->cap->create_posts) || !current_user_can($cap)) return new IXR_Error(401, __('Sorry, you are not allowed to post on this site.'));
        ...
        if (!$post_ID) return new IXR_Error(500, __('Sorry, your entry could not be posted. Something wrong happened.'));
    ...
    public function blogger_deletePost($args) {
        ...
        if (!$actual_post || $actual_post['post_type'] != 'post') return new IXR_Error(404, __('Sorry, no such post.'));

        if (!current_user_can('delete_post', $post_ID)) return new IXR_Error(401, __('Sorry, you do not have the right to delete this post.'));

        $result = wp_delete_post($post_ID);

        if (!$result) return new IXR_Error(500, __('For some strange yet very annoying reason, this post could not be deleted.'));
    ...
    public function mw_newPost($args) {
        ...
        if (!empty($content_struct['post_type'])) {
            if ($content_struct['post_type'] == 'page') {
                if ($publish) $cap = 'publish_pages'; elseif (isset($content_struct['page_status']) && 'publish' == $content_struct['page_status']) $cap = 'publish_pages';
                else
                    $cap = 'edit_pages';
                $error_message = __('Sorry, you are not allowed to publish pages on this site.');
                $post_type = 'page';
                if (!empty($content_struct['wp_page_template'])) $page_template = $content_struct['wp_page_template'];
            } elseif ($content_struct['post_type'] == 'post') {
                if ($publish) $cap = 'publish_posts'; elseif (isset($content_struct['post_status']) && 'publish' == $content_struct['post_status']) $cap = 'publish_posts';
                else
                    $cap = 'edit_posts';
        ...
        if (!current_user_can(get_post_type_object($post_type)->cap->create_posts)) return new IXR_Error(401, __('Sorry, you are not allowed to publish posts on this site.'));
        if (!current_user_can($cap)) return new IXR_Error(401, $error_message);
        ...
        $post_name = "";
        if (isset($content_struct['wp_slug'])) $post_name = $content_struct['wp_slug'];

        // Only use a password if one was given.
        if (isset($content_struct['wp_password'])) $post_password = $content_struct['wp_password'];

        // Only set a post parent if one was provided.
        if (isset($content_struct['wp_page_parent_id'])) $post_parent = $content_struct['wp_page_parent_id'];

        // Only set the menu_order if it was provided.
        if (isset($content_struct['wp_page_order'])) $menu_order = $content_struct['wp_page_order'];
        ...
        if (isset($content_struct['wp_author_id']) && ($user->ID != $content_struct['wp_author_id'])) {
            switch ($post_type) {
                case "post":
                    if (!current_user_can('edit_others_posts')) return new IXR_Error(401, __('You are not allowed to create posts as this user.'));
                    break;
                case "page":
                    if (!current_user_can('edit_others_pages')) return new IXR_Error(401, __('You are not allowed to create pages as this user.'));
                    break;
                default:
                    return new IXR_Error(401, __('Invalid post type'));
            }
        ...
        if ($post_type == 'post' && isset($content_struct['sticky'])) {
            if ($content_struct['sticky'] == true) stick_post($post_ID); elseif ($content_struct['sticky'] == false) unstick_post($post_ID);
        }

        if (isset($content_struct['custom_fields'])) $this->set_custom_fields($post_ID, $content_struct['custom_fields']);

        if (isset ($content_struct['wp_post_thumbnail'])) {
            if (set_post_thumbnail($post_ID, $content_struct['wp_post_thumbnail']) === false) return new IXR_Error(404, __('Invalid attachment ID.'));
        ...
        // in this function
        if (isset($content_struct['wp_post_format'])) set_post_format($post_ID, $content_struct['wp_post_format']);
    ...
    public function add_enclosure_if_new($post_ID, $enclosure) {
        if (is_array($enclosure) && isset($enclosure['url']) && isset($enclosure['length']) && isset($enclosure['type'])) {
            ...
            if (!$found) add_post_meta($post_ID, 'enclosure', $encstring);
        }
    }
    ...
    public function attach_uploads($post_ID, $post_content) {
        ...
        if (is_array($attachments)) {
            foreach ($attachments as $file) {
                if (!empty($file->guid) && strpos($post_content, $file->guid) !== false) $wpdb->update($wpdb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID));
            }
        }
    }
    ...
    public function mw_editPost($args) {
        ...
        if (!$postdata || empty($postdata['ID'])) return new IXR_Error(404, __('Invalid post ID.'));

        if (!current_user_can('edit_post', $post_ID)) return new IXR_Error(401, __('Sorry, you do not have the right to edit this post.'));

        // Use wp.editPost to edit post types other than post and page.
        if (!in_array($postdata['post_type'], array('post', 'page'))) return new IXR_Error(401, __('Invalid post type'));

        // Thwart attempt to change the post type.
        if (!empty($content_struct['post_type']) && ($content_struct['post_type'] != $postdata['post_type'])) return new IXR_Error(401, __('The post type may not be changed.'));
        ...
        $page_template = null;
        if (!empty($content_struct['wp_page_template']) && 'page' == $post_type) $page_template = $content_struct['wp_page_template'];
        ...
        if (!$result) return new IXR_Error(500, __('Sorry, your entry could not be edited. Something wrong happened.'));
    ...
    public function mw_getPost($args) {
        ...
        $postdata = get_post($post_ID, ARRAY_A);
        if (!$postdata) return new IXR_Error(404, __('Invalid post ID.'));
        ...
        if ($postdata['post_date'] != '') {
            ...
            foreach ($catids as $catid) $categories[] = get_cat_name($catid);
            ...
            // Consider future posts as published
            if ($postdata['post_status'] === 'future') $postdata['post_status'] = 'publish';
            ...
            $sticky = false;
            if (is_sticky($post_ID)) $sticky = true;
    ...
    public function mw_getRecentPosts($args) {
        ...
        $username = $args[1];
        $password = $args[2];
        if (isset($args[3])) $query = array('numberposts' => absint($args[3])); else
            $query = array();
        ...
        foreach ($posts_list as $entry) {
            ...
            // Consider future posts as published
            if ($entry['post_status'] === 'future') $entry['post_status'] = 'publish';

            // Get post format
            $post_format = get_post_format($entry['ID']);
            if (empty($post_format)) $post_format = 'standard';

            $recent_posts[] = array('dateCreated' => $post_date, 'userid' => $entry['post_author'], 'postid' => (string)$entry['ID'], 'description' => $post['main'], 'title' => $entry['post_title'], 'link' => $link, 'permaLink' => $link, // commented out because no other tool seems to use this
                // 'content' => $entry['post_content'],
                'categories' => $categories, 'mt_excerpt' => $entry['post_excerpt'], 'mt_text_more' => $post['extended'], 'wp_more_text' => $post['more_text'], 'mt_allow_comments' => $allow_comments, 'mt_allow_pings' => $allow_pings, 'mt_keywords' => $tagnames, 'wp_slug' => $entry['post_name'], 'wp_password' => $entry['post_password'], 'wp_author_id' => (string)$author->ID, 'wp_author_display_name' => $author->display_name, 'date_created_gmt' => $post_date_gmt, 'post_status' => $entry['post_status'], 'custom_fields' => $this->get_custom_fields($entry['ID']), 'wp_post_format' => $post_format, 'date_modified' => $post_modified, 'date_modified_gmt' => $post_modified_gmt, 'sticky' => ($entry['post_type'] === 'post' && is_sticky($entry['ID'])), 'wp_post_thumbnail' => get_post_thumbnail_id($entry['ID']));
        }
    ...
    public function mt_getRecentPostTitles($args) {
        ...
        foreach ($posts_list as $entry) {
            ...
            $recent_posts[] = array('dateCreated' => $post_date, 'userid' => $entry['post_author'], 'postid' => (string)$entry['ID'], 'title' => $entry['post_title'], 'post_status' => $entry['post_status'], 'date_created_gmt' => $post_date_gmt);
        }
    ...
    public function mt_getPostCategories($args) {
        ...
        if (!current_user_can('edit_post', $post_ID)) return new IXR_Error(401, __('Sorry, you can not edit this post.'));
        ...
        foreach ($catids as $catid) {
            $categories[] = array('categoryName' => get_cat_name($catid), 'categoryId' => (string)$catid, 'isPrimary' => $isPrimary);
            $isPrimary = false;
        }
    ...
    public function mt_getTrackbackPings($args) {
        ...
        foreach ($comments as $comment) {
            if ('trackback' == $comment->comment_type) {
                ...
                $trackback_pings[] = array('pingTitle' => $title, 'pingURL' => $comment->comment_author_url, 'pingIP' => $comment->comment_author_IP);
            }
        }
    ...
    public function mt_publishPost($args) {
        ...
        if (!current_user_can('publish_posts') || !current_user_can('edit_post', $post_ID)) return new IXR_Error(401, __('Sorry, you cannot publish this post.'));
    ...
    public function pingback_ping($args) {
        ...
        if (!$pos1) return $this->pingback_error(0, __('Is there no link to us?'));
        ...
        if ($post_ID == url_to_postid($pagelinkedfrom)) return $this->pingback_error(0, __('The source URL and the target URL cannot both point to the same resource.'));

        // Check if pings are on
        if (!pings_open($post)) return $this->pingback_error(33, __('The specified target URL cannot be used as a target. It either doesn&#8217;t exist, or it is not a pingback-enabled resource.'));

        // Let's check that the remote site didn't already pingback this entry
        if ($wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom))) return $this->pingback_error(48, __('The pingback has already been registered.'));
        ...
        $http_api_args = array('timeout' => 10, 'redirection' => 0, 'limit_response_size' => 153600, // 150 KB
            'user-agent' => "$user_agent; verifying pingback from $remote_ip", 'headers' => array('X-Pingback-Forwarded-For' => $remote_ip,),);
        $request = wp_safe_remote_get($pagelinkedfrom, $http_api_args);
        $linea = wp_remote_retrieve_body($request);

        if (!$linea) return $this->pingback_error(16, __('The source URL does not exist.'));
        ...
        $title = $matchtitle[1];
        if (empty($title)) return $this->pingback_error(32, __('We cannot find a title on that page.'));
        ...
        foreach ($p as $para) {
            if (strpos($para, $pagelinkedto) !== false) { // it exists, but is it a link?
                ...
                if (strlen($context[1]) > 100) $context[1] = substr($context[1], 0, 100) . '&#8230;';
