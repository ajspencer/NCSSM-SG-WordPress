...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
    ...
    if (null !== $check) {
        if ($single && is_array($check)) return $check[0]; else
            return $check;
    }
...
function update_meta_cache($meta_type, $object_ids) {
    ...
    foreach ($object_ids as $id) {
        ...
        if (false === $cached_object) $ids[] = $id; else
...
class WP_Meta_Query {
    ...
    public function get_cast_for_type($type = '') {
        ...
        if (!preg_match('/^(?:BINARY|CHAR|DATE|DATETIME|SIGNED|UNSIGNED|TIME|NUMERIC(?:\(\d+(?:,\s?\d+)?\))?|DECIMAL(?:\(\d+(?:,\s?\d+)?\))?)$/', $meta_type)) return 'CHAR';
...
function register_meta($meta_type, $meta_key, $sanitize_callback, $auth_callback = null) {
    if (is_callable($sanitize_callback)) add_filter("sanitize_{$meta_type}_meta_{$meta_key}", $sanitize_callback, 10, 3);

    if (empty($auth_callback)) {
        if (is_protected_meta($meta_key, $meta_type)) $auth_callback = '__return_false'; else
            $auth_callback = '__return_true';
    }

    if (is_callable($auth_callback)) add_filter("auth_{$meta_type}_meta_{$meta_key}", $auth_callback, 10, 6);
}
