...
function wp_next_scheduled($hook, $args = array()) {
    ...
    foreach ($crons as $timestamp => $cron) {
        if (isset($cron[$hook][$key])) return $timestamp;
    }
    return false;
}
...
function spawn_cron($gmt_time = 0) {
    ...
    if (!$gmt_time) $gmt_time = microtime(true);

    if (defined('DOING_CRON') || isset($_GET['doing_wp_cron'])) return;
    ...
    */
     ...
     * }
     */
    $cron_request = apply_filters('cron_request', array('url' => add_query_arg('doing_wp_cron', $doing_wp_cron, site_url('wp-cron.php')), 'key' => $doing_wp_cron, 'args' => array('timeout' => 0.01, 'blocking' => false, /** This filter is documented in wp-includes/class-http.php */
...
function wp_cron() {
    ...
    if (strpos($_SERVER['REQUEST_URI'], '/wp-cron.php') !== false || (defined('DISABLE_WP_CRON') && DISABLE_WP_CRON)) return;

    if (false === $crons = _get_cron_array()) return;
    ...
    foreach ($crons as $timestamp => $cronhooks) {
        ...
        foreach ((array)$cronhooks as $hook => $args) {
            if (isset($schedules[$hook]['callback']) && !call_user_func($schedules[$hook]['callback'])) continue;
            spawn_cron($gmt_time);
            break 2;
        }
    }
...
function wp_get_schedule($hook, $args = array()) {
    ...
    if (empty($crons)) return false;
    foreach ($crons as $timestamp => $cron) {
        if (isset($cron[$hook][$key])) return $cron[$hook][$key]['schedule'];
    }
    return false;
}
...
function _get_cron_array() {
    ...
    if (!isset($cron['version'])) $cron = _upgrade_cron_array($cron);
