...
class MagpieRSS {
    ...
    function MagpieRSS($source) {
        ...
        #
        if (!function_exists('xml_parser_create')) trigger_error("Failed to load PHP's XML Extension. http://www.php.net/manual/en/ref.xml.php");

        $parser = @xml_parser_create();

        if (!is_resource($parser)) trigger_error("Failed to create an instance of PHP's XML parser. http://www.php.net/manual/en/ref.xml.php");
...
function _fetch_remote_file($url, $headers = "") {
    ...
    foreach (wp_remote_retrieve_headers($resp) as $key => $value) {
        if (!is_array($value)) {
            ...
            foreach ($value as $v) $return_headers[] = "$key: $v";
        }
    }
