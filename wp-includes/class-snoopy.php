...
if (!class_exists('Snoopy')) :
    ...
    class Snoopy {
        ...
        function _striptext($document) {
            ...
            $replace = array("", "", "\\1", "\"", "&", "<", ">", " ", chr(161), chr(162), chr(163), chr(169), chr(174), chr(176), chr(39), chr(128), chr(0xE4), // ANSI &auml;
                chr(0xF6), // ANSI &ouml;
                chr(0xFC), // ANSI &uuml;
                chr(0xC4), // ANSI &Auml;
                chr(0xD6), // ANSI &Ouml;
            ...
            while ($currentHeader = fgets($fp, $this->_maxlinelen)) {
                ...
                if (preg_match("/^(Location:|URI:)/i", $currentHeader)) {
                    ...
                    if (!preg_match("|\:\/\/|", $matches[2])) {
                        ...
                        if (!preg_match("|^/|", $matches[2])) $this->_redirectaddr .= "/" . $matches[2]; else
        ...
        function _httpsrequest($url, $URI, $http_method, $content_type = "", $body = "") {
            ...
            //$headers[] = $http_method." ".$url." ".$this->_httpversion;
            if (!empty($this->agent)) $headers[] = "User-Agent: " . $this->agent;
            if (!empty($this->host)) if (!empty($this->port)) $headers[] = "Host: " . $this->host . ":" . $this->port; else
                $headers[] = "Host: " . $this->host;
            if (!empty($this->accept)) $headers[] = "Accept: " . $this->accept;
            if (!empty($this->referer)) $headers[] = "Referer: " . $this->referer;
            if (!empty($this->cookies)) {
            ...
            if (!empty($content_type)) {
                if ($content_type == "multipart/form-data") $headers[] = "Content-type: $content_type; boundary=" . $this->_mime_boundary; else
                    $headers[] = "Content-type: $content_type";
            }
            if (!empty($body)) $headers[] = "Content-length: " . strlen($body);
            if (!empty($this->user) || !empty($this->pass)) $headers[] = "Authorization: BASIC " . base64_encode($this->user . ":" . $this->pass);
