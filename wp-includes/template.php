...
function get_page_template() {
    ...
    if (!$pagename && $id) {
        ...
        if ($post) $pagename = $post->post_name;
    }
...
function get_single_template() {
    ...
    if (!empty($object->post_type)) $templates[] = "single-{$object->post_type}.php";
...
function get_attachment_template() {
    ...
    if (!empty($posts) && isset($posts[0]->post_mime_type)) {
        ...
        if (!empty($type)) {
            if ($template = get_query_template($type[0])) return $template; elseif (!empty($type[1])) {
                if ($template = get_query_template($type[1])) return $template; elseif ($template = get_query_template("$type[0]_$type[1]")) return $template;
            }
        }
    }
...
function get_comments_popup_template() {
    ...
    if ('' == $template) $template = ABSPATH . WPINC . '/theme-compat/comments-popup.php';
