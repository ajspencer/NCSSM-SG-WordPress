...
class WP_Roles {
    ...
    public function add_role($role, $display_name, $capabilities = array()) {
        ...
        if ($this->use_db) update_option($this->role_key, $this->roles);
    ...
    public function remove_role($role) {
        ...
        if (get_option('default_role') == $role) update_option('default_role', 'subscriber');
    }
    ...
    public function get_role($role) {
        if (isset($this->role_objects[$role])) return $this->role_objects[$role]; else
            return null;
    }
...
class WP_Role {
    ...
    public function has_cap($cap) {
        ...
        if (!empty($capabilities[$cap])) return $capabilities[$cap]; else
            return false;
    }
...
class WP_User {
    ...
    public static function get_data_by($field, $value) {
        ...
        if (false !== $user_id) {
            if ($user = wp_cache_get($user_id, 'users')) return $user;
        }

        if (!$user = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->users WHERE $db_field = %s", $value))) return false;
    ...
    public function __isset($key) {
        ...
        if (isset(self::$back_compat_keys[$key])) $key = self::$back_compat_keys[$key];
    ...
    function _init_caps($cap_key = '') {
        ...
        if (empty($cap_key)) $this->cap_key = $wpdb->get_blog_prefix() . 'capabilities'; else
    ...
    public function get_role_caps() {
        ...
        if (is_array($this->caps)) $this->roles = array_filter(array_keys($this->caps), array($wp_roles, 'is_role'));
    ...
    public function set_role($role) {
        if (1 == count($this->roles) && $role == current($this->roles)) return;

        foreach ((array)$this->roles as $oldrole) unset($this->caps[$oldrole]);
    ...
    public function has_cap($cap) {
        ...
        if (is_multisite() && is_super_admin($this->ID)) {
            if (in_array('do_not_allow', $caps)) return false;
            return true;
        }
    ...
    public function for_blog($blog_id = '') {
        global $wpdb;
        if (!empty($blog_id)) $cap_key = $wpdb->get_blog_prefix($blog_id) . 'capabilities'; else
            $cap_key = '';
...
function map_meta_cap($cap, $user_id) {
    ...
    switch ($cap) {
        ...
        case 'edit_users':
            ...
            if (is_multisite() && !is_super_admin($user_id)) $caps[] = 'do_not_allow'; else
        ...
        case 'delete_page':
            ...
            if (!$post_type->map_meta_cap) {
                ...
                if ('delete_post' == $cap) $cap = $post_type->cap->$cap;
                break;
            }
        ...
        case 'read_page':
            ...
            if (!$post_type->map_meta_cap) {
                $caps[] = $post_type->cap->$cap;
                // Prior to 3.1 we would re-call map_meta_cap here.
                if ('read_post' == $cap) $cap = $post_type->cap->$cap;
                break;
            }
        ...
        case 'unfiltered_upload':
            if (defined('ALLOW_UNFILTERED_UPLOADS') && ALLOW_UNFILTERED_UPLOADS && (!is_multisite() || is_super_admin($user_id))) $caps[] = $cap; else
                $caps[] = 'do_not_allow';
            break;
        case 'unfiltered_html' :
            // Disallow unfiltered_html for all users, even admins and super admins.
            if (defined('DISALLOW_UNFILTERED_HTML') && DISALLOW_UNFILTERED_HTML) $caps[] = 'do_not_allow'; elseif (is_multisite() && !is_super_admin($user_id)) $caps[] = 'do_not_allow';
            else
                $caps[] = $cap;
            break;
        case 'edit_files':
        ...
        case 'edit_themes':
            ...
            if (defined('DISALLOW_FILE_EDIT') && DISALLOW_FILE_EDIT) $caps[] = 'do_not_allow'; elseif (defined('DISALLOW_FILE_MODS') && DISALLOW_FILE_MODS) $caps[] = 'do_not_allow';
        ...
        case 'activate_plugins':
            ...
            if (is_multisite()) {
                ...
                if (empty($menu_perms['plugins'])) $caps[] = 'manage_network_plugins';
            }
            break;
        case 'delete_user':
        case 'delete_users':
            ...
            break;
        case 'create_users':
            if (!is_multisite()) $caps[] = $cap; elseif (is_super_admin() || get_site_option('add_new_users')) $caps[] = $cap;
            else
                $caps[] = 'do_not_allow';
            break;
        case 'manage_links' :
            if (get_option('link_manager_enabled')) $caps[] = $cap; else
                $caps[] = 'do_not_allow';
            break;
        case 'customize' :
...
function get_super_admins() {
    ...
    if (isset($super_admins)) return $super_admins; else
...
function is_super_admin($user_id = false) {
    if (!$user_id || $user_id == get_current_user_id()) $user = wp_get_current_user(); else
    ...
    if (is_multisite()) {
        ...
        if (is_array($super_admins) && in_array($user->user_login, $super_admins)) return true;
    } else {
