 ...
 * @since 1.2.0
 */
if (!defined('CUSTOM_TAGS')) define('CUSTOM_TAGS', false);
...
function wp_kses_attr($element, $attr, $allowed_html, $allowed_protocols) {
    ...
    if (!isset($allowed_html[strtolower($element)]) || count($allowed_html[strtolower($element)]) == 0) return "<$element$xhtml_slash>";
    ...
    while (strlen($attr) != 0) {
        ...
        switch ($mode) {
            ...
            case 2 : // attribute value, a URL after href= for instance
                ...
                {
                    $thisval = $match[1];
                    if (in_array(strtolower($attrname), $uris)) $thisval = wp_kses_bad_protocol($thisval, $allowed_protocols);
...
function wp_kses_bad_protocol_once2($string, $allowed_protocols) {
    ...
    $allowed = false;
    foreach ((array)$allowed_protocols as $one_protocol) if (strtolower($one_protocol) == $string2) {
        $allowed = true;
        break;
    }

    if ($allowed) return "$string2:"; else
        return '';
}
...
function kses_init_filters() {
    ...
    // Comment filtering
    if (current_user_can('unfiltered_html')) add_filter('pre_comment_content', 'wp_filter_post_kses'); else
...
function kses_init() {
    ...
    if (current_user_can('unfiltered_html') == false) kses_init_filters();
}
...
function safecss_filter_attr($css, $deprecated = '') {
    if (!empty($deprecated)) _deprecated_argument(__FUNCTION__, '2.8.1'); // Never implemented
    ...
    foreach ($css_array as $css_item) {
        ...
        if (strpos($css_item, ':') === false) {
            ...
            if (in_array(trim($parts[0]), $allowed_attr)) $found = true;
        }
        if ($found) {
            if ($css != '') $css .= ';';
            $css .= $css_item;
