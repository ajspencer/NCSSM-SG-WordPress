...
function _wp_admin_bar_init() {
    ...
    if (class_exists($admin_bar_class)) $wp_admin_bar = new $admin_bar_class; else
        return false;
...
function wp_admin_bar_sidebar_toggle($wp_admin_bar) {
    if (is_admin()) {
        $wp_admin_bar->add_menu(array('id' => 'menu-toggle', 'title' => '<span class="ab-icon"></span><span class="screen-reader-text">' . __('Menu') . '</span>', 'href' => '#',));
    }
}
...
function wp_admin_bar_my_account_menu($wp_admin_bar) {
    ...
    if ($current_user->display_name !== $current_user->user_login) $user_info .= "<span class='username'>{$current_user->user_login}</span>";
...
function wp_admin_bar_site_menu($wp_admin_bar) {
    ...
    if (!is_user_member_of_blog() && !is_super_admin()) return;
...
function wp_admin_bar_my_sites_menu($wp_admin_bar) {
    ...
    if (count($wp_admin_bar->user->blogs) < 1 && !is_super_admin()) return;
...
function wp_admin_bar_new_content_menu($wp_admin_bar) {
    ...
    if (isset($cpts['post']) && current_user_can($cpts['post']->cap->create_posts)) $actions['post-new.php'] = array($cpts['post']->labels->name_admin_bar, 'new-post');

    if (isset($cpts['attachment']) && current_user_can('upload_files')) $actions['media-new.php'] = array($cpts['attachment']->labels->name_admin_bar, 'new-media');

    if (current_user_can('manage_links')) $actions['link-add.php'] = array(_x('Link', 'add new from admin bar'), 'new-link');

    if (isset($cpts['page']) && current_user_can($cpts['page']->cap->create_posts)) $actions['post-new.php?post_type=page'] = array($cpts['page']->labels->name_admin_bar, 'new-page');

    unset($cpts['post'], $cpts['page'], $cpts['attachment']);

    // Add any additional custom post types.
    foreach ($cpts as $cpt) {
        if (!current_user_can($cpt->cap->create_posts)) continue;

        $key = 'post-new.php?post_type=' . $cpt->name;
        $actions[$key] = array($cpt->labels->name_admin_bar, 'new-' . $cpt->name);
    }
    // Avoid clash with parent node and a 'content' post type.
    if (isset($actions['post-new.php?post_type=content'])) $actions['post-new.php?post_type=content'][1] = 'add-new-content';

    if (current_user_can('create_users') || current_user_can('promote_users')) $actions['user-new.php'] = array(_x('User', 'add new from admin bar'), 'new-user');
...
function wp_admin_bar_appearance_menu($wp_admin_bar) {
    ...
    if (current_user_can('switch_themes')) {
        ...
        if (current_user_can('customize')) {
            $wp_admin_bar->add_menu(array('parent' => 'appearance', 'id' => 'customize-themes', 'title' => __('Themes'), 'href' => add_query_arg(urlencode('autofocus[section]'), 'themes', $customize_url), // urlencode() needed due to #16859
    ...
    if (current_theme_supports('widgets')) {
        ...
        if (current_user_can('customize')) {
            $wp_admin_bar->add_menu(array('parent' => 'appearance', 'id' => 'customize-widgets', 'title' => __('Widgets'), 'href' => add_query_arg(urlencode('autofocus[panel]'), 'widgets', $customize_url), // urlencode() needed due to #16859
                'meta' => array('class' => 'hide-if-no-customize',),));
        }
    }

    if (current_theme_supports('menus') || current_theme_supports('widgets')) $wp_admin_bar->add_menu(array('parent' => 'appearance', 'id' => 'menus', 'title' => __('Menus'), 'href' => admin_url('nav-menus.php')));
    ...
    if (current_theme_supports('custom-background')) {
        ...
        if (current_user_can('customize')) {
            $wp_admin_bar->add_menu(array('parent' => 'appearance', 'id' => 'customize-background', 'title' => __('Background'), 'href' => add_query_arg(urlencode('autofocus[control]'), 'background_image', $customize_url), // urlencode() needed due to #16859
                'meta' => array('class' => 'hide-if-no-customize',),));
        }
    }
    ...
    if (current_theme_supports('custom-header')) {
        ...
        if (current_user_can('customize')) {
            $wp_admin_bar->add_menu(array('parent' => 'appearance', 'id' => 'customize-header', 'title' => __('Header'), 'href' => add_query_arg(urlencode('autofocus[control]'), 'header_image', $customize_url), // urlencode() needed due to #16859
                'meta' => array('class' => 'hide-if-no-customize',),));
        }
    }
...
function is_admin_bar_showing() {
    ...
    if (defined('XMLRPC_REQUEST') || defined('DOING_AJAX') || defined('IFRAME_REQUEST')) return false;
