...
class WP_Scripts extends WP_Dependencies {
    ...
    public function print_extra_script($handle, $echo = true) {
        if (!$output = $this->get_data($handle, 'data')) return;
    ...
    public function do_item($handle, $group = false) {
        ...
        if (false === $group && in_array($handle, $this->in_footer, true)) $this->in_footer = array_diff($this->in_footer, (array)$handle);
        ...
        if (isset($this->args[$handle])) $ver = $ver ? $ver . '&amp;' . $this->args[$handle] : $this->args[$handle];
    ...
    public function set_group($handle, $recursion, $group = false) {
        ...
        if ($this->registered[$handle]->args === 1) $grp = 1; else
