...
function wp_signon($credentials = array(), $secure_cookie = '') {
    ...
    if ('' === $secure_cookie) $secure_cookie = is_ssl();
...
function wp_authenticate_username_password($user, $username, $password) {
    ...
    if (!$user) return new WP_Error('invalid_username', sprintf(__('<strong>ERROR</strong>: Invalid username. <a href="%s">Lost your password?</a>'), wp_lostpassword_url()));
    ...
    if (!wp_check_password($password, $user->user_pass, $user->ID)) return new WP_Error('incorrect_password', sprintf(__('<strong>ERROR</strong>: The password you entered for the username <strong>%1$s</strong> is incorrect. <a href="%2$s">Lost your password?</a>'), $username, wp_lostpassword_url()));
...
function wp_authenticate_cookie($user, $username, $password) {
    ...
    if (empty($username) && empty($password)) {
        ...
        if ($auth_secure_cookie) $auth_cookie = SECURE_AUTH_COOKIE; else
            $auth_cookie = AUTH_COOKIE;

        if (!empty($_COOKIE[$auth_cookie])) return new WP_Error('expired_session', __('Please log in again.'));
...
function wp_authenticate_spam_check($user) {
    if ($user instanceof WP_User && is_multisite()) {
        ...
        if ($spammed) return new WP_Error('spammer_account', __('<strong>ERROR</strong>: Your account has been marked as a spammer.'));
    }
    return $user;
}
...
function get_user_option($option, $user = 0, $deprecated = '') {
    ...
    if (!$user = get_userdata($user)) return false;

    $prefix = $wpdb->get_blog_prefix();
    if ($user->has_prop($prefix . $option)) // Blog specific
        $result = $user->get($prefix . $option); elseif ($user->has_prop($option)) // User specific and cross-blog
...
function update_user_option($user_id, $option_name, $newvalue, $global = false) {
    ...
    if (!$global) $option_name = $wpdb->get_blog_prefix() . $option_name;
...
class WP_User_Query {
    ...
    public function prepare_query($query = array()) {
        ...
        if (isset($qv['count_total']) && $qv['count_total']) $this->query_fields = 'SQL_CALC_FOUND_ROWS ' . $this->query_fields;
        ...
        if (isset($qv['number']) && $qv['number']) {
            if ($qv['offset']) $this->query_limit = $wpdb->prepare("LIMIT %d, %d", $qv['offset'], $qv['number']); else
        ...
        if ($search) {
            ...
            if ($leading_wild && $trailing_wild) $wild = 'both'; elseif ($leading_wild) $wild = 'leading';
            ...
            if ($qv['search_columns']) $search_columns = array_intersect($qv['search_columns'], array('ID', 'user_login', 'user_email', 'user_url', 'user_nicename'));
            if (!$search_columns) {
                if (false !== strpos($search, '@')) $search_columns = array('user_email'); elseif (is_numeric($search)) $search_columns = array('user_login', 'ID');
                elseif (preg_match('|^https?://|', $search) && !(is_multisite() && wp_is_large_network('users'))) $search_columns = array('user_url');
                else
    ...
    public function query() {
         ...
         */
        if (isset($qv['count_total']) && $qv['count_total']) $this->total_users = $wpdb->get_var(apply_filters('found_users_query', 'SELECT FOUND_ROWS()'));
        ...
        if ('all_with_meta' == $qv['fields']) {
            ...
            $r = array();
            foreach ($this->results as $userid) $r[$userid] = new WP_User($userid, '', $qv['blog_id']);
...
function get_blogs_of_user($user_id, $all = false) {
    ...
    foreach ($keys as $key) {
        ...
        if ($wpdb->base_prefix && 0 !== strpos($key, $wpdb->base_prefix)) continue;
        $blog_id = str_replace(array($wpdb->base_prefix, '_capabilities'), '', $key);
        if (!is_numeric($blog_id)) continue;
...
function sanitize_user_field($field, $value, $user_id, $context) {
    ...
    if ('user_url' == $field) $value = esc_url($value);
...
function clean_user_cache($user) {
    if (is_numeric($user)) $user = new WP_User($user);
...
function check_password_reset_key($key, $login) {
    ...
    if (!$row) return new WP_Error('invalid_key', __('Invalid key'));
    ...
    if ($wp_hasher->CheckPassword($key, $row->user_activation_key)) return get_userdata($row->ID);
