...
function wp_get_nav_menu_object($menu) {
    ...
    if (!$menu_obj) $menu_obj = get_term_by('slug', $menu, 'nav_menu');

    if (!$menu_obj) $menu_obj = get_term_by('name', $menu, 'nav_menu');
...
function is_nav_menu($menu) {
    ...
    if ($menu_obj && !is_wp_error($menu_obj) && !empty($menu_obj->taxonomy) && 'nav_menu' == $menu_obj->taxonomy) return true;
...
function get_registered_nav_menus() {
    global $_wp_registered_nav_menus;
    if (isset($_wp_registered_nav_menus)) return $_wp_registered_nav_menus;
    return array();
}
...
function wp_update_nav_menu_object($menu_id = 0, $menu_data = array()) {
    ...
    if ($_possible_existing && !is_wp_error($_possible_existing) && isset($_possible_existing->term_id) && $_possible_existing->term_id != $menu_id) return new WP_Error('menu_exists', sprintf(__('The menu name <strong>%s</strong> conflicts with another menu name. Please try another.'), esc_html($menu_data['menu-name'])));

    // menu doesn't already exist, so create a new menu
    if (!$_menu || is_wp_error($_menu)) {
        $menu_exists = get_term_by('name', $menu_data['menu-name'], 'nav_menu');

        if ($menu_exists) return new WP_Error('menu_exists', sprintf(__('The menu name <strong>%s</strong> conflicts with another menu name. Please try another.'), esc_html($menu_data['menu-name'])));
    ...
    if (is_wp_error($update_response)) return $update_response;
...
function wp_update_nav_menu_item($menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array()) {
    ...
    if (!empty($menu_item_db_id) && !is_nav_menu_item($menu_item_db_id)) return new WP_Error('update_nav_menu_item_failed', __('The given object ID is not that of a menu item.'));
    ...
    if ('custom' != $args['menu-item-type']) {
        ...
        */
        ...
        if ($args['menu-item-title'] == $original_title) $args['menu-item-title'] = '';
    ...
    if (!$update) {
        ...
        if (!$menu_item_db_id || is_wp_error($menu_item_db_id)) return $menu_item_db_id;
    }
    ...
    if (0 == $menu_id) update_post_meta($menu_item_db_id, '_menu_item_orphaned', (string)time()); elseif (get_post_meta($menu_item_db_id, '_menu_item_orphaned')) delete_post_meta($menu_item_db_id, '_menu_item_orphaned');
...
function _sort_nav_menu_items($a, $b) {
    ...
    if ($a->$_menu_item_sort_prop == $b->$_menu_item_sort_prop) return 0; elseif ($_a == $a->$_menu_item_sort_prop && $_b == $b->$_menu_item_sort_prop) return $_a < $_b ? -1 : 1;
    else
...
function _is_valid_nav_menu_item($item) {
    if (!empty($item->_invalid)) return false;
...
function wp_get_nav_menu_items($menu, $args = array()) {
    ...
    if (is_wp_error($items) || !is_array($items)) return false;
    ...
    if (empty($fetched[$menu->term_id]) || wp_using_ext_object_cache()) {
        ...
        foreach ($items as $item) {
            ...
            if ('post_type' == $type) $posts[$object][] = $object_id; elseif ('taxonomy' == $type) $terms[$object][] = $object_id;
        }
...
function wp_setup_nav_menu_item($menu_item) {
    if (isset($menu_item->post_type)) {
        if ('nav_menu_item' == $menu_item->post_type) {
            ...
            if ('' === $menu_item->post_title) $menu_item->post_title = sprintf(__('#%d (no title)'), $menu_item->ID);
...
function _wp_auto_add_pages_to_menu($new_status, $old_status, $post) {
    if ('publish' != $new_status || 'publish' == $old_status || 'page' != $post->post_type) return;
