...
function wp_fix_server_vars() {
    ...
    if (empty($_SERVER['REQUEST_URI']) || (PHP_SAPI != 'cgi-fcgi' && preg_match('/^Microsoft-IIS\//', $_SERVER['SERVER_SOFTWARE']))) {
        ...
        elseif (isset($_SERVER['HTTP_X_REWRITE_URL'])) {
            ...
            if (isset($_SERVER['PATH_INFO'])) {
                if ($_SERVER['PATH_INFO'] == $_SERVER['SCRIPT_NAME']) $_SERVER['REQUEST_URI'] = $_SERVER['PATH_INFO']; else
    ...
    if (isset($_SERVER['SCRIPT_FILENAME']) && (strpos($_SERVER['SCRIPT_FILENAME'], 'php.cgi') == strlen($_SERVER['SCRIPT_FILENAME']) - 7)) $_SERVER['SCRIPT_FILENAME'] = $_SERVER['PATH_TRANSLATED'];

    // Fix for Dreamhost and other PHP as CGI hosts
    if (strpos($_SERVER['SCRIPT_NAME'], 'php.cgi') !== false) unset($_SERVER['PATH_INFO']);
...
function wp_maintenance() {
    if (!file_exists(ABSPATH . '.maintenance') || defined('WP_INSTALLING')) return;
...
function wp_debug_mode() {
    if (WP_DEBUG) {
        ...
        if (WP_DEBUG_DISPLAY) ini_set('display_errors', 1); elseif (null !== WP_DEBUG_DISPLAY) ini_set('display_errors', 0);
...
function require_wp_db() {
    ...
    if (file_exists(WP_CONTENT_DIR . '/db.php')) require_once(WP_CONTENT_DIR . '/db.php');
...
function wp_using_ext_object_cache($using = null) {
    ...
    if (null !== $using) $_wp_using_ext_object_cache = $using;
...
function wp_start_object_cache() {
    ...
    if (!function_exists('wp_cache_init')) {
        if (file_exists(WP_CONTENT_DIR . '/object-cache.php')) {
            ...
            if (function_exists('wp_cache_init')) wp_using_ext_object_cache(true);
        }
     ...
     */
    if (!$first_init && function_exists('wp_cache_switch_to_blog')) wp_cache_switch_to_blog($blog_id); elseif (function_exists('wp_cache_init')) wp_cache_init();
...
function wp_get_mu_plugins() {
    ...
    while (($plugin = readdir($dh)) !== false) {
        if (substr($plugin, -4) == '.php') $mu_plugins[] = WPMU_PLUGIN_DIR . '/' . $plugin;
    }
    closedir($dh);
    sort($mu_plugins);
...
function wp_set_internal_encoding() {
    if (function_exists('mb_internal_encoding')) {
        ...
        if (!$charset || !@mb_internal_encoding($charset)) mb_internal_encoding('UTF-8');
    }
}
 ...
 */
function is_admin() {
    if (isset($GLOBALS['current_screen'])) return $GLOBALS['current_screen']->in_admin(); elseif (defined('WP_ADMIN')) return WP_ADMIN;
...
function is_blog_admin() {
    if (isset($GLOBALS['current_screen'])) return $GLOBALS['current_screen']->in_admin('site'); elseif (defined('WP_BLOG_ADMIN')) return WP_BLOG_ADMIN;
...
function is_network_admin() {
    if (isset($GLOBALS['current_screen'])) return $GLOBALS['current_screen']->in_admin('network'); elseif (defined('WP_NETWORK_ADMIN')) return WP_NETWORK_ADMIN;
...
function is_user_admin() {
    if (isset($GLOBALS['current_screen'])) return $GLOBALS['current_screen']->in_admin('user'); elseif (defined('WP_USER_ADMIN')) return WP_USER_ADMIN;
...
function wp_load_translations_early() {
    ...
    while (true) {
        ...
        if (defined('WP_CONTENT_DIR') && @is_dir(WP_CONTENT_DIR . '/languages')) $locations[] = WP_CONTENT_DIR . '/languages';

        if (@is_dir(ABSPATH . 'wp-content/languages')) $locations[] = ABSPATH . 'wp-content/languages';

        if (@is_dir(ABSPATH . WPINC . '/languages')) $locations[] = ABSPATH . WPINC . '/languages';
        ...
        foreach ($locales as $locale) {
            foreach ($locations as $location) {
                if (file_exists($location . '/' . $locale . '.mo')) {
                    ...
                    if (defined('WP_SETUP_CONFIG') && file_exists($location . '/admin-' . $locale . '.mo')) load_textdomain('default', $location . '/admin-' . $locale . '.mo');
                    break 2;
                }
            }
        }
