...
function _wp_post_revision_fields($post = null, $autosave = false) {
    ...
    if (!$fields) {
        ...
        foreach (array('ID', 'post_name', 'post_parent', 'post_date', 'post_date_gmt', 'post_status', 'post_type', 'comment_count', 'post_author') as $protect) unset($fields[$protect]);
    }

    if (!is_array($post)) return $fields;

    $return = array();
    foreach (array_intersect(array_keys($post), array_keys($fields)) as $field) $return[$field] = $post[$field];
...
function wp_save_post_revision($post_id) {
    ...
    if (!wp_revisions_enabled($post)) return;
...
function wp_get_post_autosave($post_id, $user_id = 0) {
    ...
    foreach ($revisions as $revision) {
        if (false !== strpos($revision->post_name, "{$post_id}-autosave")) {
            if ($user_id && $user_id != $revision->post_author) continue;
...
function wp_is_post_revision($post) {
    if (!$post = wp_get_post_revision($post)) return false;
...
function wp_is_post_autosave($post) {
    ...
    if (false !== strpos($post->post_name, "{$post->post_parent}-autosave")) return (int)$post->post_parent;
...
function _wp_put_post_revision($post = null, $autosave = false) {
    ...
    if (!$post || empty($post['ID'])) return new WP_Error('invalid_post', __('Invalid post ID'));

    if (isset($post['post_type']) && 'revision' == $post['post_type']) return new WP_Error('post_type', __('Cannot create a revision of a revision'));
...
function wp_get_post_revision(&$post, $output = OBJECT, $filter = 'raw') {
    if (!$revision = get_post($post, OBJECT, $filter)) return $revision;
...
function wp_restore_post_revision($revision_id, $fields = null) {
    if (!$revision = wp_get_post_revision($revision_id, ARRAY_A)) return $revision;

    if (!is_array($fields)) $fields = array_keys(_wp_post_revision_fields());
    ...
    if (!$post_id || is_wp_error($post_id)) return $post_id;
...
function wp_delete_post_revision($revision_id) {
    if (!$revision = wp_get_post_revision($revision_id)) return $revision;
...
function wp_get_post_revisions($post_id = 0, $args = null) {
    ...
    if ($args['check_enabled'] && !wp_revisions_enabled($post)) return array();

    $args = array_merge($args, array('post_parent' => $post->ID, 'post_type' => 'revision', 'post_status' => 'inherit'));

    if (!$revisions = get_children($args)) return array();
...
function wp_revisions_to_keep($post) {
    ...
    if (true === $num) $num = -1; else
        $num = intval($num);

    if (!post_type_supports($post->post_type, 'revisions')) $num = 0;
...
function _show_post_preview() {
    ...
    if (isset($_GET['preview_id']) && isset($_GET['preview_nonce'])) {
        ...
        if (false == wp_verify_nonce($_GET['preview_nonce'], 'post_preview_' . $id)) wp_die(__('You do not have permission to preview drafts.'));
...
function _wp_preview_terms_filter($terms, $post_id, $taxonomy) {
    if (!$post = get_post()) return $terms;

    if (empty($_REQUEST['post_format']) || $post->ID != $post_id || 'post_format' != $taxonomy || 'revision' == $post->post_type) return $terms;

    if ('standard' == $_REQUEST['post_format']) $terms = array(); elseif ($term = get_term_by('slug', 'post-format-' . sanitize_key($_REQUEST['post_format']), 'post_format')) $terms = array($term); // Can only have one post format
...
function _wp_get_post_revision_version($revision) {
    if (is_object($revision)) $revision = get_object_vars($revision); elseif (!is_array($revision)) return false;

    if (preg_match('/^\d+-(?:autosave|revision)-v(\d+)$/', $revision['post_name'], $matches)) return (int)$matches[1];
...
function _wp_upgrade_revisions_of_post($post, $revisions) {
    ...
    do {
        ...
        if ($result) wp_cache_delete($this_revision->ID, 'posts');
    ...
    if ($add_last) wp_save_post_revision($post->ID);
