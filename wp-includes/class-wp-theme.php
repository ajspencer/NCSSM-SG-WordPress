...
final class WP_Theme implements ArrayAccess {
     ...
     * @var array
     */
    private static $default_themes = array('classic' => 'WordPress Classic', 'default' => 'WordPress Default', 'twentyten' => 'Twenty Ten', 'twentyeleven' => 'Twenty Eleven', 'twentytwelve' => 'Twenty Twelve', 'twentythirteen' => 'Twenty Thirteen', 'twentyfourteen' => 'Twenty Fourteen', 'twentyfifteen' => 'Twenty Fifteen',);
    ...
    public function __construct($theme_dir, $theme_root, $_child = null) {
        ...
        if (!isset(self::$persistently_cache)) {
            ...
            if (self::$persistently_cache) {
                ...
                if (is_int(self::$persistently_cache)) self::$cache_expiration = self::$persistently_cache;
            } else {
        ...
        if (is_array($cache)) {
            ...
            if (isset($cache['theme_root_template'])) $theme_root_template = $cache['theme_root_template'];
        } elseif (!file_exists($this->theme_root . '/' . $theme_file)) {
            $this->headers['Name'] = $this->stylesheet;
            if (!file_exists($this->theme_root . '/' . $this->stylesheet)) $this->errors = new WP_Error('theme_not_found', sprintf(__('The theme directory "%s" does not exist.'), $this->stylesheet)); else
            ...
            if ($default_theme_slug = array_search($this->headers['Name'], self::$default_themes)) {
                if (basename($this->stylesheet) != $default_theme_slug) $this->headers['Name'] .= '/' . $this->stylesheet;
            }
        }
    ...
    public function __isset($offset) {
        static $properties = array('name', 'title', 'version', 'parent_theme', 'template_dir', 'stylesheet_dir', 'template', 'stylesheet', 'screenshot', 'description', 'author', 'tags', 'theme_root', 'theme_root_uri',);
    ...
    public function offsetExists($offset) {
        static $keys = array('Name', 'Version', 'Status', 'Title', 'Author', 'Author Name', 'Author URI', 'Description', 'Template', 'Stylesheet', 'Template Files', 'Stylesheet Files', 'Template Dir', 'Stylesheet Dir', 'Screenshot', 'Tags', 'Theme Root', 'Theme Root URI', 'Parent Theme',);
    ...
    public function cache_delete() {
        foreach (array('theme', 'screenshot', 'headers', 'page_templates') as $key) wp_cache_delete($key . '-' . $this->cache_hash, 'themes');
    ...
    public function get($header) {
        ...
        if (!isset($this->headers_sanitized)) {
            ...
            if (!is_array($this->headers_sanitized)) $this->headers_sanitized = array();
        }

        if (isset($this->headers_sanitized[$header])) return $this->headers_sanitized[$header];

        // If themes are a persistent group, sanitize everything and cache it. One cache add is better than many cache sets.
        if (self::$persistently_cache) {
            foreach (array_keys($this->headers) as $_header) $this->headers_sanitized[$_header] = $this->sanitize_header($_header, $this->headers[$_header]);
    ...
    private function markup_header($header, $value, $translate) {
        switch ($header) {
            case 'Name' :
                if (empty($value)) $value = $this->get_stylesheet();
                break;
            case 'Description' :
    ...
    private function translate_header($header, $value) {
        switch ($header) {
            case 'Name' :
                ...
                if (isset($this->name_translated)) return $this->name_translated;
                $this->name_translated = translate($value, $this->get('TextDomain'));
                return $this->name_translated;
            case 'Tags' :
                if (empty($value) || !function_exists('get_theme_feature_list')) return $value;

                static $tags_list;
                if (!isset($tags_list)) {
                    $tags_list = array();
                    $feature_list = get_theme_feature_list(false); // No API
                    foreach ($feature_list as $tags) $tags_list += $tags;
                }
    ...
    public function get_stylesheet_directory() {
        if ($this->errors() && in_array('theme_root_missing', $this->errors()->get_error_codes())) return '';
    ...
    public function get_template_directory() {
        if ($this->parent()) $theme_root = $this->parent()->theme_root; else
    ...
    public function get_template_directory_uri() {
        if ($this->parent()) $theme_root_uri = $this->parent()->get_theme_root_uri(); else
    ...
    public function get_theme_root_uri() {
        if (!isset($this->theme_root_uri)) $this->theme_root_uri = get_theme_root_uri($this->stylesheet, $this->theme_root);
    ...
    public function get_files($type = null, $depth = 0, $search_parent = false) {
        ...
        if ($search_parent && $this->parent()) $files += (array)self::scandir($this->get_template_directory(), $type, $depth);
    ...
    public function get_page_templates($post = null) {
        ...
        if ($this->errors() && $this->errors()->get_error_codes() !== array('theme_parent_invalid')) return array();
        ...
        if ($this->parent()) $page_templates += $this->parent()->get_page_templates($post);
    ...
    public function load_textdomain() {
        if (isset($this->textdomain_loaded)) return $this->textdomain_loaded;
        ...
        if ($domainpath = $this->get('DomainPath')) $path .= $domainpath; else
            $path .= '/languages';
    ...
    public static function get_allowed_on_network() {
        ...
        if (!isset($allowed_themes)) $allowed_themes = (array)get_site_option('allowedthemes');
    ...
    public static function get_allowed_on_site($blog_id = null) {
        ...
        if (!$blog_id || !is_multisite()) $blog_id = get_current_blog_id();

        if (isset($allowed_themes[$blog_id])) return $allowed_themes[$blog_id];
        ...
        if (false === $allowed_themes[$blog_id]) {
            ...
            if (!is_array($allowed_themes[$blog_id]) || empty($allowed_themes[$blog_id])) {
                ...
                foreach ($themes as $stylesheet => $theme_data) {
                    if (isset($allowed_themes[$blog_id][$theme_data->get('Name')])) $converted[$stylesheet] = true;
                }
