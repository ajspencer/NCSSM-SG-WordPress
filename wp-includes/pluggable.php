...
if (!function_exists('wp_set_current_user')) :
    ...
    function wp_set_current_user($id, $name = '') {
        ...
        if (isset($current_user) && ($current_user instanceof WP_User) && ($id == $current_user->ID)) return $current_user;
...
if (!function_exists('wp_mail')) :
    ...
    function wp_mail($to, $subject, $message, $headers = '', $attachments = array()) {
        ...
        if (!isset($from_name)) $from_name = 'WordPress';
	 ...
	 */
        ...
        if (!empty($headers)) {
            ...
            if (false !== stripos($content_type, 'multipart') && !empty($boundary)) $phpmailer->AddCustomHeader(sprintf("Content-Type: %s;\n\t boundary=\"%s\"", $content_type, $boundary));
        }
...
if (!function_exists('wp_set_auth_cookie')) :
    ...
    function wp_set_auth_cookie($user_id, $remember = false, $secure = '') {
        ...
        if (COOKIEPATH != SITECOOKIEPATH) setcookie(LOGGED_IN_COOKIE, $logged_in_cookie, $expire, SITECOOKIEPATH, COOKIE_DOMAIN, $secure_logged_in_cookie, true);
    }
endif;
...
if (!function_exists('check_admin_referer')) :
    ...
    function check_admin_referer($action = -1, $query_arg = '_wpnonce') {
        if (-1 == $action) _doing_it_wrong(__FUNCTION__, __('You should specify a nonce action to be verified by using the first parameter.'), '3.2');
...
if (!function_exists('check_ajax_referer')) :
    ...
    function check_ajax_referer($action = -1, $query_arg = false, $die = true) {
        ...
        if ($query_arg && isset($_REQUEST[$query_arg])) $nonce = $_REQUEST[$query_arg]; elseif (isset($_REQUEST['_ajax_nonce'])) $nonce = $_REQUEST['_ajax_nonce'];
        elseif (isset($_REQUEST['_wpnonce'])) $nonce = $_REQUEST['_wpnonce'];

        $result = wp_verify_nonce($nonce, $action);

        if ($die && false == $result) {
            if (defined('DOING_AJAX') && DOING_AJAX) wp_die(-1); else
                die('-1');
        }
...
if (!function_exists('wp_redirect')) :
    ...
    function wp_redirect($location, $status = 302) {
        ...
        if (!$is_IIS && PHP_SAPI != 'cgi-fcgi') status_header($status); // This causes problems on IIS and some FastCGI setups
...
if (!function_exists('wp_validate_redirect')) :
    ...
    function wp_validate_redirect($location, $default = '') {
        ...
        if (isset($lp['scheme']) && !('http' == $lp['scheme'] || 'https' == $lp['scheme'])) return $default;

        // Reject if scheme is set but host is not. This catches urls like https:host.com for which parse_url does not set the host field.
        if (isset($lp['scheme']) && !isset($lp['host'])) return $default;
        ...
        if (isset($lp['host']) && (!in_array($lp['host'], $allowed_hosts) && $lp['host'] != strtolower($wpp['host']))) $location = $default;
...
if (!function_exists('wp_notify_moderator')) :
    ...
    function wp_notify_moderator($comment_id) {
        ...
        if (0 == get_option('moderation_notify')) return true;
        ...
        if (user_can($user->ID, 'edit_comment', $comment_id) && !empty($user->user_email)) {
            if (0 !== strcasecmp($user->user_email, get_option('admin_email'))) $emails[] = $user->user_email;
        }
...
if (!function_exists('wp_rand')) :
    ...
    function wp_rand($min = 0, $max = 0) {
        ...
        if (strlen($rnd_value) < 8) {
            if (defined('WP_SETUP_CONFIG')) static $seed = ''; else
                $seed = get_transient('random_seed');
            $rnd_value = md5(uniqid(microtime() . mt_rand(), true) . $seed);
            $rnd_value .= sha1($rnd_value);
            $rnd_value .= sha1($rnd_value . $seed);
            $seed = md5($seed . $rnd_value);
            if (!defined('WP_SETUP_CONFIG')) set_transient('random_seed', $seed);
        }
...
if (!function_exists('get_avatar')) :
    ...
    function get_avatar($id_or_email, $size = 96, $default = '', $alt = '', $args = null) {
        $defaults = array(// get_avatar_data() args.
            'size' => 96, 'height' => null, 'width' => null, 'default' => get_option('avatar_default', 'mystery'), 'force_default' => false, 'rating' => get_option('avatar_rating'), 'scheme' => null, 'alt' => '', 'class' => null, 'force_display' => false, 'extra_attr' => '',);
...
if (!function_exists('wp_text_diff')) :
    ...
    function wp_text_diff($left_string, $right_string, $args = null) {
        ...
        if (!class_exists('WP_Text_Diff_Renderer_Table')) require(ABSPATH . WPINC . '/wp-diff.php');
        ...
        if ($args['title']) $r .= "<tr class='diff-title'><th colspan='4'>$args[title]</th></tr>\n";
