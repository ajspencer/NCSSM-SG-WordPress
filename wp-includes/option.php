...
function get_option($option, $default = false) {
    ...
    if ('home' == $option && '' == $value) return get_option('siteurl');

    if (in_array($option, array('siteurl', 'home', 'category_base', 'tag_base'))) $value = untrailingslashit($value);
...
function wp_protect_special_option($option) {
    if ('alloptions' === $option || 'notoptions' === $option) wp_die(sprintf(__('%s is a protected WP option and may not be modified'), esc_html($option)));
}
...
function wp_load_alloptions() {
    ...
    if (!defined('WP_INSTALLING') || !is_multisite()) $alloptions = wp_cache_get('alloptions', 'options'); else
        $alloptions = false;

    if (!$alloptions) {
        $suppress = $wpdb->suppress_errors();
        if (!$alloptions_db = $wpdb->get_results("SELECT option_name, option_value FROM $wpdb->options WHERE autoload = 'yes'")) $alloptions_db = $wpdb->get_results("SELECT option_name, option_value FROM $wpdb->options");
        $wpdb->suppress_errors($suppress);
        $alloptions = array();
        foreach ((array)$alloptions_db as $o) {
            $alloptions[$o->option_name] = $o->option_value;
        }
        if (!defined('WP_INSTALLING') || !is_multisite()) wp_cache_add('alloptions', $alloptions, 'options');
    }
...
function wp_load_core_site_options($site_id = null) {
    ...
    if (!is_multisite() || wp_using_ext_object_cache() || defined('WP_INSTALLING')) return;

    if (empty($site_id)) $site_id = $wpdb->siteid;
...
function add_option($option, $value = '', $deprecated = '', $autoload = 'yes') {
    ...
    if (!is_array($notoptions) || !isset($notoptions[$option])) /** This filter is documented in wp-includes/option.php */
        if (apply_filters('default_option_' . $option, false) !== get_option($option)) return false;
...
function delete_transient($transient) {
    ...
    if (wp_using_ext_object_cache()) {
        ...
        if ($result) delete_option($option_timeout);
    }
...
function add_site_option($option, $value) {
    ...
    if (!is_multisite()) {
        ...
        // Make sure the option doesn't already exist. We can check the 'notoptions' cache before we ask for a db query
        $notoptions = wp_cache_get($notoptions_key, 'site-options');
        if (!is_array($notoptions) || !isset($notoptions[$option])) if (false !== get_site_option($option)) return false;
...
function update_site_option($option, $value) {
    ...
    if (false === $old_value) return add_site_option($option, $value);
...
function delete_site_transient($transient) {
    ...
    if (wp_using_ext_object_cache()) {
        ...
        if ($result) delete_site_option($option_timeout);
    }
    if ($result) {
...
function get_site_transient($transient) {
    ...
    if (wp_using_ext_object_cache()) {
        ...
        if (!isset($value)) $value = get_site_option($transient_option);
    }
...
function set_site_transient($transient, $value, $expiration = 0) {
    ...
    if (wp_using_ext_object_cache()) {
        ...
        if (false === get_site_option($option)) {
            if ($expiration) add_site_option($transient_timeout, time() + $expiration);
