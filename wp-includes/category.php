...
function get_categories($args = '') {
    ...
    foreach (array_keys($categories) as $k) _make_cat_compat($categories[$k]);
...
function get_category_by_path($category_path, $full_match = true, $output = OBJECT) {
    ...
    $full_path = '';
    foreach ((array)$category_paths as $pathdir) $full_path .= ($pathdir != '' ? '/' : '') . sanitize_title($pathdir);
    ...
    foreach ($categories as $category) {
        ...
        while (($curcategory->parent != 0) && ($curcategory->parent != $curcategory->term_id)) {
            ...
            if (is_wp_error($curcategory)) return $curcategory;
