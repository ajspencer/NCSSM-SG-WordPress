...
class WP_Customize_Setting {
    ...
    public function __construct($manager, $id, $args = array()) {
        ...
        if (!empty($this->id_data['keys'])) $this->id .= '[' . implode('][', $this->id_data['keys']) . ']';

        if ($this->sanitize_callback) add_filter("customize_sanitize_{$this->id}", $this->sanitize_callback, 10, 2);

        if ($this->sanitize_js_callback) add_filter("customize_sanitize_js_{$this->id}", $this->sanitize_js_callback, 10, 2);
    }
    ...
    public function preview() {
        ...
        switch ($this->type) {
            case 'theme_mod' :
                ...
                break;
            case 'option' :
                if (empty($this->id_data['keys'])) add_filter('pre_option_' . $this->id_data['base'], array($this, '_preview_filter')); else {
    ...
    final public function save() {
        ...
        if (!$this->check_capabilities() || !isset($value)) return false;
    ...
    protected function _update_theme_mod($value) {
        ...
        if (empty($this->id_data['keys'])) return set_theme_mod($this->id_data['base'], $value);
        ...
        if (isset($mods)) return set_theme_mod($this->id_data['base'], $mods);
    }
    ...
    protected function _update_option($value) {
        ...
        if (empty($this->id_data['keys'])) return update_option($this->id_data['base'], $value);
        ...
        if (isset($options)) return update_option($this->id_data['base'], $options);
    }
    ...
    public function value() {
        ...
        if (empty($this->id_data['keys'])) return $function($this->id_data['base'], $this->default);
    ...
    final public function check_capabilities() {
        if ($this->capability && !call_user_func_array('current_user_can', (array)$this->capability)) return false;

        if ($this->theme_supports && !call_user_func_array('current_theme_supports', (array)$this->theme_supports)) return false;
    ...
    final protected function multidimensional(&$root, $keys, $create = false) {
        if ($create && empty($root)) $root = array();

        if (!isset($root) || empty($keys)) return;

        $last = array_pop($keys);
        $node = &$root;

        foreach ($keys as $key) {
            if ($create && !isset($node[$key])) $node[$key] = array();

            if (!is_array($node) || !isset($node[$key])) return;
    ...
    final protected function multidimensional_replace($root, $keys, $value) {
        if (!isset($value)) return $root; elseif (empty($keys)) // If there are no keys, we're replacing the root.
            return $value;
...
final class WP_Customize_Header_Image_Setting extends WP_Customize_Setting {
    ...
    public function update($value) {
        ...
        if (!$value) $value = $this->manager->get_setting('header_image')->post_value();

        if (is_array($value) && isset($value['choice'])) $custom_image_header->set_header_image($value['choice']); else
