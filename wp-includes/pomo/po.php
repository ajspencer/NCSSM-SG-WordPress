...
if (!class_exists('PO')):
    class PO extends Gettext_Translations {
        ...
        function export_headers() {
            ...
            if ($this->comments_before_headers) $before_headers = $this->prepend_each_line(rtrim($this->comments_before_headers) . "\n", '# '); else
                $before_headers = '';
            ...
            foreach ($lines as $line) {
                ...
                foreach ($chars as $char) {
                    if (!$previous_is_backslash) {
                        if ('\\' == $char) $previous_is_backslash = true; else
                            $unpoified .= $char;
                    } else {
                        $previous_is_backslash = false;
        ...
        function read_entry($f, $lineno = 0) {
            ...
            while (true) {
                ...
                if (!$line) {
                    if (feof($f)) {
                        if ($is_final($context)) break; elseif (!$context) // we haven't read a line and eof came
                            return null;
                        else
                            return false;
                    } else {
