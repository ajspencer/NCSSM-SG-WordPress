...
class WP_Widget {
    ...
    public function update_callback($deprecated = 1) {
        ...
        if (isset($_POST['delete_widget']) && $_POST['delete_widget']) {
            ...
            if (isset($_POST['the-widget-id'])) $del_id = $_POST['the-widget-id']; else
                return;

            if (isset($wp_registered_widgets[$del_id]['params'][0]['number'])) {
                $number = $wp_registered_widgets[$del_id]['params'][0]['number'];

                if ($this->id_base . '-' . $number == $del_id) unset($all_instances[$number]);
            }
        } else {
    ...
    public function get_settings() {
        ...
        if (false === $settings && isset($this->alt_option_name)) $settings = get_option($this->alt_option_name);
...
class WP_Widget_Factory {
    ...
    public function unregister($widget_class) {
        if (isset($this->widgets[$widget_class])) unset($this->widgets[$widget_class]);
    }
...
function register_sidebars($number = 1, $args = array()) {
    ...
    for ($i = 1; $i <= $number; $i++) {
        ...
        if ($number > 1) $_args['name'] = isset($args['name']) ? sprintf($args['name'], $i) : sprintf(__('Sidebar %d'), $i); else
        ...
        if (isset($args['id'])) {
            ...
            while (isset($wp_registered_sidebars[$_args['id']])) $_args['id'] = $args['id'] . '-' . $n++;
        } else {
...
function register_sidebar($args = array()) {
    ...
    $defaults = array('name' => sprintf(__('Sidebar %d'), $i), 'id' => "sidebar-$i", 'description' => '', 'class' => '', 'before_widget' => '<li id="%1$s" class="widget %2$s">', 'after_widget' => "</li>\n", 'before_title' => '<h2 class="widgettitle">', 'after_title' => "</h2>\n",);
...
function unregister_sidebar($name) {
    ...
    if (isset($wp_registered_sidebars[$name])) unset($wp_registered_sidebars[$name]);
}
...
function wp_register_sidebar_widget($id, $name, $output_callback, $options = array()) {
    ...
    if (in_array($output_callback, $_wp_deprecated_widgets_callbacks, true) && !is_callable($output_callback)) {
        if (isset($wp_registered_widget_controls[$id])) unset($wp_registered_widget_controls[$id]);

        if (isset($wp_registered_widget_updates[$id_base])) unset($wp_registered_widget_updates[$id_base]);
...
function wp_widget_description($id) {
    if (!is_scalar($id)) return;

    global $wp_registered_widgets;

    if (isset($wp_registered_widgets[$id]['description'])) return esc_html($wp_registered_widgets[$id]['description']);
}
...
function wp_sidebar_description($id) {
    ...
    if (isset($wp_registered_sidebars[$id]['description'])) return esc_html($wp_registered_sidebars[$id]['description']);
}
...
function wp_register_widget_control($id, $name, $control_callback, $options = array()) {
    ...
    if (in_array($control_callback, $_wp_deprecated_widgets_callbacks, true) && !is_callable($control_callback)) {
        if (isset($wp_registered_widgets[$id])) unset($wp_registered_widgets[$id]);
    ...
    if (isset($wp_registered_widget_updates[$id_base])) return;
...
function _register_widget_update_callback($id_base, $update_callback, $options = array()) {
    ...
    if (isset($wp_registered_widget_updates[$id_base])) {
        if (empty($update_callback)) unset($wp_registered_widget_updates[$id_base]);
        return;
    }
...
function is_dynamic_sidebar() {
    ...
    foreach ((array)$wp_registered_sidebars as $index => $sidebar) {
        if (count($sidebars_widgets[$index])) {
            foreach ((array)$sidebars_widgets[$index] as $widget) if (array_key_exists($widget, $wp_registered_widgets)) return true;
        }
    }
    return false;
}
...
function wp_get_sidebars_widgets($deprecated = true) {
    if ($deprecated !== true) _deprecated_argument(__FUNCTION__, '2.8.1');
    ...
    if (!is_admin()) {
        if (empty($_wp_sidebars_widgets)) $_wp_sidebars_widgets = get_option('sidebars_widgets', array());
...
function wp_get_widget_defaults() {
    ...
    foreach ((array)$wp_registered_sidebars as $index => $sidebar) $defaults[$index] = array();
...
function wp_convert_widget_settings($base_name, $option_name, $settings) {
    ...
    if ($single) {
        ...
        if (is_admin()) {
            ...
            if (empty($GLOBALS['_wp_sidebars_widgets'])) $GLOBALS['_wp_sidebars_widgets'] = get_option('sidebars_widgets', array());
        ...
        if (is_admin() && $changed) update_option('sidebars_widgets', $sidebars_widgets);
    }

    $settings['_multiwidget'] = 1;
    if (is_admin()) update_option($option_name, $settings);
...
function _wp_sidebars_changed() {
    ...
    if (!is_array($sidebars_widgets)) $sidebars_widgets = wp_get_sidebars_widgets();
...
function retrieve_widgets($theme_changed = false) {
    ...
    if (is_array($old_sidebars_widgets)) {
        ...
        if ($old == $registered_sidebar_keys) return;
        ...
        foreach ($sidebars_widgets as $val) {
            if (is_array($val) && !empty($val)) $_sidebars_widgets['orphaned_widgets_' . ++$orphaned] = $val;
        }
    }
    ...
    foreach ($_sidebars_widgets as $sidebar => $widgets) {
        ...
        $_widgets = array();
        foreach ($widgets as $widget) {
            if (isset($wp_registered_widgets[$widget])) $_widgets[] = $widget;
        }
    ...
    foreach ($wp_registered_widgets as $key => $val) {
        if (in_array($key, $shown_widgets, true)) continue;

        $number = preg_replace('/.+?-([0-9]+)$/', '$1', $key);

        if (2 > (int)$number) continue;
