...
function wptexturize($text, $reset = false) {
    ...
    $shortcode_regex = '\['              // Find start of shortcode.
...
function _wp_specialchars($string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false) {
    ...
    if (empty($quote_style)) $quote_style = ENT_NOQUOTES; elseif (!in_array($quote_style, array(0, 2, 3, 'single', 'double'), true)) $quote_style = ENT_QUOTES;
    ...
    if (in_array($charset, array('utf8', 'utf-8', 'UTF8'))) $charset = 'UTF-8';
...
function sanitize_title($title, $fallback_title = '', $context = 'save') {
    ...
    if ('save' == $context) $title = remove_accents($title);
...
function sanitize_title_with_dashes($title, $raw_title = '', $context = 'display') {
    ...
    if ('save' == $context) {
        ...
        $title = str_replace(array(// iexcl and iquest
            '%c2%a1', '%c2%bf', // angle quotes
            '%c2%ab', '%c2%bb', '%e2%80%b9', '%e2%80%ba', // curly quotes
            '%e2%80%98', '%e2%80%99', '%e2%80%9c', '%e2%80%9d', '%e2%80%9a', '%e2%80%9b', '%e2%80%9e', '%e2%80%9f', // copy, reg, deg, hellip and trade
            '%c2%a9', '%c2%ae', '%c2%b0', '%e2%80%a6', '%e2%84%a2', // acute accents
            '%c2%b4', '%cb%8a', '%cc%81', '%cd%81', // grave accent, macron, caron
            '%cc%80', '%cc%84', '%cc%8c',), '', $title);
...
function convert_chars($content, $deprecated = '') {
    if (!empty($deprecated)) _deprecated_argument(__FUNCTION__, '0.71');

    // Translation of invalid Unicode references range to valid range
    $wp_htmltranswinuni = array('&#128;' => '&#8364;', // the Euro sign
    ...
    while (preg_match("/<(\/?[\w:]*)\s*([^>]*)>/", $text, $regex)) {
        ...
        if (isset($regex[1][0]) && '/' == $regex[1][0]) { // End Tag
            ...
            elseif (substr($regex[2], -1) == '/') {
                ...
                if (!in_array($tag, $single_tags)) $regex[2] = trim(substr($regex[2], 0, -1)) . "></$tag";
            ...
            // Attributes
            $attributes = $regex[2];
            if (!empty($attributes) && $attributes[0] != '>') $attributes = ' ' . $attributes;
...
function format_to_edit($content, $richedit = false) {
    ...
    if (!$richedit) $content = esc_textarea($content);
    return $content;
}
...
function make_clickable($text) {
    ...
    foreach ($textarr as $piece) {
        ...
        if (preg_match('|^<code[\s>]|i', $piece) || preg_match('|^<pre[\s>]|i', $piece)) $nested_code_pre++; elseif (('</code>' === strtolower($piece) || '</pre>' === strtolower($piece)) && $nested_code_pre) $nested_code_pre--;
...
function get_gmt_from_date($string, $format = 'Y-m-d H:i:s') {
    ...
    if ($tz) {
        ...
        if (!$datetime) return gmdate($format, 0);
        $datetime->setTimezone(new DateTimeZone('UTC'));
        $string_gmt = $datetime->format($format);
    } else {
        if (!preg_match('#([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})#', $string, $matches)) return gmdate($format, 0);
...
function get_date_from_gmt($string, $format = 'Y-m-d H:i:s') {
    ...
    if ($tz) {
        ...
        if (!$datetime) return date($format, 0);
        $datetime->setTimezone(new DateTimeZone($tz));
        $string_localtime = $datetime->format($format);
    } else {
        if (!preg_match('#([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})#', $string, $matches)) return date($format, 0);
        $string_time = gmmktime($matches[4], $matches[5], $matches[6], $matches[2], $matches[3], $matches[1]);
...
function wp_htmledit_pre($output) {
    if (!empty($output)) $output = htmlspecialchars($output, ENT_NOQUOTES, get_option('blog_charset')); // convert only < > &
...
function esc_url($url, $protocols = null, $_context = 'display') {
	 ...
	 */
    if (strpos($url, ':') === false && !in_array($url[0], array('/', '#', '?')) && !preg_match('/^[a-z0-9-]+?\.php/i', $url)) $url = 'http://' . $url;
    ...
    if ('/' === $url[0]) {
        ...
        if (!is_array($protocols)) $protocols = wp_allowed_protocols();
...
function sanitize_option($option, $value) {
    ...
    switch ($option) {
        ...
        case 'default_role' :
            if (!get_role($value) && get_role('subscriber')) $value = 'subscriber';
            break;
...
function wp_pre_kses_less_than_callback($matches) {
    if (false === strpos($matches[0], '>')) return esc_html($matches[0]);
    return $matches[0];
}
...
function wp_sprintf($pattern) {
    ...
    while ($len > $start) {
        ...
        if ($pattern[$start] == '%') {
            ...
            if ($_fragment != $fragment) $fragment = $_fragment; else
...
function wp_sprintf_l($pattern, $args) {
    // Not a match
    if (substr($pattern, 0, 2) != '%l') return $pattern;
     ...
     */
    $l = apply_filters('wp_sprintf_l', array(/* translators: used to join items in a list with more than 2 items */
        'between' => sprintf(__('%s, %s'), '', ''), /* translators: used to join last two items in a list with more than 2 times */
        'between_last_two' => sprintf(__('%s, and %s'), '', ''), /* translators: used to join items in a list with only 2 items */
        'between_only_two' => sprintf(__('%s and %s'), '', ''),));

    $args = (array)$args;
    $result = array_shift($args);
    if (count($args) == 1) $result .= $l['between_only_two'] . array_shift($args);
    // Loop when more than two args
    $i = count($args);
    while ($i) {
        $arg = array_shift($args);
        $i--;
        if (0 == $i) $result .= $l['between_last_two'] . $arg; else
...
function wp_html_excerpt($str, $count, $more = null) {
    ...
    if ($str != $excerpt) $excerpt = trim($excerpt) . $more;
    return $excerpt;
}
...
function capital_P_dangit($text) {
    ...
    if ('the_title' === $current_filter || 'wp_title' === $current_filter) return str_replace('Wordpress', 'WordPress', $text);
    // Still here? Use the more judicious replacement
    static $dblq = false;
    if (false === $dblq) $dblq = _x('&#8220;', 'opening curly double quote');
...
function print_emoji_detection_script() {
    ...
    if ($develop_src || (defined('SCRIPT_DEBUG') && SCRIPT_DEBUG)) {
        $settings['source'] = array(/** This filter is documented in wp-includes/class.wp-scripts.php */
            'wpemoji' => apply_filters('script_loader_src', includes_url("js/wp-emoji.js?$version"), 'wpemoji'), /** This filter is documented in wp-includes/class.wp-scripts.php */
