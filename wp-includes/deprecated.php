...
function previous_post($format = '%', $previous = 'previous post: ', $title = 'yes', $in_same_cat = 'no', $limitprev = 1, $excluded_categories = '') {
    ...
    if (empty($in_same_cat) || 'no' == $in_same_cat) $in_same_cat = false; else
        $in_same_cat = true;
    ...
    if ('yes' == $title) $string .= apply_filters('the_title', $post->post_title, $post->ID);
    $string .= '</a>';
...
function user_can_edit_user($user_id, $other_user) {
    ...
    if ($user->user_level > $other->user_level || $user->user_level > 8 || $user->ID == $other->ID) return true; else
        return false;
}
...
function get_linkobjects($category = 0, $orderby = 'name', $limit = 0) {
    ...
    $links_array = array();
    foreach ($links as $link) $links_array[] = $link;
...
function wp_list_cats($args = '') {
    ...
    // Map to new names.
    if (isset($r['optionall']) && isset($r['all'])) $r['show_option_all'] = $r['all'];
    if (isset($r['sort_column'])) $r['orderby'] = $r['sort_column'];
    if (isset($r['sort_order'])) $r['order'] = $r['sort_order'];
    if (isset($r['optiondates'])) $r['show_last_update'] = $r['optiondates'];
    if (isset($r['optioncount'])) $r['show_count'] = $r['optioncount'];
    if (isset($r['list'])) $r['style'] = $r['list'] ? 'list' : 'break';
    $r['title_li'] = '';
...
function dropdown_cats($optionall = 1, $all = 'All', $orderby = 'ID', $order = 'asc', $show_last_update = 0, $show_count = 0, $hide_empty = 1, $optionnone = false, $selected = 0, $exclude = 0) {
    ...
    $show_option_all = '';
    if ($optionall) $show_option_all = $all;

    $show_option_none = '';
    if ($optionnone) $show_option_none = __('None');
...
function get_links($category = -1, $before = '', $after = '<br />', $between = ' ', $show_images = true, $orderby = 'name', $show_description = true, $show_rating = false, $limit = -1, $show_updated = 1, $echo = true) {
    ...
    foreach ((array)$results as $row) {
        ...
        $output .= $before;
        if ($show_updated && $row->recently_updated) $output .= get_option('links_recently_updated_prepend');
        $the_link = '#';
        if (!empty($row->link_url)) $the_link = esc_url($row->link_url);
        $rel = $row->link_rel;
        if ('' != $rel) $rel = ' rel="' . $rel . '"';
        ...
        if ($show_updated) if (substr($row->link_updated_f, 0, 2) != '00') $title .= ' (' . __('Last updated') . ' ' . date(get_option('links_updated_date_format'), $row->link_updated_f + (get_option('gmt_offset') * HOUR_IN_SECONDS)) . ')';
        ...
        if ($row->link_image != null && $show_images) {
            if (strpos($row->link_image, 'http') !== false) $output .= "<img src=\"$row->link_image\" $alt $title />"; else // If it's a relative path
        ...
        if ($show_updated && $row->recently_updated) $output .= get_option('links_recently_updated_append');
...
function get_linkcatname($id = 0) {
    ...
    if (empty($cats) || !is_array($cats)) return '';
...
function get_the_attachment_link($id = 0, $fullsize = false, $max_dims = false, $permalink = false) {
    ...
    if (('attachment' != $_post->post_type) || !$url = wp_get_attachment_url($_post->ID)) return __('Missing Attachment');

    if ($permalink) $url = get_attachment_link($_post->ID);
...
function get_attachment_icon_src($id = 0, $fullsize = false) {
    ...
    $id = (int)$id;
    if (!$post = get_post($id)) return false;
...
function get_attachment_icon($id = 0, $fullsize = false, $max_dims = false) {
    ...
    if (!$src = get_attachment_icon_src($post->ID, $fullsize)) return false;
...
function get_attachment_innerHTML($id = 0, $fullsize = false, $max_dims = false) {
    ...
    if ($innerHTML = get_attachment_icon($post->ID, $fullsize, $max_dims)) return $innerHTML;
...
function clean_url($url, $protocols = null, $context = 'display') {
    if ($context == 'db') _deprecated_function('clean_url( $context = \'db\' )', '3.0', 'esc_url_raw()'); else
...
function register_sidebar_widget($name, $output_callback, $classname = '') {
    ...
    // Compat
    if (is_array($name)) {
        if (count($name) == 3) $name = sprintf($name[0], $name[2]); else
            $name = $name[0];
    }

    $id = sanitize_title($name);
    $options = array();
    if (!empty($classname) && is_string($classname)) $options['classname'] = $classname;
    $params = array_slice(func_get_args(), 2);
    $args = array($id, $name, $output_callback, $options);
    if (!empty($params)) $args = array_merge($args, $params);
...
function delete_usermeta($user_id, $meta_key, $meta_value = '') {
    ...
    if ($cur && $cur->umeta_id) do_action('delete_usermeta', $cur->umeta_id, $user_id, $meta_key, $meta_value);

    if (!empty($meta_value)) $wpdb->query($wpdb->prepare("DELETE FROM $wpdb->usermeta WHERE user_id = %d AND meta_key = %s AND meta_value = %s", $user_id, $meta_key, $meta_value)); else
        $wpdb->query($wpdb->prepare("DELETE FROM $wpdb->usermeta WHERE user_id = %d AND meta_key = %s", $user_id, $meta_key));

    clean_user_cache($user_id);
    wp_cache_delete($user_id, 'user_meta');

    if ($cur && $cur->umeta_id) do_action('deleted_usermeta', $cur->umeta_id, $user_id, $meta_key, $meta_value);
...
function get_usermeta($user_id, $meta_key = '') {
    ...
    if (!empty($meta_key)) {
        ...
        if (false !== $user && isset($user->$meta_key)) $metas = array($user->$meta_key); else
    ...
    if (empty($metas)) {
        if (empty($meta_key)) return array(); else
            return '';
    }

    $metas = array_map('maybe_unserialize', $metas);

    if (count($metas) == 1) return $metas[0]; else
        return $metas;
}
...
function update_usermeta($user_id, $meta_key, $meta_value) {
    ...
    if ($cur) do_action('update_usermeta', $cur->umeta_id, $user_id, $meta_key, $meta_value);

    if (!$cur) $wpdb->insert($wpdb->usermeta, compact('user_id', 'meta_key', 'meta_value')); elseif ($cur->meta_value != $meta_value) $wpdb->update($wpdb->usermeta, compact('meta_value'), compact('user_id', 'meta_key'));
    else
        return false;
    ...
    if (!$cur) do_action('added_usermeta', $wpdb->insert_id, $user_id, $meta_key, $meta_value); else
...
function automatic_feed_links($add = true) {
    ...
    if ($add) add_theme_support('automatic-feed-links'); else
...
function is_plugin_page() {
    ...
    if (isset($plugin_page)) return true;
...
function get_user_metavalues($ids) {
    ...
    foreach ($ids as $id) $objects[$id] = array();
    ...
    if (is_object($user)) {
        ...
        if (!($user instanceof WP_User)) {
            ...
            foreach (array_keys($vars) as $field) {
                if (is_string($user->$field) || is_numeric($user->$field)) $user->$field = sanitize_user_field($field, $user->$field, $user->ID, $context);
            }
        }
        $user->filter = $context;
    } else {
        if (!isset($user['ID'])) $user['ID'] = 0;
        foreach (array_keys($user) as $field) $user[$field] = sanitize_user_field($field, $user[$field], $user['ID'], $context);
        $user['filter'] = $context;
    }
...
function get_boundary_post_rel_link($title = '%title', $in_same_cat = false, $excluded_categories = '', $start = true) {
    ...
    if (empty($post->post_title)) $post->post_title = $start ? __('First Post') : __('Last Post');
...
function get_parent_post_rel_link($title = '%title') {
    ...
    if (!empty($GLOBALS['post']) && !empty($GLOBALS['post']->post_parent)) $post = get_post($GLOBALS['post']->post_parent);
...
function wp_admin_bar_dashboard_view_site_menu($wp_admin_bar) {
    ...
    if (0 != $user_id) {
        if (is_admin()) $wp_admin_bar->add_menu(array('id' => 'view-site', 'title' => __('Visit Site'), 'href' => home_url())); elseif (is_multisite()) $wp_admin_bar->add_menu(array('id' => 'dashboard', 'title' => __('Dashboard'), 'href' => get_dashboard_url($user_id)));
        else
...
function debug_fwrite($fp, $string) {
    _deprecated_function(__FUNCTION__, 'error_log()');
    if (!empty($GLOBALS['debug'])) error_log($string);
}
...
function get_themes() {
    ...
    global $wp_themes;
    if (isset($wp_themes)) return $wp_themes;
    ...
    foreach ($themes as $theme) {
        $name = $theme->get('Name');
        if (isset($wp_themes[$name])) $wp_themes[$name . '/' . $theme->get_stylesheet()] = $theme; else
...
function get_theme($theme) {
    ...
    $themes = get_themes();
    if (is_array($themes) && array_key_exists($theme, $themes)) return $themes[$theme];
    return null;
}
...
function get_current_theme() {
    ...
    if ($theme = get_option('current_theme')) return $theme;
...
function clean_pre($matches) {
    ...
    if (is_array($matches)) $text = $matches[1] . $matches[2] . "</pre>"; else
        $text = $matches;
...
function get_theme_data($theme_file) {
    ...
    foreach (apply_filters('extra_theme_headers', array()) as $extra_header) {
        if (!isset($theme_data[$extra_header])) $theme_data[$extra_header] = $theme->get($extra_header);
    }
...
function sticky_class($post_id = null) {
    ...
    if (is_sticky($post_id)) echo ' sticky';
}
...
function wp_load_image($file) {
    ...
    if (!is_file($file)) return sprintf(__('File &#8220;%s&#8221; doesn&#8217;t exist?'), $file);
    ...
    if (!is_resource($image)) return sprintf(__('File &#8220;%s&#8221; is not an image.'), $file);
...
function rich_edit_exists() {
    ...
    if (!isset($wp_rich_edit_exists)) $wp_rich_edit_exists = file_exists(ABSPATH . WPINC . '/js/tinymce/tinymce.js');
