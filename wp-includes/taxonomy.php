...
function create_initial_taxonomies() {
    ...
    register_taxonomy('category', 'post', array('hierarchical' => true, 'query_var' => 'category_name', 'rewrite' => $rewrite['category'], 'public' => true, 'show_ui' => true, 'show_admin_column' => true, '_builtin' => true,));

    register_taxonomy('post_tag', 'post', array('hierarchical' => false, 'query_var' => 'tag', 'rewrite' => $rewrite['post_tag'], 'public' => true, 'show_ui' => true, 'show_admin_column' => true, '_builtin' => true,));
...
function get_object_taxonomies($object, $output = 'names') {
    ...
    if (is_object($object)) {
        if ($object->post_type == 'attachment') return get_attachment_taxonomies($object);
    ...
    foreach ((array)$wp_taxonomies as $tax_name => $tax_obj) {
        if (array_intersect($object, (array)$tax_obj->object_type)) {
            if ('names' == $output) $taxonomies[] = $tax_name; else
...
function register_taxonomy($taxonomy, $object_type, $args = array()) {
    ...
    if (false !== $args['query_var'] && !empty($wp)) {
        if (true === $args['query_var']) $args['query_var'] = $taxonomy; else
    ...
    if (false !== $args['rewrite'] && (is_admin() || '' != get_option('permalink_structure'))) {
        ...
        if (empty($args['rewrite']['slug'])) $args['rewrite']['slug'] = sanitize_title_with_dashes($taxonomy);

        if ($args['hierarchical'] && $args['rewrite']['hierarchical']) $tag = '(.+?)'; else
            $tag = '([^/]+)';
    ...
    if (null === $args['meta_box_cb']) {
        if ($args['hierarchical']) $args['meta_box_cb'] = 'post_categories_meta_box'; else
...
function get_taxonomy_labels($tax) {
    ...
    if (isset($tax->helps) && empty($tax->labels['separate_items_with_commas'])) $tax->labels['separate_items_with_commas'] = $tax->helps;

    if (isset($tax->no_tagcloud) && empty($tax->labels['not_found'])) $tax->labels['not_found'] = $tax->no_tagcloud;
...
function register_taxonomy_for_object_type($taxonomy, $object_type) {
    ...
    if (!get_post_type_object($object_type)) return false;

    if (!in_array($object_type, $wp_taxonomies[$taxonomy]->object_type)) $wp_taxonomies[$taxonomy]->object_type[] = $object_type;
...
function get_term_children($term_id, $taxonomy) {
    if (!taxonomy_exists($taxonomy)) return new WP_Error('invalid_taxonomy', __('Invalid taxonomy'));

    $term_id = intval($term_id);

    $terms = _get_term_hierarchy($taxonomy);

    if (!isset($terms[$term_id])) return array();
    ...
    foreach ((array)$terms[$term_id] as $child) {
        ...
        if (isset($terms[$child])) $children = array_merge($children, get_term_children($child, $taxonomy));
    }
...
function term_exists($term, $taxonomy = '', $parent = null) {
    ...
    if (is_int($term)) {
        if (0 == $term) return 0;
        $where = 't.term_id = %d';
        if (!empty($taxonomy)) return $wpdb->get_row($wpdb->prepare($tax_select . $where . " AND tt.taxonomy = %s", $term, $taxonomy), ARRAY_A); else
    ...
    if (!empty($taxonomy)) {
        ...
        if ($result = $wpdb->get_row($wpdb->prepare("SELECT tt.term_id, tt.term_taxonomy_id FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy as tt ON tt.term_id = t.term_id WHERE $where AND tt.taxonomy = %s $orderby $limit", $where_fields), ARRAY_A)) return $result;

        return $wpdb->get_row($wpdb->prepare("SELECT tt.term_id, tt.term_taxonomy_id FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy as tt ON tt.term_id = t.term_id WHERE $else_where AND tt.taxonomy = %s $orderby $limit", $else_where_fields), ARRAY_A);
    }

    if ($result = $wpdb->get_var($wpdb->prepare("SELECT term_id FROM $wpdb->terms as t WHERE $where $orderby $limit", $where_fields))) return $result;
...
function term_is_ancestor_of($term1, $term2, $taxonomy) {
    if (!isset($term1->term_id)) $term1 = get_term($term1, $taxonomy);
    if (!isset($term2->parent)) $term2 = get_term($term2, $taxonomy);

    if (empty($term1->term_id) || empty($term2->parent)) return false;
...
function sanitize_term($term, $taxonomy, $context = 'display') {
    ...
    foreach ((array)$fields as $field) {
        if ($do_object) {
            if (isset($term->$field)) $term->$field = sanitize_term_field($field, $term->$field, $term_id, $taxonomy, $context);
        } else {
            if (isset($term[$field])) $term[$field] = sanitize_term_field($field, $term[$field], $term_id, $taxonomy, $context);
        }
    }

    if ($do_object) $term->filter = $context; else
        $term['filter'] = $context;
...
function sanitize_term_field($field, $value, $term_id, $taxonomy, $context) {
    ...
    if ('edit' == $context) {
        ...
        if ('description' == $field) $value = esc_html($value); // textarea_escaped
        else
...
function wp_delete_term($term, $taxonomy, $args = array()) {
    ...
    if (!$ids = term_exists($term, $taxonomy)) return false;
    ...
    if (is_taxonomy_hierarchical($taxonomy)) {
        ...
        if (is_wp_error($term_obj)) return $term_obj;
    ...
    foreach ((array)$objects as $object) {
        ...
        if (1 == count($terms) && isset($default)) {
            ...
            if (isset($default) && isset($force_default) && $force_default) $terms = array_merge($terms, array($default));
        }
    ...
    foreach ($tax_object->object_type as $object_type) clean_object_term_cache($objects, $object_type);
    ...
    if (!$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->term_taxonomy WHERE term_id = %d", $term))) $wpdb->delete($wpdb->terms, array('term_id' => $term));
...
function wp_get_object_terms($object_ids, $taxonomies, $args = array()) {
    ...
    if (empty($object_ids) || empty($taxonomies)) return array();
    ...
    if (('tt_ids' == $fields) && !empty($orderby)) $orderby = 'tr.term_taxonomy_id';

    if (!empty($orderby)) $orderby = "ORDER BY $orderby";

    $order = strtoupper($order);
    if ('' !== $order && !in_array($order, array('ASC', 'DESC'))) $order = 'ASC';
...
function wp_set_object_terms($object_id, $terms, $taxonomy, $append = false) {
    ...
    if (!$append) $old_tt_ids = wp_get_object_terms($object_id, $taxonomy, array('fields' => 'tt_ids', 'orderby' => 'none')); else
        $old_tt_ids = array();
    ...
    foreach ((array)$terms as $term) {
        ...
        if (is_wp_error($term_info)) return $term_info;
        $term_ids[] = $term_info['term_id'];
        $tt_id = $term_info['term_taxonomy_id'];
        $tt_ids[] = $tt_id;

        if ($wpdb->get_var($wpdb->prepare("SELECT term_taxonomy_id FROM $wpdb->term_relationships WHERE object_id = %d AND term_taxonomy_id = %d", $object_id, $tt_id))) continue;
    ...
    if (!$append && isset($t->sort) && $t->sort) {
        ...
        foreach ($tt_ids as $tt_id) if (in_array($tt_id, $final_tt_ids)) $values[] = $wpdb->prepare("(%d, %d, %d)", $object_id, $tt_id, ++$term_order);
        if ($values) if (false === $wpdb->query("INSERT INTO $wpdb->term_relationships (object_id, term_taxonomy_id, term_order) VALUES " . join(',', $values) . " ON DUPLICATE KEY UPDATE term_order = VALUES(term_order)")) return new WP_Error('db_insert_error', __('Could not insert term relationship into the database'), $wpdb->last_error);
    }
...
function wp_unique_term_slug($slug, $term) {
    ...
    if (!empty($term->term_id)) $query = $wpdb->prepare("SELECT slug FROM $wpdb->terms WHERE slug = %s AND term_id != %d", $slug, $term->term_id); else
...
function wp_update_term($term_id, $taxonomy, $args = array()) {
    ...
    if ('' == trim($name)) return new WP_Error('empty_term_name', __('A name is required for this term'));
    ...
    if ($duplicate && $duplicate->term_id != $term_id) {
        ...
        // Otherwise, bail.
        if ($empty_slug || ($parent != $term['parent'])) $slug = wp_unique_term_slug($slug, (object)$args); else
...
function wp_defer_term_counting($defer = null) {
    ...
    if (is_bool($defer)) {
        ...
        if (!$defer) wp_update_term_count(null, null, true);
    }
...
function wp_update_term_count($terms, $taxonomy, $do_deferred = false) {
    ...
    if (wp_defer_term_counting()) {
        if (!isset($_deferred[$taxonomy])) $_deferred[$taxonomy] = array();
...
function wp_update_term_count_now($terms, $taxonomy) {
    ...
    if (!empty($taxonomy->update_count_callback)) {
        ...
        foreach ($object_types as &$object_type) {
            if (0 === strpos($object_type, 'attachment:')) list($object_type) = explode(':', $object_type);
        }
...
function update_object_term_cache($object_ids, $object_type) {
    ...
    $object_terms = array();
    foreach ((array)$terms as $term) $object_terms[$term->object_id][$term->taxonomy][] = $term;

    foreach ($ids as $id) {
        foreach ($taxonomies as $taxonomy) {
            if (!isset($object_terms[$id][$taxonomy])) {
                if (!isset($object_terms[$id])) $object_terms[$id] = array();
...
function update_term_cache($terms, $taxonomy = '') {
    foreach ((array)$terms as $term) {
        ...
        if (empty($term_taxonomy)) $term_taxonomy = $term->taxonomy;
...
function _get_term_children($term_id, $terms, $taxonomy, &$ancestors = array()) {
    ...
    if ((0 != $term_id) && !isset($has_children[$term_id])) return $empty_array;
    ...
    foreach ((array)$terms as $term) {
        ...
        if ($term->parent == $term_id) {
            if ($use_id) $term_list[] = $term->term_id; else
                $term_list[] = $term;

            if (!isset($has_children[$term->term_id])) continue;

            $ancestors[$term->term_id] = 1;

            if ($children = _get_term_children($term->term_id, $terms, $taxonomy, $ancestors)) $term_list = array_merge($term_list, $children);
        }
    }
...
function _pad_term_counts(&$terms, $taxonomy) {
    ...
    foreach ($term_ids as $term_id) {
        ...
        while (!empty($terms_by_id[$child]) && $parent = $terms_by_id[$child]->parent) {
            $ancestors[] = $child;
            if (!empty($term_items[$term_id])) foreach ($term_items[$term_id] as $item_id => $touches) {
    ...
    foreach ((array)$term_items as $id => $items) if (isset($terms_by_id[$id])) $terms_by_id[$id]->count = count($items);
}
...
function _update_post_term_count($terms, $taxonomy) {
    ...
    if ($object_types) $object_types = esc_sql(array_filter($object_types, 'post_type_exists'));
    ...
    foreach ((array)$terms as $term) {
        ...
        if ($object_types) $count += (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts WHERE $wpdb->posts.ID = $wpdb->term_relationships.object_id AND post_status = 'publish' AND post_type IN ('" . implode("', '", $object_types) . "') AND term_taxonomy_id = %d", $term));
...
function get_term_link($term, $taxonomy = '') {
    ...
    if (!is_object($term)) $term = new WP_Error('invalid_term', __('Empty Term'));
    ...
    if (empty($termlink)) {
        if ('category' == $taxonomy) $termlink = '?cat=' . $term->term_id; elseif ($t->query_var) $termlink = "?$t->query_var=$slug";
        else
...
function get_the_taxonomies($post = 0, $args = array()) {
    ...
    $args = wp_parse_args($args, array(/* translators: %s: taxonomy label, %l: list of terms formatted as per $term_template */
        'template' => __('%s: %l.'), 'term_template' => '<a href="%1$s">%2$s</a>',));
...
function is_object_in_term($object_id, $taxonomy, $terms = null) {
    if (!$object_id = (int)$object_id) return new WP_Error('invalid_object', __('Invalid object ID'));

    $object_terms = get_object_term_cache($object_id, $taxonomy);
    if (false === $object_terms) $object_terms = wp_get_object_terms($object_id, $taxonomy);
    ...
    if ($ints = array_filter($terms, 'is_int')) $strs = array_diff($terms, $ints); else
        $strs =& $terms;
...
function wp_check_term_hierarchy_for_loops($parent, $term_id, $taxonomy) {
    ...
    if (!$loop = wp_find_hierarchy_loop('wp_get_term_taxonomy_parent_id', $term_id, $parent, array($taxonomy))) return $parent; // No loop

    // Setting $parent to the given value causes a loop
    if (isset($loop[$term_id])) return 0;

    // There's a loop, but it doesn't contain $term_id. Break the loop.
    foreach (array_keys($loop) as $loop_member) wp_update_term($loop_member, $taxonomy, array('parent' => 0));
