...
function get_blog_details($fields = null, $get_all = true) {
    ...
    if (is_array($fields)) {
        ...
        if (!$fields) $blog_id = get_current_blog_id(); elseif (!is_numeric($fields)) $blog_id = get_id_from_blogname($fields);
        else
            $blog_id = $fields;
    }
...
function update_blog_details($blog_id, $details = array()) {
    ...
    if (is_object($details)) $details = get_object_vars($details);

    $current_details = get_blog_details($blog_id, false);
    if (empty($current_details)) return false;
...
function get_blog_option($id, $option, $default = false) {
    ...
    if (get_current_blog_id() == $id) return get_option($option, $default);
...
function add_blog_option($id, $option, $value) {
    ...
    if (get_current_blog_id() == $id) return add_option($option, $value);
...
function delete_blog_option($id, $option) {
    ...
    if (get_current_blog_id() == $id) return delete_option($option);
...
function update_blog_option($id, $option, $value, $deprecated = null) {
    ...
    if (get_current_blog_id() == $id) return update_option($option, $value);
...
function switch_to_blog($new_blog, $deprecated = null) {
    ...
    if (empty($new_blog)) $new_blog = $GLOBALS['blog_id'];
    ...
    if (function_exists('wp_cache_switch_to_blog')) {
        ...
        if (is_object($wp_object_cache) && isset($wp_object_cache->global_groups)) $global_groups = $wp_object_cache->global_groups; else
            $global_groups = false;
...
function update_blog_status($blog_id, $pref, $value, $deprecated = null) {
    ...
    if (!in_array($pref, array('site_id', 'domain', 'path', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id'))) return $value;
...
function get_last_updated($deprecated = '', $start = 0, $quantity = 40) {
    ...
    if (!empty($deprecated)) _deprecated_argument(__FUNCTION__, 'MU'); // never used
