...
function get_bookmark($bookmark, $output = OBJECT, $filter = 'raw') {
    ...
    if (empty($bookmark)) {
        if (isset($GLOBALS['link'])) $_bookmark = &$GLOBALS['link']; else
            $_bookmark = null;
...
function get_bookmark_field($field, $bookmark, $context = 'display') {
    ...
    if (is_wp_error($bookmark)) return $bookmark;
...
function sanitize_bookmark($bookmark, $context = 'display') {
    ...
    foreach ($fields as $field) {
        if ($do_object) {
            if (isset($bookmark->$field)) $bookmark->$field = sanitize_bookmark_field($field, $bookmark->$field, $link_id, $context);
        } else {
            if (isset($bookmark[$field])) $bookmark[$field] = sanitize_bookmark_field($field, $bookmark[$field], $link_id, $context);
        }
    }
...
function sanitize_bookmark_field($field, $value, $bookmark_id, $context) {
    switch ($field) {
        ...
        case 'link_target' : // "enum"
            ...
            if (!in_array($value, $targets)) $value = '';
            break;
    }
