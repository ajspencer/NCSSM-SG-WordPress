...
function before_last_bar($string) {
    ...
    if (false == $last_bar) return $string; else
...
function translate_nooped_plural($nooped_plural, $count, $domain = 'default') {
    ...
    if ($nooped_plural['context']) return _nx($nooped_plural['singular'], $nooped_plural['plural'], $count, $nooped_plural['context'], $domain); else
...
function unload_textdomain($domain) {
    ...
    if ($plugin_override) return true;
...
function load_default_textdomain($locale = null) {
    ...
    if (is_network_admin() || (defined('WP_INSTALLING_NETWORK') && WP_INSTALLING_NETWORK)) load_textdomain('default', WP_LANG_DIR . "/admin-network-$locale.mo");
...
function load_plugin_textdomain($domain, $deprecated = false, $plugin_rel_path = false) {
    ...
    if ($loaded = load_textdomain($domain, $path . '/' . $mofile)) return $loaded;
...
function load_child_theme_textdomain($domain, $path = false) {
    if (!$path) $path = get_stylesheet_directory();
