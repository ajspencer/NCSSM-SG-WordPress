...
class WP_Text_Diff_Renderer_Table extends Text_Diff_Renderer {
    ...
    public function _changed($orig, $final) {
        ...
        foreach (array_keys($orig_rows) as $row) {
            ...
            if ($orig_rows[$row] < 0 && $final_rows[$row] < 0) continue;

            // If we have a word based diff, use it. Otherwise, use the normal line.
            if (isset($orig_diffs[$orig_rows[$row]])) $orig_line = $orig_diffs[$orig_rows[$row]]; elseif (isset($orig[$orig_rows[$row]])) $orig_line = htmlspecialchars($orig[$orig_rows[$row]]);
            else
                $orig_line = '';

            if (isset($final_diffs[$final_rows[$row]])) $final_line = $final_diffs[$final_rows[$row]]; elseif (isset($final[$final_rows[$row]])) $final_line = htmlspecialchars($final[$final_rows[$row]]);
            else
                $final_line = '';
    ...
    public function interleave_changed_lines($orig, $final) {
        ...
        foreach ($matches as $keys => $difference) {
            ...
            if (isset($orig_matches[$o]) && isset($final_matches[$f])) continue;
