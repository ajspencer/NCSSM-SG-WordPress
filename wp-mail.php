...
if (!apply_filters('enable_post_by_email_configuration', true)) wp_die(__('This action has been disabled by the administrator.'));
...
if (!defined('WP_MAIL_INTERVAL')) define('WP_MAIL_INTERVAL', 300); // 5 minutes
...
if (!$pop3->connect(get_option('mailserver_url'), get_option('mailserver_port')) || !$pop3->user(get_option('mailserver_login'))) wp_die(esc_html($pop3->ERROR));

$count = $pop3->pass(get_option('mailserver_pass'));

if (false === $count) wp_die(esc_html($pop3->ERROR));
...
for ($i = 1; $i <= $count; $i++) {
    ...
    foreach ($message as $line) {
        // Body signal.
        if (strlen($line) < 3) $bodysignal = true;
        if ($bodysignal) {
            $content .= $line;
        } else {
            ...
            if (!$author_found && preg_match('/^(From|Reply-To): /', $line)) {
                if (preg_match('|[a-z0-9_.-]+@[a-z0-9_.-]+(?!.*<)|i', $line, $matches)) $author = $matches[0]; else
                    $author = trim($line);
