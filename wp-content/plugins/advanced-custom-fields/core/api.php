...
function get_fields($post_id = false, $format_value = true) {
    ...
    // vars
    $options = array('load_value' => true, 'format_value' => $format_value);
...
function api_acf_get_field_groups($return) {
    ...
    foreach ($GLOBALS['acf_register_field_group'] as $acf) {
        $return[] = array('id' => $acf['id'], 'title' => $acf['title'], 'menu_order' => $acf['menu_order'],);
    }
...
function acf_shortcode($atts) {
    // extract attributs
    extract(shortcode_atts(array('field' => "", 'post_id' => false,), $atts));
...
function acf_form_head() {
    ...
    // need wp styling
    wp_enqueue_style(array('colors-fresh'));
...
function acf_form($options = array()) {
    ...
    // defaults
    $defaults = array('post_id' => false, 'field_groups' => array(), 'form' => true, 'form_attributes' => array('id' => 'post', 'class' => '', 'action' => '', 'method' => 'post',), 'return' => add_query_arg('updated', 'true', get_permalink()), 'html_before_fields' => '', 'html_after_fields' => '', 'submit_value' => __("Update", 'acf'), 'updated_message' => __("Post updated", 'acf'),);
