...
class acf_controller_post {
    ...
    */
    ...
    function admin_head() {
        ...
        // get field groups
        $filter = array('post_id' => $post_id, 'post_type' => $typenow);
        $metabox_ids = array();
        ...
        if ($acfs) {
            foreach ($acfs as $acf) {
                ...
                // add meta box
                add_meta_box('acf_' . $acf['id'], $acf['title'], array($this, 'meta_box_input'), $typenow, $acf['options']['position'], $priority, array('field_group' => $acf, 'show' => $show, 'post_id' => $post_id));
    ...
    */
    ...
    function ajax_render_fields() {
        ...
        // defaults
        $options = array('acf_id' => 0, 'post_id' => 0, 'nonce' => '');
