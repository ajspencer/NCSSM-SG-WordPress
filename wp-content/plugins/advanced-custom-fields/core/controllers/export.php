...
class acf_export {
    ...
    */
    ...
    function html_index() {
        // vars
        $acfs = get_posts(array('numberposts' => -1, 'post_type' => 'acf', 'orderby' => 'menu_order title', 'order' => 'asc',));
            ...
            <div class="wp-box">
                ...
                <table class="acf_input widefat">
                    <tr>
                        ...
                        <td>
                            <?php do_action('acf/create_field', array('type' => 'select', 'name' => 'acf_posts', 'value' => '', 'choices' => $choices, 'multiple' => 1,)); ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="label"></td>
...
define( 'ACF_LITE', true );
				...
				<textarea class="pre" readonly="true"><?php
                    ...
                    if (isset($_POST['acf_posts'])) {
                        $acfs = get_posts(array('numberposts' => -1, 'post_type' => 'acf', 'orderby' => 'menu_order title', 'order' => 'asc', 'include' => $_POST['acf_posts'], 'suppress_filters' => false,));
                    }
                    if ($acfs) {
                        ?>
                        ...
                        foreach ($acfs as $i => $acf) {
                            // populate acfs
                            $var = array('id' => $acf->post_name, 'title' => $acf->post_title, 'fields' => apply_filters('acf/field_group/get_fields', array(), $acf->ID), 'location' => apply_filters('acf/field_group/get_location', array(), $acf->ID), 'options' => apply_filters('acf/field_group/get_options', array(), $acf->ID), 'menu_order' => $acf->menu_order,);
