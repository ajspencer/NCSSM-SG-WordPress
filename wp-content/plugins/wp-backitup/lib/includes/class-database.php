...
class WPBackItUp_DataAccess {
    ...
    public function insert_job_items($sql_values) {
        ...
        if (false === $sql_rtn || $sql_rtn == 0) return false; else return true;
    ...
    public function create_job_control($job_id) {
        ...
        $sql = $wpdb->prepare("INSERT  $wpdb->wpbackitup_job
    ...
    private function update_job_control_status($job_id, $status) {
        ...
        $sql = $wpdb->prepare("UPDATE  $wpdb->wpbackitup_job
				set status=%s
		         ,update_date=NOW()
		     WHERE
		     	  record_type=%s
    ...
    function get_old_batch_control($days) {
        ...
        $sql_select = $wpdb->prepare("SELECT * FROM $wpdb->wpbackitup_job
			          WHERE
						  record_type=%s
    ...
    function get_batch_open_tasks($batch_id, $batch_size, $job_id, $group_id) {
        ...
        $sql_update = $wpdb->prepare("UPDATE  $wpdb->wpbackitup_job
				set batch_id=%d
    ...
    function delete_job_records($job_id) {
        ...
        $sql_update = $wpdb->prepare("DELETE FROM $wpdb->wpbackitup_job
		     WHERE
				job_id=%d
		    ", $job_id);
    ...
    function get_open_task_count($job_id, $group_id) {
        ...
        $sql = $wpdb->prepare("SELECT count(*) as task_count FROM $wpdb->wpbackitup_job
		     WHERE
		     	  record_type=%s
				  && job_id=%d
				  && group_id=%s
