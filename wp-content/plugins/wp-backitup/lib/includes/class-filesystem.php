...
//        $this->logger->log($ignore);
    ...
    private function ignore($file, $ignoreList) {
        ...
        if (in_array(basename($file), $ignoreList) || substr($file, 0, 1) == '.' || ($file == ".") || ($file == "..") || ($file == "._") || ($file == "cgi-bin")) {
    ...
    private function delete_ignore($file, $ignoreList) {
        ...
        //Exclude these files and folders from the delete
        if (in_array(basename($file), $ignoreList) || //substr($file, 0, 1) == '.'   ||
            ($file == ".") || ($file == "..")
        )
            //($file == "._" )
            //($file == "cgi-bin" ))
    ...
    private function is_backup_folder($dir) {
        if (strpos(strtolower($dir), '/wp-content/backup') !== false || strpos(strtolower($dir), '/wp-content/updraft') !== false || strpos(strtolower($dir), '/wp-content/wp-clone') !== false || strpos(strtolower($dir), '/wp-content/uploads/backwpup') !== false || strpos(strtolower($dir), '/wp-content/uploads/backupwordpress') !== false) {
