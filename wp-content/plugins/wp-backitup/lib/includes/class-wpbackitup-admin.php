...
class WPBackitup_Admin {
    ...
    public $defaults = array('logging' => false, 'license_key' => "lite", 'license_last_check_date' => "1970-01-01 00:00:00", 'license_status' => "", 'license_status_message' => "", 'license_type' => "0", 'license_expires' => "1970-01-01 00:00:00", 'license_limit' => "1", 'license_sitecount' => "", 'license_customer_name' => "", 'license_customer_email' => "", 'notification_email' => "", 'backup_retained_number' => "3", 'lite_backup_retained_number' => "1", 'backup_count' => 0, 'successful_backup_count' => 0, 'stats_last_check_date' => "1970-01-01 00:00:00", 'backup_schedule' => "", 'backup_lastrun_date' => "-2147483648", 'cleanup_lastrun_date' => "-2147483648", 'backup_batch_size' => "500", 'support_email' => "",);
...
//			}
        ...
        if (wp_verify_nonce($_REQUEST['_wpnonce'], "{$this->namespace}-register")) {
            ...
            if ($this->license_type() == '0') {
                ...
                if (!empty($license_email) && filter_var($license_email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $form_data = array('email' => $license_email, 'site' => $domain, 'name' => $license_name, 'time_zone' => get_option('timezone_string'),);
                    ...
                    $response = wp_remote_post($post_url, array('method' => 'POST', 'timeout' => 45, 'blocking' => true, 'headers' => array(), 'body' => $form_data, 'cookies' => array()));
    ...
    public function get_backup_list() {
        ...
        if (is_array($folder_list) && count($folder_list) > 0) {
            foreach ($folder_list as $folder) {
                ...
                if ($backup_prefix != 'TMP_' && $backup_prefix != 'DLT_') {
                    ...
                    array_push($backup_list, array("backup_name" => $backup_name, "log_exists" => $log_exists, "date_time" => filectime($folder), "zip_files" => $zip_files,));
    ...
    private function update_license_options($license) {
        ...
        if (empty($license) || 'lite' == $license) {
            ...
            $api_params = array('edd_action' => 'activate_license', 'license' => $license, 'item_name' => urlencode(WPBACKITUP__ITEM_NAME), // the name of product in EDD
                //'url'        => home_url()
            );
    ...
    public function set_option($option_name, $value) {
        ...
        if ($option_name == 'license_type') $this->license_type = $value;
    }
    ...
    private function update_stats($license) {
        ...
        if (is_wp_error($response)) return false; //Exit and don't update
