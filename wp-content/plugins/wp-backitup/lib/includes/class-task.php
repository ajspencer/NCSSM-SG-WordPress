...
class WPBackItUp_Task {
    ...
    public function __construct($job_id, $task_name, $task_info) {
        ...
        if (empty($job_id) || empty($task_name) || empty($task_info['task_id']) || empty($task_info['task_status'])) {
    ...
    private function save() {
        ...
        $meta_value = array('task_id' => $this->id, 'task_status' => $this->status, 'task_allocated_id' => $this->allocated_id, 'task_error_code' => $this->error_code, 'task_retry_count' => $this->retry_count, 'task_last_updated' => time());
