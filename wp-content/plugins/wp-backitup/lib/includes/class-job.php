...
class WPBackItUp_Job {
    ...
    static private $backup_tasks = array(1 => 'task_preparing', 2 => 'task_backup_db', 3 => 'task_backup_siteinfo', 4 => 'task_backup_themes', 5 => 'task_backup_plugins', 6 => 'task_backup_uploads', 7 => 'task_backup_other', 8 => 'task_validate_backup', 9 => 'task_finalize_backup',);

    static private $restore_tasks = array(1 => 'task_preparing', 2 => 'task_unzip_backup_set', 3 => 'task_validate_backup', 4 => 'task_create_checkpoint', 5 => 'task_stage_wpcontent', 6 => 'task_restore_wpcontent', 8 => 'task_restore_database',);

    static private $cleanup_tasks = array(1 => 'task_scheduled_cleanup');

    static private $cloud_upload_tasks = array(1 => 'task_validate_cloud_credentials', 2 => 'task_upload_files_to_cloud', 3 => 'task_update_backup_status');
    ...
    private function update_task_status($job_id, $task_name, $task_id, $task_status, $task_allocated_id = '', $task_error_code = '') {
        ...
        $meta_value = array('task_id' => $task_id, 'task_status' => $task_status, 'task_allocated_id' => $task_allocated_id, 'task_error_code' => $task_error_code, 'task_last_updated' => time());
    ...
    private function update_job_status($status) {
        ...
        $job = array('ID' => $this->job_id, 'post_status' => $status);
    ...
    private function set_job_start_time() {
        ...
        $job_info = array('start_time' => $this->job_start_time,);

        $job = array('ID' => $this->job_id, 'post_content' => serialize($job_info));
    ...
    private function set_job_end_time() {
        ...
        $job_info = array('start_time' => $this->job_start_time, 'end_time' => $this->job_end_time,);
    ...
    public static function is_job_queued($job_name) {
        ...
        //Get top 1
        $args = array('posts_per_page' => 1, 'post_type' => $job_name, 'post_status' => array('queued', 'active'), 'orderby' => 'post_date', 'order' => 'ASC', 'suppress_filters' => true);
        $jobs = get_posts($args);
        $logger->log($jobs);
    ...
    public static function get_completed_jobs($job_name, $count = 25) {
        ...
        $args = array('posts_per_page' => $count, 'post_type' => $job_name, 'post_status' => array('complete', 'cancelled', 'error'), 'orderby' => 'post_date', 'order' => 'ASC', 'suppress_filters' => true);
        $jobs = get_posts($args);
    ...
    public static function queue_job($job_name) {
        ...
        $new_job = array('post_title' => self::JOB_TITLE, 'post_name' => time(), 'post_status' => 'queued', 'post_type' => $job_name);
    ...
    private static function create_tasks($job_id, $tasks) {
        ...
        foreach ($tasks as $key => $value) {
            $task_name = $value;
            $task_data = array('task_id' => $task_name, 'task_status' => 'queued', 'task_allocated_id' => '', 'task_last_updated' => $last_updated_time);
