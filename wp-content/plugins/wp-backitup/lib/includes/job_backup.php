...
global $status_array;
$status_array = array('preparing' => $inactive, 'backupdb' => $inactive, 'infofile' => $inactive, 'backup_themes' => $inactive, 'backup_plugins' => $inactive, 'backup_uploads' => $inactive, 'backup_other' => $inactive, 'validate_backup' => $inactive, 'finalize_backup' => $inactive,);
...
if ('task_preparing' == $current_task) {
    ...
    $upload_exclude = array_merge($global_exclude, array("backup", "backwpup",));
    ...
    $other_exclude = array_merge($global_exclude, array("plugins", "themes", "uploads", "wpbackitup_backups", "wpbackitup_restore", "backup", "w3tc-config", "updraft", "wp-clone", "backwpup", "backupwordpress", "cache", "backupcreator", "backupbuddy", "wptouch-data",));
...
if ('task_validate_backup' == $current_task) {
    ...
    if ($validate_plugins === false || $validate_themes === false || $validate_uploads === false || $validate_others === false) {
        //ERROR
...
function send_backup_notification_email($err, $success, $logs = array()) {
    ...
    if ($notification_email) $utility->send_email($notification_email, $subject, $message, $logs);
