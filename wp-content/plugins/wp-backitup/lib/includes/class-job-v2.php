...
//	}
    ...
    public static function is_job_queued($job_name) {
        ...
        //Get top 1
        $args = array('posts_per_page' => 1, 'post_type' => $job_name, 'post_status' => array(self::QUEUED, self::ACTIVE), 'orderby' => 'post_date', 'order' => 'ASC', 'suppress_filters' => true);
        $jobs = get_posts($args);
        $logger->log($jobs);
    ...
    public static function get_completed_jobs($job_name) {
        ...
        $args = array('posts_per_page' => -1, 'post_type' => $job_name, 'post_status' => array(self::COMPLETE, self::CANCELLED, self::ERROR), 'orderby' => 'post_date', 'order' => 'DESC', 'suppress_filters' => true);
        $jobs = get_posts($args);
        $logger->log_info(__METHOD__, 'Jobs found:' . count($jobs));
    ...
    public static function queue_job($job_name, $tasks, $job_info = null) {
        ...
        $new_job = array('post_title' => self::JOB_TITLE, 'post_name' => time(), 'post_status' => self::QUEUED, 'post_type' => $job_name);
