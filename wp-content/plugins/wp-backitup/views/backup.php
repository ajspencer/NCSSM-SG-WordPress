...
if (!$backup_folder_exists) {
    echo('<div style="overflow: hidden;" class="error" id="wp-backitup-notification-parent" class="updated">
    ...
    echo('<div style="overflow: hidden; display:none" id="wp-backitup-notification-parent" class="updated">
...
<div class="wrap">
    ...
    <div id="content">
        ...
        <div class="widget">
            ...
            <p><b>Click the backup button to create a zipped backup file of this site's database, plugins, themes and
                    settings.</b></p>

            <p>Once your backup file has been created it will appear in the available backups section below. This file
                may remain on your hosting providers server but we recommend that you download and save it somewhere
                safe.</p>

            <p> WP BackItUp premium customers can use these backup files to perform an automated restore of their
                site.</p>

            <p>
                <?php if ($backup_folder_exists) : ?>
                <input type="submit" id="backup-button" class="backup-button button-primary"
                       value="<?php _e("Backup", $namespace) ?>"/><img class="backup-icon status-icon"
                                                                       src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                                       height="16" width="16"/></p>
            <?php endif; ?>
            <?php
            //Display a note for lite customers
            if (!$license_active) echo '<p> * WP BackItUp lite customers may use these backup files to manually restore their site.  Please visit ' . $this->get_anchor_with_utm('www.wpbackitup.com', 'documentation/restore/how-to-manually-restore-your-wordpress-database', 'backup', 'manual+restore') . ' for manual restore instructions.</p>';
            ?>
        </div>
        ...
        <div class="widget">
            ...
            <div id="scheduled-backups" style="display: none;">
                <p>Backup your site once per week or every day, it's up to you. If you have email notifications turned
                    on we'll even send you an email when it's done.
                    Once your backup file has been created it will appear in the available backups section below. This
                    file may remain on your hosting providers server but we recommend that you download and save it
                    somewhere safe.</p>
                ...
                <form action="admin-post.php" method="post" id="<?php echo $namespace; ?>-save_schedule_form">
                    ...
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '1')) ? _e('checked') : _e(''); ?>
                                                                  value="1">Monday<br>
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '2')) ? _e('checked') : _e(''); ?>
                                                                  value="2">Tuesday<br>
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '3')) ? _e('checked') : _e(''); ?>
                                                                  value="3">Wednesday<br>
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '4')) ? _e('checked') : _e(''); ?>
                                                                  value="4">Thursday<br>
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '5')) ? _e('checked') : _e(''); ?>
                                                                  value="5">Friday<br>
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '6')) ? _e('checked') : _e(''); ?>
                                                                  value="6">Saturday<br>
                    <input <?php _e($schedule_style_disabled); ?> type="checkbox"
                                                                  name="dow" <?php (false !== strpos($backup_schedule, '7')) ? _e('checked') : _e(''); ?>
                                                                  value="7">Sunday<br>

                    <br/>
                    <input <?php _e($schedule_style_disabled); ?> type="submit" id="schedule-button"
                                                                  class="schedule-button button-primary"
                                                                  value="<?php _e("Save Schedule", $namespace) ?>"/>
                </form>
                <?php
                //Display restore note for lite customers
                if (!$license_active || 'expired' == $license_status) echo '<p>* Scheduled backups are only available to WP BackItUp premium customers.  Please visit ' . $this->get_anchor_with_utm('www.wpbackitup.com', 'pricing-purchase', 'scheduled+backups', 'risk+free') . ' to get WP BackItUp risk free for 30 days.</p>';
                ?>
            </div>
        </div>
        ...
        <div class="widget">
                ...
                if ($backup_list != false) {
                    ...
                    foreach ($backup_list as $file) {
                        ...
                        <tr <?php echo $class ?> id="row<?php echo $i; ?>">
                            ...
                            <!--Download Link-->
                            <td>
                                <a href="#TB_inline?width=600&height=550&inlineId=<?php echo preg_replace('/[^A-Za-z0-9\-]/', '', $backup_name) ?>"
                                   class="thickbox" title="<?php echo $backup_name ?>">Download</a>
                            </td>
            ...
            <?php
            //Display restore note for lite customers
            if (!$license_active) echo '<p>* The automated restore feature is only available to WP BackItUp premium customers.  Please visit ' . $this->get_anchor_with_utm('www.wpbackitup.com', 'pricing-purchase', 'available+backups', 'risk+free') . ' to get WP BackItUp risk free for 30 days.</p>';
            ?>
        </div>
        ...
        <div id="status" class="widget">
            ...
            <ul class="backup-status">
                <li class="preparing"><?php _e('Preparing for backup', $namespace); ?>...<span class='status-icon'><img
                        ...
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='backupdb'><?php _e('Backing up database', $namespace); ?>...<span class='status-icon'><img
                            class="backupdb-icon" src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                            height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='infofile'><?php _e('Creating backup information file', $namespace); ?>...<span
                        class='status-icon'><img class="infofile-icon"
                                                 src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                 height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='backup_themes'><?php _e('Backing up themes', $namespace); ?>...<span class='status-icon'><img
                            class="backup_themes-icon"
                            src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>" height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='backup_plugins'><?php _e('Backing up plugins', $namespace); ?>...<span
                        class='status-icon'><img class="backup_plugins-icon"
                                                 src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                 height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='backup_uploads'><?php _e('Backing up uploads', $namespace); ?>...<span
                        class='status-icon'><img class="backup_uploads-icon"
                                                 src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                 height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='backup_other'><?php _e('Backing up everything else', $namespace); ?>...<span
                        class='status-icon'><img class="backup_other-icon"
                                                 src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                 height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='validate_backup'><?php _e('Validating backup', $namespace); ?>...<span
                        class='status-icon'><img class="validate_backup-icon"
                                                 src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                 height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
                        class='wpbackitup-warning'><?php _e('Warning', $namespace); ?></span></li>
                <li class='finalize_backup'><?php _e('Finalizing backup', $namespace); ?>...<span
                        class='status-icon'><img class="finalize_backup-icon"
                                                 src="<?php echo WPBACKITUP__PLUGIN_URL . "/images/loader.gif"; ?>"
                                                 height="16" width="16"/></span><span
                        class='status'><?php _e('Done', $namespace); ?></span><span
                        class='fail error'><?php _e('Failed', $namespace); ?></span><span
    ...
    <div id="sidebar">
        ...
        <div class="widget">
            <h3 class="promo"><?php _e('Backups', $namespace); ?> <span
                    style="float: right"><?php _e('Version ' . $version, $namespace); ?></span></h3>
            ...
            <?php if ($this->successful_backup_count() >= 1) : ?>
                <p>Congratulations! You have performed <span
                        style="font-weight:bold;font-size:medium;color: green"><?php _e($this->successful_backup_count(), $namespace); ?></span>
        ...
        <?php if (!$license_active) : ?>
            <?php if (!$is_lite_registered) : ?>
                ...
                <br />Enter your license key below to activate it on this site.', $namespace); ?></p>
                        <input type="text" name="license_name" id="license_name" placeholder="name"
                               value="<?php echo($license_customer_name) ?>"/><br/>
                        <input type="text" name="license_email" id="license_email" placeholder="email address"
                               value="<?php echo($license_customer_email) ?>"/><br/>
                        <input type="text" name="license_key" id="license_key" placeholder="license key"
                               value="<?php if ($license_key != 'lite') echo($license_key) ?>"/> &nbsp;<span
                            style="color:red"><?php _e($license_status_message, $namespace); ?></span>
                        <br/>* Free plugin customers do not need to enter license key.
                        <div class="submit"><input type="submit" name="Submit" class="button-secondary"
                                                   value="<?php _e("Register", $namespace) ?>"/></div>
                    </form>
                </div>
            <?php endif ?>
        <?php endif; ?>
        ...
        <?php if ($license_active || $is_lite_registered) : ?>
            ...
            <div class="widget">
                <h3 class="promo"><span><?php _e('License Info', $namespace); ?></span><span style="float: right">
                </h3></h3>
                <form action="" method="post" id="<?php echo $namespace; ?>-form">
                    ...
                    $fontColor = 'green';
                    if ($license_status == 'valid') $fontColor = 'green';

                    if ($license_status == 'invalid') $fontColor = 'red';

                    if ($license_status == 'expired') $fontColor = 'orange';
                    ...
                    <input type="text" name="data[license_key]" id="license_key"
                           value="<?php _e($license_key, $namespace); ?>"/>&nbsp;
                    ...
                    <?php if ($license_status == 'invalid'): ?>
                        <span
                            style="color:<?php _e($fontColor); ?>"><?php _e($license_status_message, $namespace); ?></span>
                    <?php endif; ?>

                    <?php if ($license_status == 'expired'): ?>
                        <span
                            style="color:red">License expired:&nbsp;<?php _e($formatted_expired_date, $namespace); ?></span>
                    <?php endif; ?>

                    <?php if ($license_active) : ?>
                        <div class="submit"><input type="submit" name="Submit" class="button-secondary"
                                                   value="<?php _e("Update", $namespace) ?>"/></div>
                    <?php endif; ?>

                    <?php if (!$license_active) : ?>
                        <p class="submit"><input type="submit" name="Submit" class="button-secondary"
                                                 value="<?php _e("Activate", $namespace) ?>"/></p>
                    <?php endif; ?>

                    <?php if ($license_status == 'invalid' || $license_status == ''): ?>
                        <p>Purchase
                            a <?php echo($this->get_anchor_with_utm('no-risk', 'pricing-purchase', 'license', 'no+risk')) ?>
                            license using the purchase link above.</p>
                    <?php endif; ?>

                    <?php if ($license_status == 'expired'): ?>
                        <div><?php echo('<a target="_blank" href="' . WPBACKITUP__SECURESITE_URL . '/checkout?edd_license_key=' . $license_key . '&download_id=679&nocache=true&utm_medium=plugin&utm_source=wp-backitup&utm_campaign=premium&utm_content=license&utm_term=license+expired">Renew</a>');
                            ?>&nbsp; your license now for another year of <strong>product updates</strong> and <strong>priority
                                support.</strong></div>
                    <?php endif; ?>
            ...
            <table id="datatable" class="widefat">
                ...
                <?php foreach ($zip_files as $file) :
                    ...
                    <tr id="<?php echo $row_id ?>" class="<?php echo $class ?>">
                        <td><a href="<?php echo $zip_file ?>" class="downloadbackuplink"><?php echo $zip_file ?></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
