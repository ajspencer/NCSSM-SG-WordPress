...
class Akismet {
    ...
    private static function init_hooks() {
        ...
        if ($akismet_comment_nonce_option == 'true' || $akismet_comment_nonce_option == '') add_action('comment_form', array('Akismet', 'add_comment_nonce'), 1);
        ...
        if ('3.0.5' == $GLOBALS['wp_version']) {
            ...
            if (is_admin()) add_filter('comment_text', 'wp_kses_post');
        }
    }
    ...
    public static function verify_key($key, $ip = null) {
        ...
        if ($response[1] != 'valid' && $response[1] != 'invalid') return 'failed';
    ...
    public static function auto_check_comment($commentdata) {
        ...
        if (!empty($comment['user_ID'])) $comment['user_role'] = Akismet::get_user_roles($comment['user_ID']);

        $akismet_nonce_option = apply_filters('akismet_comment_nonce', get_option('akismet_comment_nonce'));
        $comment['akismet_comment_nonce'] = 'inactive';
        if ($akismet_nonce_option == 'true' || $akismet_nonce_option == '') {
            $comment['akismet_comment_nonce'] = 'failed';
            if (isset($_POST['akismet_comment_nonce']) && wp_verify_nonce($_POST['akismet_comment_nonce'], 'akismet_comment_nonce_' . $comment['comment_post_ID'])) $comment['akismet_comment_nonce'] = 'passed';

            // comment reply in wp-admin
            if (isset($_POST['_ajax_nonce-replyto-comment']) && check_ajax_referer('replyto-comment', '_ajax_nonce-replyto-comment')) $comment['akismet_comment_nonce'] = 'passed';

        }

        if (self::is_test_mode()) $comment['is_test'] = 'true';

        foreach ($_POST as $key => $value) {
            if (is_string($value)) $comment["POST_{$key}"] = $value;
        }

        $ignore = array('HTTP_COOKIE', 'HTTP_COOKIE2', 'PHP_AUTH_PW');

        foreach ($_SERVER as $key => $value) {
            if (!in_array($key, $ignore) && is_string($value)) $comment["$key"] = $value; else
                $comment["$key"] = '';
        }
        ...
        if (isset($response[0]['x-akismet-pro-tip'])) $commentdata['akismet_pro_tip'] = $response[0]['x-akismet-pro-tip'];
        ...
        if (isset($response[0]['x-akismet-error'])) {
            ...
            if ($discard) {
                ...
                if ($incr = apply_filters('akismet_spam_count_incr', 1)) update_option('akismet_spam_count', get_option('akismet_spam_count') + $incr);
        ...
        if (function_exists('wp_next_scheduled') && function_exists('wp_schedule_event')) {
            ...
            if (!wp_next_scheduled('akismet_scheduled_delete')) wp_schedule_event(time(), 'daily', 'akismet_scheduled_delete');
    ...
    public static function auto_check_update_meta($id, $comment) {
        ...
        if (!function_exists('add_comment_meta')) return false;

        if (!isset(self::$last_comment['comment_author_email'])) self::$last_comment['comment_author_email'] = '';
        ...
        if (is_object($comment) && !empty(self::$last_comment) && is_array(self::$last_comment)) {
            if (self::matches_last_comment($comment)) {
                ...
                if (self::$last_comment['akismet_result'] == 'true') {
                    ...
                    if ($comment->comment_approved != 'spam') self::update_comment_history($comment->comment_ID, sprintf(__('Comment status was changed to %s', 'akismet'), $comment->comment_approved), 'status-changed' . $comment->comment_approved);
                    ...
                    if ($comment->comment_approved == 'spam') {
                        if (wp_blacklist_check($comment->comment_author, $comment->comment_author_email, $comment->comment_author_url, $comment->comment_content, $comment->comment_author_IP, $comment->comment_agent)) self::update_comment_history($comment->comment_ID, __('Comment was caught by wp_blacklist_check', 'akismet'), 'wp-blacklisted'); else
                ...
                if (isset(self::$last_comment['comment_as_submitted'])) update_comment_meta($comment->comment_ID, 'akismet_as_submitted', self::$last_comment['comment_as_submitted']);

                if (isset(self::$last_comment['akismet_pro_tip'])) update_comment_meta($comment->comment_ID, 'akismet_pro_tip', self::$last_comment['akismet_pro_tip']);
            }
        }
    }
    ...
    public static function get_user_comments_approved($user_id, $comment_author_email, $comment_author, $comment_author_url) {
        ...
        if (!empty($user_id)) return (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->comments} WHERE user_id = %d AND comment_approved = 1", $user_id));

        if (!empty($comment_author_email)) return (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->comments} WHERE comment_author_email = %s AND comment_author = %s AND comment_author_url = %s AND comment_approved = 1", $comment_author_email, $comment_author, $comment_author_url));
    ...
    public static function update_comment_history($comment_id, $message, $event = null) {
        ...
        $user = '';
        if (is_object($current_user) && isset($current_user->user_login)) $user = $current_user->user_login;

        $event = array('time' => self::_get_microtime(), 'message' => $message, 'event' => $event, 'user' => $user,);
    ...
    public static function check_db_comment($id, $recheck_reason = 'recheck_queue') {
        ...
        if (self::is_test_mode()) $c['is_test'] = 'true';
    ...
    public static function transition_comment_status($new_status, $old_status, $comment) {
        ...
        if ($new_status == 'delete') return;

        if (!current_user_can('edit_post', $comment->comment_post_ID) && !current_user_can('moderate_comments')) return;

        if (defined('WP_IMPORTING') && WP_IMPORTING == true) return;

        // if this is present, it means the status has been changed by a re-check, not an explicit user action
        if (get_comment_meta($comment->comment_ID, 'akismet_rechecking')) return;

        global $current_user;
        $reporter = '';
        if (is_object($current_user)) $reporter = $current_user->user_login;
        ...
        if ((isset($_POST['status']) && in_array($_POST['status'], array('spam', 'unspam'))) || (isset($_POST['spam']) && (int)$_POST['spam'] == 1) || (isset($_POST['unspam']) && (int)$_POST['unspam'] == 1) || (isset($_POST['comment_status']) && in_array($_POST['comment_status'], array('spam', 'unspam'))) || (isset($_GET['action']) && in_array($_GET['action'], array('spam', 'unspam'))) || (isset($_POST['action']) && in_array($_POST['action'], array('editedcomment')))) {
    ...
    public static function submit_spam_comment($comment_id) {
        ...
        if ($as_submitted && is_array($as_submitted) && isset($as_submitted['comment_content'])) $comment = (object)array_merge((array)$comment, $as_submitted);
        ...
        if (is_object($current_user)) $comment->reporter = $current_user->user_login;

        if (is_object($current_site)) $comment->site_domain = $current_site->domain;

        $comment->user_role = '';
        if (isset($comment->user_ID)) $comment->user_role = Akismet::get_user_roles($comment->user_ID);
    ...
    public static function submit_nonspam_comment($comment_id) {
        ...
        if (Akismet::is_test_mode()) $comment->is_test = 'true';
            ...
            if (!empty($msg)) {
                ...
                if ($comment && 'unapproved' == wp_get_comment_status($comment_id)) {
                    if ($status == 'true') {
                        ...
                        if (check_comment($comment->comment_author, $comment->comment_author_email, $comment->comment_author_url, $comment->comment_content, $comment->comment_author_IP, $comment->comment_agent, $comment->comment_type)) wp_set_comment_status($comment_id, 1); else if (get_comment_meta($comment_id, 'akismet_delayed_moderation_email', true)) wp_notify_moderator($comment_id);
                    }
                }
    ...
    public static function matches_last_comment($comment) {
        if (is_object($comment)) $comment = (array)$comment;
    ...
    public static function get_user_roles($user_id) {
        ...
        if (!class_exists('WP_User')) return false;

        if ($user_id > 0) {
            $comment_user = new WP_User($user_id);
            if (isset($comment_user->roles)) $roles = join(',', $comment_user->roles);
        }
    ...
    public static function pre_check_pingback($method) {
        ...
        if (!is_object($wp_xmlrpc_server)) return false;
