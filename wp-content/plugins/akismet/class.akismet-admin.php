...
class Akismet_Admin {
    ...
    public static function admin_menu() {
        if (class_exists('Jetpack')) add_action('jetpack_admin_menu', array('Akismet_Admin', 'load_menu')); else
            self::load_menu();
    }
    ...
    public static function load_menu() {
        if (class_exists('Jetpack')) $hook = add_submenu_page('jetpack', __('Akismet', 'akismet'), __('Akismet', 'akismet'), 'manage_options', 'akismet-key-config', array('Akismet_Admin', 'display_page')); else
    ...
    public static function load_resources() {
        ...
        if (in_array($hook_suffix, array('index.php', # dashboard
            'edit-comments.php', 'comment.php', 'post.php', 'settings_page_akismet-key-config', 'jetpack_page_akismet-key-config',))) {
            ...
            wp_localize_script('akismet.js', 'WPAkismet', array('comment_author_url_nonce' => wp_create_nonce('comment_author_url_nonce'), 'strings' => array('Remove this URL' => __('Remove this URL', 'akismet'), 'Removing...' => __('Removing...', 'akismet'), 'URL removed' => __('URL removed', 'akismet'), '(undo)' => __('(undo)', 'akismet'), 'Re-adding...' => __('Re-adding...', 'akismet'),)));
        }
    }
    ...
    public static function admin_help() {
        ...
        if (current_user_can('manage_options')) {
            if (!Akismet::get_api_key() || (isset($_GET['view']) && $_GET['view'] == 'start')) {
                ...
                //stats page
                $current_screen->add_help_tab(array('id' => 'overview', 'title' => __('Overview', 'akismet'), 'content' => '<p><strong>' . esc_html__('Akismet Stats', 'akismet') . '</strong></p>' . '<p>' . esc_html__('Akismet filters out your comment and trackback spam for you, so you can focus on more important things.', 'akismet') . '</p>' . '<p>' . esc_html__('On this page, you are able to view stats on spam filtered on your site.', 'akismet') . '</p>',));
            } else {
                //configuration page
    ...
    public static function enter_api_key() {
        if (function_exists('current_user_can') && !current_user_can('manage_options')) die(__('Cheatin&#8217; uh?', 'akismet'));

        if (!wp_verify_nonce($_POST['_wpnonce'], self::NONCE)) return false;
        ...
        if (defined('WPCOM_API_KEY')) return false; //shouldn't have option to save key if already defined
    ...
    public static function save_key($api_key) {
        ...
        if ($key_status == 'valid') {
            ...
            if ($akismet_user) {
                if (in_array($akismet_user->status, array('active', 'active-dunning', 'no-sub'))) update_option('wordpress_api_key', $api_key);

                if ($akismet_user->status == 'active') self::$notices['status'] = 'new-key-valid'; else
                    self::$notices['status'] = $akismet_user->status;
            } else
                self::$notices['status'] = 'new-key-invalid';
        } elseif (in_array($key_status, array('invalid', 'failed'))) self::$notices['status'] = 'new-key-' . $key_status;
    }

    public static function dashboard_stats() {
        if (!function_exists('did_action') || did_action('rightnow_end')) return; // We already displayed this info in the "Right Now" section

        if (!$count = get_option('akismet_spam_count')) return;
    ...
    public static function rightnow_stats() {
        ...
        if (8645 < $wp_db_version) // 2.7
            $link = add_query_arg(array('comment_status' => 'spam'), admin_url('edit-comments.php')); elseif (isset($submenu['edit-comments.php'])) $link = add_query_arg(array('page' => 'akismet-admin'), admin_url('edit-comments.php'));
        else
    ...
    public static function check_for_spam_button($comment_status) {
        ...
        if (function_exists('plugins_url')) $link = add_query_arg(array('action' => 'akismet_recheck_queue'), admin_url('admin.php')); else
    ...
    public static function recheck_queue() {
        ...
        if (!(isset($_GET['recheckqueue']) || (isset($_REQUEST['action']) && 'akismet_recheck_queue' == $_REQUEST['action']))) return;
        ...
        foreach ((array)$moderation as $c) {
            ...
            $c['user_role'] = '';
            if (isset($c['user_ID'])) $c['user_role'] = Akismet::get_user_roles($c['user_ID']);

            if (Akismet::is_test_mode()) $c['is_test'] = 'true';
    ...
    public static function comment_row_action($a, $comment) {
        ...
        if (!function_exists('add_comment_meta')) return $a;
        ...
        if ($akismet_error) {
            ...
            if ($akismet_result == 'true' && $comment_status != 'spam' && $comment_status != 'trash') $desc = __('Flagged as spam by Akismet', 'akismet'); elseif ($akismet_result == 'false' && $comment_status == 'spam') $desc = __('Cleared by Akismet', 'akismet');
        } else {
            $who = get_comment_meta($comment->comment_ID, 'akismet_user', true);
            if ($user_result == 'true') $desc = sprintf(__('Flagged as spam by %s', 'akismet'), $who); else
        ...
        if ($desc) echo '<span class="akismet-status" commentid="' . $comment->comment_ID . '"><a href="comment.php?action=editcomment&amp;c=' . $comment->comment_ID . '#akismet-status" title="' . esc_attr__('View comment history', 'akismet') . '">' . esc_html($desc) . '</a></span>';
    ...
    public static function text_add_link_callback($m) {
        // bare link?
        if ($m[4] == $m[2]) return '<a ' . $m[1] . ' href="' . $m[2] . '" ' . $m[3] . ' class="comment-link">' . $m[4] . '</a>'; else
        ...
        if (function_exists('gethostbynamel')) {
            ...
            if ($ips && is_array($ips) && count($ips)) {
                ...
                foreach ($ips as $ip) {
                    ...
                    if ($response == 'valid' || $response == 'invalid') $servers[$ip] = 'connected'; else
    ...
    public static function check_server_connectivity($cache_timeout = 86400) {
        ...
        if ($response && 'connected' == wp_remote_retrieve_body($response)) return true;
    ...
    public static function get_page_url($page = 'config') {
        ...
        if ($page == 'stats') $args = array('page' => 'akismet-key-config', 'view' => 'stats'); elseif ($page == 'delete_key') $args = array('page' => 'akismet-key-config', 'view' => 'start', 'action' => 'delete-key', '_wpnonce' => wp_create_nonce(self::NONCE));
    ...
    public static function get_akismet_user($api_key) {
        ...
        if (!empty($akismet_user[1])) $akismet_user = json_decode($akismet_user[1]); else
            $akismet_user = false;
    ...
    public static function verify_wpcom_key($api_key, $user_id, $extra = array()) {
        ...
        if (!empty($akismet_account[1])) $akismet_account = json_decode($akismet_account[1]);
    ...
    public static function display_page() {
        if (!Akismet::get_api_key() || (isset($_GET['view']) && $_GET['view'] == 'start')) self::display_start_page(); elseif (isset($_GET['view']) && $_GET['view'] == 'stats') self::display_stats_page();
        else
    ...
    public static function display_start_page() {
        if (isset($_GET['action'])) {
            if ($_GET['action'] == 'delete-key') {
                if (isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], self::NONCE)) delete_option('wordpress_api_key');
            }
        }
        ...
        if (isset($_GET['token']) && preg_match('/^(\d+)-[0-9a-f]{20}$/', $_GET['token'])) $akismet_user = self::verify_wpcom_key('', '', array('token' => $_GET['token'])); elseif ($jetpack_user = self::get_jetpack_user()) $akismet_user = self::verify_wpcom_key($jetpack_user['api_key'], $jetpack_user['user_id']);
    ...
    public static function display_configuration_page() {
        ...
        if (get_option('akismet_strictness') === false) add_option('akismet_strictness', (get_option('akismet_discard_month') === 'true' ? '1' : '0'));
        ...
        if (empty(self::$notices)) {
            ...
            if (!empty($stat_totals['all']) && isset($stat_totals['all']->time_saved) && $akismet_user->status == 'active' && $akismet_user->account_type == 'free-api-key') {
                ...
                if ($stat_totals['all']->time_saved > 1800) {
                    ...
                    if ($total_in_days > 1) $time_saved = $cleaning_up . ' ' . sprintf(__('Since you joined us, Akismet has saved you %s days!', 'akismet'), number_format_i18n($total_in_days)); elseif ($total_in_hours > 1) $time_saved = $cleaning_up . ' ' . sprintf(__('Since you joined us, Akismet has saved you %d hours!', 'akismet'), $total_in_hours);
                    elseif ($total_in_minutes >= 30) $time_saved = $cleaning_up . ' ' . sprintf(__('Since you joined us, Akismet has saved you %d minutes!', 'akismet'), $total_in_minutes);
                }
        ...
        if (!isset(self::$notices['status']) && in_array($akismet_user->status, array('cancelled', 'suspended', 'missing', 'no-sub'))) Akismet::view('notice', array('type' => $akismet_user->status));
    ...
    public static function display_notice() {
        ...
        if (in_array($hook_suffix, array('jetpack_page_akismet-key-config', 'settings_page_akismet-key-config', 'edit-comments.php')) && (int)get_option('akismet_alert_code') > 0) {
            ...
            if (get_option('akismet_alert_code') > 0) self::display_alert();
    ...
    public static function display_status() {
        ...
        if (!self::get_server_connectivity()) $type = 'servers-be-down';

        if (!empty($type)) Akismet::view('notice', compact('type')); elseif (!empty(self::$notices)) {
            foreach (self::$notices as $type) Akismet::view('notice', compact('type'));
        }
    }
