...
class WP_List_Table {
    ...
    public function __construct($args = array()) {
        ...
        if (!$args['plural']) $args['plural'] = $this->screen->base;
    ...
    protected function set_pagination_args($args) {
        ...
        if (!$args['total_pages'] && $args['per_page'] > 0) $args['total_pages'] = ceil($args['total_items'] / $args['per_page']);
    ...
    public function get_pagination_arg($key) {
        ...
        if (isset($this->_pagination_args[$key])) return $this->_pagination_args[$key];
    }
    ...
    public function search_box($text, $input_id) {
        ...
        if (!empty($_REQUEST['orderby'])) echo '<input type="hidden" name="orderby" value="' . esc_attr($_REQUEST['orderby']) . '" />';
        if (!empty($_REQUEST['order'])) echo '<input type="hidden" name="order" value="' . esc_attr($_REQUEST['order']) . '" />';
        if (!empty($_REQUEST['post_mime_type'])) echo '<input type="hidden" name="post_mime_type" value="' . esc_attr($_REQUEST['post_mime_type']) . '" />';
        if (!empty($_REQUEST['detached'])) echo '<input type="hidden" name="detached" value="' . esc_attr($_REQUEST['detached']) . '" />';
        ?>
        <p class="search-box">
    ...
    public function current_action() {
        if (isset($_REQUEST['filter_action']) && !empty($_REQUEST['filter_action'])) return false;

        if (isset($_REQUEST['action']) && -1 != $_REQUEST['action']) return $_REQUEST['action'];

        if (isset($_REQUEST['action2']) && -1 != $_REQUEST['action2']) return $_REQUEST['action2'];
    ...
    protected function view_switcher($current_mode) {
        ...
        <div class="view-switch">
            ...
            foreach ($this->modes as $mode => $title) {
                ...
                if ($current_mode == $mode) $classes[] = 'current';
    ...
    public function get_pagenum() {
        ...
        if (isset($this->_pagination_args['total_pages']) && $pagenum > $this->_pagination_args['total_pages']) $pagenum = $this->_pagination_args['total_pages'];
    ...
    protected function get_items_per_page($option, $default = 20) {
        ...
        if (empty($per_page) || $per_page < 1) $per_page = $default;
    ...
    public function print_column_headers($with_id = true) {
        ...
        if (isset($_GET['orderby'])) $current_orderby = $_GET['orderby']; else
            $current_orderby = '';

        if (isset($_GET['order']) && 'desc' == $_GET['order']) $current_order = 'desc'; else
            $current_order = 'asc';
        ...
        foreach ($columns as $column_key => $column_display_name) {
            ...
            $style = '';
            if (in_array($column_key, $hidden)) $style = 'display:none;';

            $style = ' style="' . $style . '"';

            if ('cb' == $column_key) $class[] = 'check-column'; elseif (in_array($column_key, array('posts', 'comments', 'links'))) $class[] = 'num';
    ...
    protected function display_tablenav($which) {
        if ('top' == $which) wp_nonce_field('bulk-' . $this->_args['plural']);
        ?>
