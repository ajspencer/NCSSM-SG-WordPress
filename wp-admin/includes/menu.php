...
foreach ($menu as $menu_page) {
    if (false !== $pos = strpos($menu_page[2], '?')) {
        ...
        if (isset($hook_args['post_type'])) $hook_name = $hook_args['post_type']; else
    ...
    if (isset($compat[$hook_name])) $hook_name = $compat[$hook_name]; elseif (!$hook_name) continue;
...
foreach ($submenu as $parent => $sub) {
    ...
    if (empty($submenu[$parent])) unset($submenu[$parent]);
}
unset($sub, $parent);
...
foreach ($menu as $id => $data) {
    ...
    if ($new_parent != $old_parent) {
        ...
        if (isset($_wp_submenu_nopriv[$old_parent])) $_wp_submenu_nopriv[$new_parent] = $_wp_submenu_nopriv[$old_parent];
    }
}
 ...
 */
foreach ($menu as $id => $data) {
    if (!current_user_can($data[1])) $_wp_menu_nopriv[$data[2]] = true;
...
if (apply_filters('custom_menu_order', false)) {
    ...
    function sort_menu($a, $b) {
        ...
        if (isset($menu_order[$a]) && !isset($menu_order[$b])) {
            ...
            if ($menu_order[$a] == $menu_order[$b]) return 0;
...
if (!empty($menu) && 'wp-menu-separator' == $menu[$last_menu_key][4]) unset($menu[$last_menu_key]);
unset($last_menu_key);
