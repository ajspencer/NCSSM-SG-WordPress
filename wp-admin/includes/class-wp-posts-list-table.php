...
class WP_Posts_List_Table extends WP_List_Table {
    ...
    public function __construct($args = array()) {
        ...
        if (!current_user_can($post_type_object->cap->edit_others_posts)) {
            ...
            if ($this->user_posts_count && empty($_REQUEST['post_status']) && empty($_REQUEST['all_posts']) && empty($_REQUEST['author']) && empty($_REQUEST['show_sticky'])) $_GET['author'] = get_current_user_id();
        }
    ...
    public function prepare_items() {
        ...
        if ($this->hierarchical_display) $total_pages = ceil($total_items / $per_page); else
    ...
    public function no_items() {
        if (isset($_REQUEST['post_status']) && 'trash' == $_REQUEST['post_status']) echo get_post_type_object($this->screen->post_type)->labels->not_found_in_trash; else
    ...
    protected function get_views() {
        ...
        foreach (get_post_stati(array('show_in_admin_all_list' => false)) as $state) $total_posts -= $num_posts->$state;
    ...
    protected function pagination($which) {
        ...
        if ('top' == $which && !is_post_type_hierarchical($this->screen->post_type)) $this->view_switcher($mode);
    }
    ...
    public function get_columns() {
        ...
        if (post_type_supports($post_type, 'comments') && !in_array($post_status, array('pending', 'draft', 'future'))) $posts_columns['comments'] = '<span class="vers comment-grey-bubble" title="' . esc_attr__('Comments') . '"><span class="screen-reader-text">' . __('Comments') . '</span></span>';
    ...
    private function _display_rows($posts, $level = 0) {
        ...
        foreach ($posts as $post) $this->single_row($post, $level);
    }
    ...
    private function _display_rows_hierarchical($pages, $pagenum = 1, $per_page = 20) {
        ...
        if (empty($_REQUEST['s'])) {
            ...
            foreach ($pages as $page) {
                ...
                if (0 == $page->post_parent) $top_level_pages[] = $page; else
        ...
        foreach ($pages as $page) {
            ...
            if (isset($children_pages)) $this->_page_rows($children_pages, $count, $page->ID, $level + 1, $pagenum, $per_page, $to_display);
        }
            ...
            foreach ($columns as $column_name => $column_display_name) {
                ...
                switch ($column_name) {
                    ...
                    case 'title':
                        ...
                        if (isset($parent_name)) echo ' | ' . $post_type_object->labels->parent_item_colon . ' ' . esc_html($parent_name);
                        ...
                        if (current_user_can('delete_post', $post->ID)) {
                            if ('trash' == $post->post_status) $actions['untrash'] = "<a title='" . esc_attr__('Restore this item from the Trash') . "' href='" . wp_nonce_url(admin_url(sprintf($post_type_object->_edit_link . '&amp;action=untrash', $post->ID)), 'untrash-post_' . $post->ID) . "'>" . __('Restore') . "</a>"; elseif (EMPTY_TRASH_DAYS) $actions['trash'] = "<a class='submitdelete' title='" . esc_attr__('Move this item to the Trash') . "' href='" . get_delete_post_link($post->ID) . "'>" . __('Trash') . "</a>";
                            if ('trash' == $post->post_status || !EMPTY_TRASH_DAYS) $actions['delete'] = "<a class='submitdelete' title='" . esc_attr__('Delete this item permanently') . "' href='" . get_delete_post_link($post->ID, '', true) . "'>" . __('Delete Permanently') . "</a>";
                        }
                    ...
                    case 'date':
                        if ('0000-00-00 00:00:00' == $post->post_date) {
                            ...
                            if ($time_diff > 0 && $time_diff < DAY_IN_SECONDS) $h_time = sprintf(__('%s ago'), human_time_diff($time)); else
                        ...
                        if ('publish' == $post->post_status) {
                            ...
                            if ($time_diff > 0) echo '<strong class="attention">' . __('Missed schedule') . '</strong>'; else
                                _e('Scheduled');
                        } else {
                            _e('Last Modified');
                        }
    ...
    public function inline_edit() {
        ...
        foreach ($taxonomy_names as $taxonomy_name) {
            ...
            if ($taxonomy->hierarchical) $hierarchical_taxonomies[] = $taxonomy; else
                ...
                while ($bulk < 2) { ?>
                    ...
                    <tr id="<?php echo $bulk ? 'bulk-edit' : 'inline-edit'; ?>"
                        ...
                        <td colspan="<?php echo $this->get_column_count(); ?>" class="colspanchange">
                                ...
                                <div class="inline-edit-col">
                                    ...
                                    if (post_type_supports($screen->post_type, 'author')) :
                                        ...
                                        if (is_super_admin() || current_user_can($post_type_object->cap->edit_others_posts)) :
                                            ...
                                            if ($bulk) $users_opt['show_option_none'] = __('&mdash; No Change &mdash;');
                                    ...
                                    if (post_type_supports($screen->post_type, 'page-attributes')) :
                                        ...
                                        if ($post_type_object->hierarchical) :
                                            ...
                                            <label>
                                                ...
                                                if ($bulk) $dropdown_args['show_option_no_change'] = __('&mdash; No Change &mdash;');
                            ...
                            foreach ($columns as $column_name => $column_display_name) {
                                if (isset($core_columns[$column_name])) continue;
