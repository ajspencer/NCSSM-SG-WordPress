...
function edit_comment() {
    ...
    if (!current_user_can('edit_comment', (int)$_POST['comment_ID'])) wp_die(__('You are not allowed to edit comments on this post.'));

    if (isset($_POST['newcomment_author'])) $_POST['comment_author'] = $_POST['newcomment_author'];
    if (isset($_POST['newcomment_author_email'])) $_POST['comment_author_email'] = $_POST['newcomment_author_email'];
    if (isset($_POST['newcomment_author_url'])) $_POST['comment_author_url'] = $_POST['newcomment_author_url'];
...
function get_pending_comments_num($post_id) {
    ...
    if ($single) {
        if (empty($pending)) return 0; else
    ...
    if (!empty($pending)) foreach ($pending as $pend) $pending_keyed[$pend['comment_post_ID']] = absint($pend['num_comments']);
