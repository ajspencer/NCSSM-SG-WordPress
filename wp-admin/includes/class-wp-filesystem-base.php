...
class WP_Filesystem_Base {
    ...
    public function abspath() {
        ...
        if (!$folder && $this->is_dir('/' . WPINC)) $folder = '/';
        return $folder;
    }
    ...
    public function wp_themes_dir($theme = false) {
        ...
        if ('/themes' == $theme_root || !is_dir($theme_root)) $theme_root = WP_CONTENT_DIR . $theme_root;
    ...
    public function find_folder($folder) {
        ...
        if (isset($this->cache[$folder])) return $this->cache[$folder];
        ...
        if (stripos($this->method, 'ftp') !== false) {
            ...
            foreach ($constant_overrides as $constant => $dir) {
                ...
                if ($folder === $dir) return trailingslashit(constant($constant));
            }
        ...
        if ($return = $this->search_for_folder($folder)) $this->cache[$folder] = $return;
        return $return;
    }
    ...
    public function search_for_folder($folder, $base = '.', $loop = false) {
        if (empty($base) || '.' == $base) $base = trailingslashit($this->cwd());

        $folder = untrailingslashit($folder);

        if ($this->verbose) printf("\n" . __('Looking for %1$s in %2$s') . "<br/>\n", $folder, $base);
        ...
        foreach ($folder_parts as $index => $key) {
            if ($index == $last_index) continue; // We want this to be caught by the next code block.
            ...
            if (isset($files[$key])) {
                ...
                if ($ret = $this->search_for_folder($newfolder, $newdir, $loop)) return $ret;
            }
        }
    ...
    public function gethchmod($file) {
        ...
        if (($perms & 0xC000) == 0xC000) // Socket
            $info = 's'; elseif (($perms & 0xA000) == 0xA000) // Symbolic Link
            $info = 'l';
