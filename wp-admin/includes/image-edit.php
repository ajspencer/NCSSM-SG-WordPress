...
function wp_image_editor($post_id, $msg = false) {
    ...
    if (isset($meta['width'], $meta['height'])) $big = max($meta['width'], $meta['height']); else
    ...
    $can_restore = false;
    if (!empty($backup_sizes) && isset($backup_sizes['full-orig'], $meta['file'])) $can_restore = $backup_sizes['full-orig']['file'] != basename($meta['file']);

    if ($msg) {
        if (isset($msg->error)) $note = "<div class='error'><p>$msg->error</p></div>"; elseif (isset($msg->msg)) $note = "<div class='updated'><p>$msg->msg</p></div>";
    }
...
function wp_stream_image($image, $mime_type, $post_id) {
    if ($image instanceof WP_Image_Editor) {
        ...
        if (is_wp_error($image->stream($mime_type))) return false;
...
function image_edit_apply_changes($image, $changes) {
    if (is_resource($image)) _deprecated_argument(__FUNCTION__, '3.5', __('$image needs to be an WP_Image_Editor object'));
    ...
    if (count($changes) > 1) {
        ...
        for ($i = 0, $j = 1, $c = count($changes); $j < $c; $j++) {
            ...
            if (!$combined) $filtered[++$i] = $changes[$j];
        }
        $changes = $filtered;
        unset($filtered);
    }
        ...
        switch ($operation->type) {
            case 'rotate':
                if ($operation->angle != 0) {
                    if ($image instanceof WP_Image_Editor) $image->rotate($operation->angle); else
                        $image = _rotate_image_resource($image, $operation->angle);
                }
                break;
            case 'flip':
                if ($operation->axis != 0) if ($image instanceof WP_Image_Editor) $image->flip(($operation->axis & 1) != 0, ($operation->axis & 2) != 0); else
...
function wp_restore_image($post_id) {
    ...
    if (!$restored) $msg->error = __('Image metadata is inconsistent.'); else
...
function wp_save_image($post_id) {
    ...
    if ($scale && $fwidth > 0 && $fheight > 0) {
        ...
        if (-0.1 < $diff && $diff < 0.1) {
            ...
            if ($img->resize($fwidth, $fheight)) $scaled = true;
        }
    ...
    if (defined('IMAGE_EDIT_OVERWRITE') && IMAGE_EDIT_OVERWRITE && isset($backup_sizes['full-orig']) && $backup_sizes['full-orig']['file'] != $path_parts['basename']) {
        ...
        if ('thumbnail' == $target) $new_path = "{$path_parts['dirname']}/{$filename}-temp.{$path_parts['extension']}"; else
            $new_path = $path;
    } else {
        while (true) {
            ...
            if (file_exists($new_path)) $suffix++; else
                break;
        }
    }
    ...
    if ('nothumb' == $target || 'all' == $target || 'full' == $target || $scaled) {
        ...
        if (isset($backup_sizes['full-orig'])) {
            if ((!defined('IMAGE_EDIT_OVERWRITE') || !IMAGE_EDIT_OVERWRITE) && $backup_sizes['full-orig']['file'] != $path_parts['basename']) $tag = "full-$suffix";
        } else {
            $tag = 'full-orig';
        }

        if ($tag) $backup_sizes[$tag] = array('width' => $meta['width'], 'height' => $meta['height'], 'file' => $path_parts['basename']);
        ...
        foreach ($sizes as $size) {
            ...
            if (isset($meta['sizes'][$size])) {
                if (isset($backup_sizes["$size-orig"])) {
                    if ((!defined('IMAGE_EDIT_OVERWRITE') || !IMAGE_EDIT_OVERWRITE) && $backup_sizes["$size-orig"]['file'] != $meta['sizes'][$size]['file']) $tag = "$size-$suffix";
                } else {
                    $tag = "$size-orig";
                }
