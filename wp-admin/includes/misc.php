...
function update_recently_edited($file) {
    ...
    if ($oldfiles) {
        ...
        if (5 < count($oldfiles)) array_pop($oldfiles);
    } else {
        $oldfiles[] = $file;
    }
...
function url_shorten($url) {
    ...
    if (strlen($short_url) > 35) $short_url = substr($short_url, 0, 32) . '&hellip;';
    return $short_url;
}
...
function show_message($message) {
    if (is_wp_error($message)) {
        if ($message->get_error_data() && is_string($message->get_error_data())) $message = $message->get_error_message() . ': ' . $message->get_error_data(); else
...
function wp_doc_link_parse($content) {
    if (!is_string($content) || empty($content)) return array();

    if (!function_exists('token_get_all')) return array();
    ...
    foreach ($functions as $function) {
        if (in_array($function, $ignore_functions)) continue;
        $out[] = $function;
    }
...
function set_screen_options() {
    ...
    if (isset($_POST['wp_screen_options']) && is_array($_POST['wp_screen_options'])) {
        ...
        if ($option != sanitize_key($option)) return;

        $map_option = $option;
        $type = str_replace('edit_', '', $map_option);
        $type = str_replace('_per_page', '', $type);
        if (in_array($type, get_taxonomies())) $map_option = 'edit_tags_per_page'; elseif (in_array($type, get_post_types())) $map_option = 'edit_per_page';
        else
...
function iis7_rewrite_rule_exists($filename) {
    ...
    if ($rules->length == 0) return false; else
        return true;
}
...
function wp_check_locked_posts($response, $data, $screen_id) {
    ...
    if (array_key_exists('wp-check-locked-posts', $data) && is_array($data['wp-check-locked-posts'])) {
        foreach ($data['wp-check-locked-posts'] as $key) {
            if (!$post_id = absint(substr($key, 5))) continue;
            ...
            if (($user_id = wp_check_post_lock($post_id)) && ($user = get_userdata($user_id)) && current_user_can('edit_post', $post_id)) {
                ...
                if (($avatar = get_avatar($user->ID, 18)) && preg_match("|src='([^']+)'|", $avatar, $matches)) $send['avatar_src'] = $matches[1];
    ...
    if (!empty($checked)) $response['wp-check-locked-posts'] = $checked;
...
function wp_refresh_post_lock($response, $data, $screen_id) {
    if (array_key_exists('wp-refresh-post-lock', $data)) {
        ...
        if (!$post_id = absint($received['post_id'])) return $response;

        if (!current_user_can('edit_post', $post_id)) return $response;
        ...
        if (($user_id = wp_check_post_lock($post_id)) && ($user = get_userdata($user_id))) {
            ...
            if ($avatar = get_avatar($user->ID, 64)) {
                if (preg_match("|src='([^']+)'|", $avatar, $matches)) $error['avatar_src'] = $matches[1];
            }

            $send['lock_error'] = $error;
        } else {
            if ($new_lock = wp_set_post_lock($post_id)) $send['new_lock'] = implode(':', $new_lock);
        }
...
function wp_refresh_post_nonces($response, $data, $screen_id) {
    if (array_key_exists('wp-refresh-post-nonces', $data)) {
        ...
        if (!current_user_can('edit_post', $post_id) || empty($received['post_nonce'])) return $response;
