        ...
        <div id="major-publishing-actions">
            ...
            <div id="delete-action">
                ...
                if (current_user_can("delete_post", $post->ID)) {
                    if (!EMPTY_TRASH_DAYS) $delete_text = __('Delete Permanently'); else
...
function post_format_meta_box($post, $box) {
    if (current_theme_supports('post-formats') && post_type_supports($post->post_type, 'post-formats')) :
        ...
        if (is_array($post_formats[0])) :
            ...
            if (!$post_format) $post_format = '0';
            // Add in the current one if it isn't there yet, in case the current theme doesn't support it
            if ($post_format && !in_array($post_format, $post_formats[0])) $post_formats[0][] = $post_format;
            ?>
...
function post_custom_meta_box($post) {
    ...
    <div id="postcustomstuff">
        ...
        foreach ($metadata as $key => $value) {
            if (is_protected_meta($metadata[$key]['meta_key'], 'post') || !current_user_can('edit_post_meta', $post->ID, $metadata[$key]['meta_key'])) unset($metadata[$key]);
        }
        list_meta($metadata);
        meta_form($post); ?>
    </div>
    ...
    <div id="taxonomy-linkcategory" class="categorydiv">
        ...
        <div id="categories-all" class="tabs-panel">
            <ul id="categorychecklist" data-wp-lists="list:category" class="categorychecklist form-no-clear">
                <?php
                if (isset($link->link_id)) wp_link_category_checklist($link->link_id); else
...
function xfn_check($class, $value = '', $deprecated = '') {
    ...
    if (!empty($deprecated)) _deprecated_argument(__FUNCTION__, '0.0'); // Never implemented
    ...
    <table class="links-table" cellpadding="0">
        ...
        <tr>
            ...
            <td><select name="link_rating" id="link_rating" size="1">
                    ...
                    for ($r = 0; $r <= 10; $r++) {
                        echo '<option value="' . $r . '"';
                        if (isset($link->link_rating) && $link->link_rating == $r) echo ' selected="selected"';
                        echo('>' . $r . '</option>');
                    }
