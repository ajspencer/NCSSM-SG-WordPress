...
function install_themes_feature_list() {
    ...
    if (!$cache = get_transient('wporg_theme_feature_list')) set_transient('wporg_theme_feature_list', array(), 3 * HOUR_IN_SECONDS);

    if ($cache) return $cache;

    $feature_list = themes_api('feature_list', array());
    if (is_wp_error($feature_list)) return array();
...
function install_theme_search_form($type_selector = true) {
    ...
    if (!$type_selector) echo '<p class="install-help">' . __('Search for themes by keyword.') . '</p>';
    ?>
...
function install_theme_information() {
    ...
    if (is_wp_error($theme)) wp_die($theme);
