...
class WP_Filesystem_SSH2 extends WP_Filesystem_Base {
    ...
    public function __construct($opt = '') {
        ...
        // Set defaults:
        if (empty($opt['port'])) $this->options['port'] = 22; else
            $this->options['port'] = $opt['port'];

        if (empty($opt['hostname'])) $this->errors->add('empty_hostname', __('SSH2 hostname is required')); else
        ...
        if (!empty($opt['username'])) $this->options['username'] = $opt['username'];

        if (empty ($opt['password'])) {
            // Password can be blank if we are using keys.
            if (!$this->keys) $this->errors->add('empty_password', __('SSH2 password is required'));
        } else {
    ...
    public function run_command($command, $returnbool = false) {
        ...
        if (!($stream = ssh2_exec($this->link, $command))) {
            ...
            if ($returnbool) return ($data === false) ? false : '' != trim($data); else
                return $data;
        }
        return false;
    }
    ...
    public function put_contents($file, $contents, $mode = false) {
        ...
        if ($ret !== strlen($contents)) return false;
    ...
    public function chgrp($file, $group, $recursive = false) {
        ...
        if (!$recursive || !$this->is_dir($file)) return $this->run_command(sprintf('chgrp %s %s', escapeshellarg($group), escapeshellarg($file)), true);
    ...
    public function chmod($file, $mode = false, $recursive = false) {
        ...
        if (!$recursive || !$this->is_dir($file)) return $this->run_command(sprintf('chmod %o %s', $mode, escapeshellarg($file)), true);
    ...
    public function chown($file, $owner, $recursive = false) {
        if (!$this->exists($file)) return false;
        if (!$recursive || !$this->is_dir($file)) return $this->run_command(sprintf('chown %s %s', escapeshellarg($owner), escapeshellarg($file)), true);
    ...
    public function owner($file) {
        ...
        if (!function_exists('posix_getpwuid')) return $owneruid;
    ...
    public function delete($file, $recursive = false, $type = false) {
        if ('f' == $type || $this->is_file($file)) return ssh2_sftp_unlink($this->sftp_link, $file);
    ...
    public function dirlist($path, $include_hidden = true, $recursive = false) {
        ...
        while (false !== ($entry = $dir->read())) {
            ...
            if ('.' == $struc['name'] || '..' == $struc['name']) continue; //Do not care about these folders.
