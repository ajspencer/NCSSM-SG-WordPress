    ...
    foreach (array_keys($wp_meta_boxes[$screen->id]) as $context) {
        foreach (array_keys($wp_meta_boxes[$screen->id][$context]) as $priority) {
            foreach ($wp_meta_boxes[$screen->id][$context][$priority] as $box) {
                ...
                if ('submitdiv' == $box['id'] || 'linksubmitdiv' == $box['id']) continue;
                $box_id = $box['id'];
...
function get_hidden_meta_boxes($screen) {
    ...
    if ($use_defaults) {
        $hidden = array();
        if ('post' == $screen->base) {
            if ('post' == $screen->post_type || 'page' == $screen->post_type || 'attachment' == $screen->post_type) $hidden = array('slugdiv', 'trackbacksdiv', 'postcustom', 'postexcerpt', 'commentstatusdiv', 'commentsdiv', 'authordiv', 'revisionsdiv'); else
...
final class WP_Screen {
    ...
    public static function get($hook_name = '') {
        ...
        if ($hook_name) $id = $hook_name; else
        ...
        if ($hook_name && post_type_exists($hook_name)) {
            ...
            if ('.php' == substr($id, -4)) $id = substr($id, 0, -4);
        ...
        if (!$post_type && $hook_name) {
            ...
            if (defined('WP_NETWORK_ADMIN') && WP_NETWORK_ADMIN) $in_admin = 'network'; elseif (defined('WP_USER_ADMIN') && WP_USER_ADMIN) $in_admin = 'user';
            else
                $in_admin = 'site';
        }

        if ('index' == $id) $id = 'dashboard'; elseif ('front' == $id) $in_admin = false;

        $base = $id;

        // If this is the current screen, see if we can be more accurate for post types and taxonomies.
        if (!$hook_name) {
            if (isset($_REQUEST['post_type'])) $post_type = post_type_exists($_REQUEST['post_type']) ? $_REQUEST['post_type'] : false;
            if (isset($_REQUEST['taxonomy'])) $taxonomy = taxonomy_exists($_REQUEST['taxonomy']) ? $_REQUEST['taxonomy'] : false;

            switch ($base) {
                case 'post' :
                    if (isset($_GET['post'])) $post_id = (int)$_GET['post']; elseif (isset($_POST['post_ID'])) $post_id = (int)$_POST['post_ID'];
                    else
                        $post_id = 0;
                    ...
                    break;
                case 'edit-tags' :
                    if (null === $post_type && is_object_in_taxonomy('post', $taxonomy ? $taxonomy : 'post_tag')) $post_type = 'post';
                    break;
            }
        }
        ...
        switch ($base) {
            ...
            case 'edit-tags' :
                ...
                if (null === $post_type) {
                    $post_type = 'post';
                    if (isset($_REQUEST['post_type']) && post_type_exists($_REQUEST['post_type'])) $post_type = $_REQUEST['post_type'];
                }
        ...
        if (isset(self::$_registry[$id])) {
            ...
            if ($screen === get_current_screen()) return $screen;
        } else {
    ...
    public function get_option($option, $key = false) {
        if (!isset($this->_options[$option])) return null;
        if ($key) {
            if (isset($this->_options[$option][$key])) return $this->_options[$option][$key];
            return null;
        }
    ...
    public function get_help_tab($id) {
        if (!isset($this->_help_tabs[$id])) return null;
    ...
    public function render_screen_meta() {
        ...
        if (empty($old_help) && !$this->get_help_tabs()) {
            ...
            if ($default_help) $old_help = '<p>' . $default_help . '</p>';
        }
        ...
        <div id="screen-meta" class="metabox-prefs">
                    ...
                    <div class="contextual-help-tabs-wrap">
                        ...
                        foreach ($this->get_help_tabs() as $tab):
                            ...
                            <div id="<?php echo esc_attr($panel_id); ?>" class="<?php echo $classes; ?>">
                                ...
                                if (!empty($tab['callback'])) call_user_func_array($tab['callback'], array($this, $tab));
                                ?>
                            </div>
                            <?php
            ...
            if (!empty($columns) && isset($columns[$this->id])) $this->add_option('layout_columns', array('max' => $columns[$this->id]));

            if ($this->get_option('layout_columns')) {
                $this->columns = (int)get_user_option("screen_layout_$this->id");

                if (!$this->columns && $this->get_option('layout_columns', 'default')) $this->columns = $this->get_option('layout_columns', 'default');
            }
    ...
    public function show_screen_options() {
        ...
        if (is_bool($this->_show_screen_options)) return $this->_show_screen_options;
        ...
        if ($this->_screen_settings || $this->_options) $show_screen = true;
                    ...
                    <div class="metabox-prefs">
                        ...
                        if ('dashboard' === $this->id && has_action('welcome_panel') && current_user_can('edit_theme_options')) {
                            if (isset($_GET['welcome'])) {
                                ...
                                if (2 == $welcome_checked && wp_get_current_user()->user_email != get_option('admin_email')) $welcome_checked = false;
                            }
                ...
                if ($columns) :
                    ...
                    <div class="metabox-prefs">
                        ...
                        foreach ($columns as $column => $title) {
                            ...
                            if (in_array($column, $special)) continue;
                            if (empty($title)) continue;

                            if ('comments' == $column) $title = __('Comments');
                            $id = "$column-hide";
                            echo '<label for="' . $id . '">';
    ...
    public function render_screen_layout() {
        if (!$this->get_option('layout_columns')) return;
