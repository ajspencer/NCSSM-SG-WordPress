...
class WP_Importer {
    ...
    public function count_imported_posts($importer_name, $bid) {
        ...
        if (!empty($result)) $count = intval($result[0]->cnt);
    ...
    public function get_page($url, $username = '', $password = '', $head = false) {
        ...
        $headers = array();
        $args = array();
        if (true === $head) $args['method'] = 'HEAD';
        if (!empty($username) && !empty($password)) $headers['Authorization'] = 'Basic ' . base64_encode("$username:$password");
