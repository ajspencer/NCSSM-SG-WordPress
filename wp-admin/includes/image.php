...
function wp_crop_image($src, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false) {
    ...
    if (!$dst_file) $dst_file = str_replace(basename($src_file), 'cropped-' . basename($src_file), $src_file);
...
function wp_generate_attachment_metadata($attachment_id, $file) {
    ...
    if (preg_match('!^image/!', get_post_mime_type($attachment)) && file_is_displayable_image($file)) {
        ...
        foreach (get_intermediate_image_sizes() as $s) {
            ...
            if (isset($_wp_additional_image_sizes[$s]['width'])) $sizes[$s]['width'] = intval($_wp_additional_image_sizes[$s]['width']); // For theme-added sizes
            else
                $sizes[$s]['width'] = get_option("{$s}_size_w"); // For default sizes set in options
            if (isset($_wp_additional_image_sizes[$s]['height'])) $sizes[$s]['height'] = intval($_wp_additional_image_sizes[$s]['height']); // For theme-added sizes
            else
                $sizes[$s]['height'] = get_option("{$s}_size_h"); // For default sizes set in options
            if (isset($_wp_additional_image_sizes[$s]['crop'])) $sizes[$s]['crop'] = $_wp_additional_image_sizes[$s]['crop']; // For theme-added sizes
            else
        ...
        if ($sizes) {
            ...
            if (!is_wp_error($editor)) $metadata['sizes'] = $editor->multi_resize($sizes);
        } else {
    ...
    if (isset($metadata['image']['data'])) unset($metadata['image']['data']);
    ...
    if (is_callable('iptcparse')) {
        ...
        if (!empty($info['APP13'])) {
            ...
            if (!empty($iptc['2#110'][0])) // credit
                $meta['credit'] = trim($iptc['2#110'][0]); elseif (!empty($iptc['2#080'][0])) // creator / legacy byline
...
function _copy_image_file($attachment_id) {
    ...
    if (!file_exists($src_file)) $src_file = _load_image_to_edit_path($attachment_id);
