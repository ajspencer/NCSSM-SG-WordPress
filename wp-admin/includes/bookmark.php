...
function edit_link($link_id = 0) {
    if (!current_user_can('manage_links')) wp_die(__('Cheatin&#8217; uh?'), 403);
...
function get_default_link_to_edit() {
    $link = new stdClass;
    if (isset($_GET['linkurl'])) $link->link_url = esc_url(wp_unslash($_GET['linkurl'])); else
        $link->link_url = '';

    if (isset($_GET['name'])) $link->link_name = esc_attr(wp_unslash($_GET['name'])); else
        $link->link_name = '';
...
function wp_set_link_cats($link_id = 0, $link_categories = array()) {
    ...
    if (!is_array($link_categories) || 0 == count($link_categories)) $link_categories = array(get_option('default_link_category'));
...
function wp_update_link($linkdata) {
    ...
    if (isset($linkdata['link_category']) && is_array($linkdata['link_category']) && 0 != count($linkdata['link_category'])) $link_cats = $linkdata['link_category']; else
...
function wp_link_manager_disabled_message() {
    global $pagenow;
    if ('link-manager.php' != $pagenow && 'link-add.php' != $pagenow && 'link.php' != $pagenow) return;
