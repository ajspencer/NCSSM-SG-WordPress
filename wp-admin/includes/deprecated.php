...
function wp_dropdown_cats($currentcat = 0, $currentparent = 0, $parent = 0, $level = 0, $categories = 0) {
    ...
    if (!$categories) $categories = get_categories(array('hide_empty' => 0));
    ...
    if ($categories) {
        foreach ($categories as $category) {
            if ($currentcat != $category->term_id && $parent == $category->parent) {
                ...
                if ($currentparent == $category->term_id) echo " selected='selected'";
...
function get_author_user_ids() {
    ...
    global $wpdb;
    if (!is_multisite()) $level_key = $wpdb->get_blog_prefix() . 'user_level'; else
...
function get_editable_user_ids($user_id, $exclude_zeros = true, $post_type = 'post') {
    ...
    if (!$user = get_userdata($user_id)) return array();
    $post_type_obj = get_post_type_object($post_type);

    if (!$user->has_cap($post_type_obj->cap->edit_others_posts)) {
        if ($user->has_cap($post_type_obj->cap->edit_posts) || !$exclude_zeros) return array($user->ID); else
            return array();
    }
    ...
    if ($exclude_zeros) $query .= " AND meta_value != '0'";
    ...
    class WP_User_Search {
        ...
        function prepare_query() {
            ...
            if ($this->search_term) {
                $searches = array();
                $search_sql = 'AND (';
                foreach (array('user_login', 'user_nicename', 'user_email', 'user_url', 'display_name') as $col) $searches[] = $wpdb->prepare($col . ' LIKE %s', '%' . like_escape($this->search_term) . '%');
        ...
        function query() {
            ...
            if ($this->results) $this->total_users_for_query = $wpdb->get_var("SELECT COUNT(DISTINCT($wpdb->users.ID))" . $this->query_from . $this->query_where); // no limit
            else
        ...
        function do_paging() {
            if ($this->total_users_for_query > $this->users_per_page) { // have to page the results
                $args = array();
                if (!empty($this->search_term)) $args['usersearch'] = urlencode($this->search_term);
                if (!empty($this->role)) $args['role'] = urlencode($this->role);
...
function get_others_unpublished_posts($user_id, $type = 'any') {
    ...
    if (in_array($type, array('draft', 'pending'))) $type_sql = " post_status = '$type' "; else
...
function wp_tiny_mce($teeny = false, $settings = false) {
    ...
    if (!class_exists('_WP_Editors')) require_once(ABSPATH . WPINC . '/class-wp-editor.php');
...
function wp_update_core($current, $feedback = '') {
    ...
    if (!empty($feedback)) add_filter('update_feedback', $feedback);
