...
function wp_create_category($cat_name, $parent = 0) {
    if ($id = category_exists($cat_name, $parent)) return $id;
...
function wp_create_categories($categories, $post_id = '') {
    ...
    if ($post_id) wp_set_post_categories($post_id, $cat_ids);
...
function wp_update_category($catarr) {
    ...
    if (isset($catarr['category_parent']) && ($cat_ID == $catarr['category_parent'])) return false;
...
function wp_create_term($tag_name, $taxonomy = 'post_tag') {
    if ($id = term_exists($tag_name, $taxonomy)) return $id;
