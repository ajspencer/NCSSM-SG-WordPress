...
function wp_ajax_nopriv_heartbeat() {
    ...
    if (!empty($_POST['screen_id'])) $screen_id = sanitize_key($_POST['screen_id']); else
        $screen_id = 'front';
...
function wp_ajax_fetch_list() {
    ...
    if (!$wp_list_table) wp_die(0);

    if (!$wp_list_table->ajax_user_can()) wp_die(-1);
...
function wp_ajax_wp_compression_test() {
    ...
    if (isset($_GET['test'])) {
        ...
        if (1 == $_GET['test']) {
            ...
            if (!isset($_SERVER['HTTP_ACCEPT_ENCODING'])) wp_die(-1);
...
function wp_ajax_imgedit_preview() {
    ...
    if (empty($post_id) || !current_user_can('edit_post', $post_id)) wp_die(-1);

    check_ajax_referer("image_editor-$post_id");

    include_once(ABSPATH . 'wp-admin/includes/image-edit.php');
    if (!stream_preview_image($post_id)) wp_die(-1);
...
function wp_ajax_autocomplete_user() {
    if (!is_multisite() || !current_user_can('promote_users') || wp_is_large_network('users')) wp_die(-1);

    /** This filter is documented in wp-admin/user-new.php */
    if (!is_super_admin() && !apply_filters('autocomplete_users_for_site_admins', false)) wp_die(-1);
    ...
    foreach ($users as $user) {
        $return[] = array(/* translators: 1: user_login, 2: user_email */
            'label' => sprintf(__('%1$s (%2$s)'), $user->user_login, $user->user_email), 'value' => $user->$field,);
    }
...
function _wp_ajax_delete_comment_response($comment_id, $delta = -1) {
    ...
    if (!$total || !$per_page || !$page || !$url) wp_die(time());
    ...
    if (0 == $total % $per_page || 1 == mt_rand(1, $per_page)) {
        ...
        if (isset($parsed['query'])) {
            ...
            if (!empty($query_vars['comment_status'])) $status = $query_vars['comment_status'];
            if (!empty($query_vars['p'])) $post_id = (int)$query_vars['p'];
        }

        $comment_count = wp_count_comments($post_id);

        // We're looking for a known type of comment count.
        if (isset($comment_count->$status)) $total = $comment_count->$status;
    ...
    $x = new WP_Ajax_Response(array('what' => 'comment', // Here for completeness - not used.
        'id' => $comment_id, 'supplemental' => array('total_items_i18n' => sprintf(_n('%s item', '%s items', $total), number_format_i18n($total)), 'total_pages' => ceil($total / $per_page), 'total_pages_i18n' => number_format_i18n(ceil($total / $per_page)), 'total' => $total, 'time' => $time)));
    $x->send();
}
...
function _wp_ajax_add_hierarchical_term() {
    ...
    if ($taxonomy->name == 'category') $post_category = isset($_POST['post_category']) ? (array)$_POST['post_category'] : array(); else
...
function wp_ajax_delete_comment() {
    ...
    if (!$comment = get_comment($id)) wp_die(time());
    if (!current_user_can('edit_comment', $comment->comment_ID)) wp_die(-1);
...
function wp_ajax_delete_tag() {
    ...
    if (!current_user_can($tax->cap->delete_terms)) wp_die(-1);

    $tag = get_term($tag_id, $taxonomy);
    if (!$tag || is_wp_error($tag)) wp_die(1);

    if (wp_delete_term($tag_id, $taxonomy)) wp_die(1); else
        wp_die(0);
}
...
function wp_ajax_delete_link() {
    ...
    if (!current_user_can('manage_links')) wp_die(-1);

    $link = get_bookmark($id);
    if (!$link || is_wp_error($link)) wp_die(1);

    if (wp_delete_link($id)) wp_die(1); else
        wp_die(0);
}
...
function wp_ajax_delete_meta() {
    ...
    if (!$meta = get_metadata_by_mid('post', $id)) wp_die(1);

    if (is_protected_meta($meta->meta_key, 'post') || !current_user_can('delete_post_meta', $meta->post_id, $meta->meta_key)) wp_die(-1);
    if (delete_meta($meta->meta_id)) wp_die(1);
    wp_die(0);
}
...
function wp_ajax_delete_post($action) {
    ...
    if (!get_post($id)) wp_die(1);

    if (wp_delete_post($id)) wp_die(1); else
        wp_die(0);
}
...
function wp_ajax_trash_post($action) {
    if (empty($action)) $action = 'trash-post';
    $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
    ...
    if ('trash-post' == $action) $done = wp_trash_post($id); else
...
function wp_ajax_untrash_post($action) {
    if (empty($action)) $action = 'untrash-post';
    wp_ajax_trash_post($action);
}

function wp_ajax_delete_page($action) {
    if (empty($action)) $action = 'delete-page';
    $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;

    check_ajax_referer("{$action}_$id");
    if (!current_user_can('delete_page', $id)) wp_die(-1);
...
function wp_ajax_dim_comment() {
    ...
    if (!current_user_can('edit_comment', $comment->comment_ID) && !current_user_can('moderate_comments')) wp_die(-1);

    $current = wp_get_comment_status($comment->comment_ID);
    if (isset($_POST['new']) && $_POST['new'] == $current) wp_die(time());

    check_ajax_referer("approve-comment_$id");
    if (in_array($current, array('unapproved', 'spam'))) $result = wp_set_comment_status($comment->comment_ID, 'approve', true); else
...
function wp_ajax_add_link_category($action) {
    ...
    foreach ($names as $cat_name) {
        ...
        if ('' === $slug) continue;
        if (!$cat_id = term_exists($cat_name, 'link_category')) $cat_id = wp_insert_term($cat_name, 'link_category');
        if (is_wp_error($cat_id)) {
            continue;
        } elseif (is_array($cat_id)) {
            $cat_id = $cat_id['term_id'];
...
function wp_ajax_add_tag() {
    ...
    if (!current_user_can($tax->cap->edit_terms)) wp_die(-1);
    ...
    if (!$tag || is_wp_error($tag) || (!$tag = get_term($tag['term_id'], $taxonomy))) {
        ...
        if (is_wp_error($tag) && $tag->get_error_message()) $message = $tag->get_error_message();
...
function wp_ajax_get_tagcloud() {
    ...
    if (empty($tags)) wp_die($tax->labels->not_found);

    if (is_wp_error($tags)) wp_die($tags->get_error_message());
...
function wp_ajax_get_comments($action) {
    ...
    if (empty($action)) $action = 'get-comments';

    check_ajax_referer($action);

    if (empty($post_id) && !empty($_REQUEST['p'])) {
        $id = absint($_REQUEST['p']);
        if (!empty($id)) $post_id = $id;
    }

    if (empty($post_id)) wp_die(-1);
    ...
    if (!$wp_list_table->has_items()) wp_die(1);

    $x = new WP_Ajax_Response();
    ob_start();
    foreach ($wp_list_table->items as $comment) {
        if (!current_user_can('edit_comment', $comment->comment_ID)) continue;
        get_comment($comment);
...
function wp_ajax_replyto_comment($action) {
    global $wp_list_table;
    if (empty($action)) $action = 'replyto-comment';
    ...
    if (!current_user_can('edit_post', $comment_post_ID)) wp_die(-1);

    if (empty($post->post_status)) wp_die(1); elseif (in_array($post->post_status, array('draft', 'pending', 'trash'))) wp_die(__('ERROR: you are replying to a comment on a draft post.'));
    ...
    if ($user->exists()) {
        ...
        if (current_user_can('unfiltered_html')) {
            if (!isset($_POST['_wp_unfiltered_html_comment'])) $_POST['_wp_unfiltered_html_comment'] = '';
    ...
    $comment_parent = 0;
    if (isset($_POST['comment_ID'])) $comment_parent = absint($_POST['comment_ID']);
    ...
    if (!empty($_POST['approve_parent'])) {
        ...
        if ($parent && $parent->comment_approved === '0' && $parent->comment_post_ID == $comment_post_ID) {
            if (wp_set_comment_status($parent->comment_ID, 'approve')) $comment_auto_approved = true;
        }
    }
    ...
    if ($comment_auto_approved) $response['supplemental'] = array('parent_approved' => $parent->comment_ID);
...
function wp_ajax_edit_comment() {
    ...
    if (!current_user_can('edit_comment', $comment_id)) wp_die(-1);

    if ('' == $_POST['content']) wp_die(__('ERROR: please type a comment.'));

    if (isset($_POST['status'])) $_POST['comment_status'] = $_POST['status'];
    edit_comment();
    ...
    $comment = get_comment($comment_id);
    if (empty($comment->comment_ID)) wp_die(-1);
...
function wp_ajax_add_menu_item() {
    ...
    if (!class_exists($walker_class_name)) wp_die(0);
...
function wp_ajax_add_meta() {
    ...
    if (isset($_POST['metakeyselect']) || isset($_POST['metakeyinput'])) {
        if (!current_user_can('edit_post', $pid)) wp_die(-1);
        ...
        if ($post->post_status == 'auto-draft') {
            ...
            if ($pid = edit_post()) {
                ...
                if (!$mid = add_meta($pid)) wp_die(__('Please provide a custom field value.'));
            } else {
                wp_die(0);
            }
        ...
        if ('' == trim($key)) wp_die(__('Please provide a custom field name.'));
        if ('' == trim($value)) wp_die(__('Please provide a custom field value.'));
        if (!$meta = get_metadata_by_mid('post', $mid)) wp_die(0); // if meta doesn't exist
        if (is_protected_meta($meta->meta_key, 'post') || is_protected_meta($key, 'post') || !current_user_can('edit_post_meta', $meta->post_id, $meta->meta_key) || !current_user_can('edit_post_meta', $meta->post_id, $key)) wp_die(-1);
        if ($meta->meta_value != $value || $meta->meta_key != $key) {
            if (!$u = update_metadata_by_mid('post', $mid, $value, $key)) wp_die(0); // We know meta exists; we also know it's unchanged (or DB error, in which case there are bigger problems).
        }
...
function wp_ajax_add_user($action) {
    ...
    check_ajax_referer($action);
    if (!current_user_can('create_users')) wp_die(-1);
...
function wp_ajax_closed_postboxes() {
    ...
    if ($page != sanitize_key($page)) wp_die(0);

    if (!$user = wp_get_current_user()) wp_die(-1);

    if (is_array($closed)) update_user_option($user->ID, "closedpostboxes_$page", $closed, true);
...
function wp_ajax_hidden_columns() {
    ...
    if (is_array($hidden)) update_user_option($user->ID, "manage{$page}columnshidden", $hidden, true);
...
function wp_ajax_wp_link_ajax() {
    ...
    if (isset($_POST['search'])) $args['s'] = wp_unslash($_POST['search']);
    $args['pagenum'] = !empty($_POST['page']) ? absint($_POST['page']) : 1;

    require(ABSPATH . WPINC . '/class-wp-editor.php');
    $results = _WP_Editors::wp_link_query($args);

    if (!isset($results)) wp_die(0);
...
function wp_ajax_meta_box_order() {
    ...
    if ($page_columns != 'auto') $page_columns = (int)$page_columns;
    ...
    if ($order) update_user_option($user->ID, "meta-box-order_$page", $order, true);

    if ($page_columns) update_user_option($user->ID, "screen_layout_$page", $page_columns, true);
...
function wp_ajax_inline_save() {
    ...
    if (!isset($_POST['post_ID']) || !($post_ID = (int)$_POST['post_ID'])) wp_die();

    if ('page' == $_POST['post_type']) {
        if (!current_user_can('edit_page', $post_ID)) wp_die(__('You are not allowed to edit this page.'));
    } else {
    ...
    if (isset($data['post_parent'])) $data['parent_id'] = $data['post_parent'];

    // Status.
    if (isset($data['keep_private']) && 'private' == $data['keep_private']) $data['post_status'] = 'private'; else
        $data['post_status'] = $data['_status'];

    if (empty($data['comment_status'])) $data['comment_status'] = 'closed';
    if (empty($data['ping_status'])) $data['ping_status'] = 'closed';
...
function wp_ajax_inline_save_tax() {
    ...
    if (!isset($_POST['tax_ID']) || !($id = (int)$_POST['tax_ID'])) wp_die(-1);
    ...
    if ($updated && !is_wp_error($updated)) {
        ...
        if (!$tag || is_wp_error($tag)) {
            if (is_wp_error($tag) && $tag->get_error_message()) wp_die($tag->get_error_message());
            wp_die(__('Item not updated.'));
        }
    } else {
        if (is_wp_error($updated) && $updated->get_error_message()) wp_die($updated->get_error_message());
        wp_die(__('Item not updated.'));
    }
    $level = 0;
    $parent = $tag->parent;
...
function wp_ajax_save_widget() {
    ...
    if (isset($_POST['delete_widget']) && $_POST['delete_widget']) {
        ...
        if (!isset($wp_registered_widgets[$widget_id])) wp_die($error);

        $sidebar = array_diff($sidebar, array($widget_id));
        $_POST = array('sidebar' => $sidebar_id, 'widget-' . $id_base => array(), 'the-widget-id' => $widget_id, 'delete_widget' => '1');
    } elseif ($settings && preg_match('/__i__|%i%/', key($settings))) {
        if (!$multi_number) wp_die($error);
...
function wp_ajax_upload_attachment() {
    ...
    if (isset($post_data['context']) && isset($post_data['theme'])) {
        if ('custom-background' === $post_data['context']) update_post_meta($attachment_id, '_wp_attachment_is_custom_background', $post_data['theme']);

        if ('custom-header' === $post_data['context']) update_post_meta($attachment_id, '_wp_attachment_is_custom_header', $post_data['theme']);
    }

    if (!$attachment = wp_prepare_attachment_for_js($attachment_id)) wp_die();
...
function wp_ajax_image_editor() {
    ...
    if (empty($attachment_id) || !current_user_can('edit_post', $attachment_id)) wp_die(-1);
...
function wp_ajax_set_post_thumbnail() {
    ...
    if ($json) check_ajax_referer("update-post_$post_ID"); else
...
function wp_ajax_wp_remove_post_lock() {
    ...
    if (!$post = get_post($post_id)) wp_die(0);
    ...
    if ($active_lock[1] != get_current_user_id()) wp_die(0);
...
function wp_ajax_dismiss_wp_pointer() {
    ...
    if ($pointer != sanitize_key($pointer)) wp_die(0);

//	check_ajax_referer( 'dismiss-pointer_' . $pointer );

    $dismissed = array_filter(explode(',', (string)get_user_meta(get_current_user_id(), 'dismissed_wp_pointers', true)));

    if (in_array($pointer, $dismissed)) wp_die(0);
...
function wp_ajax_get_attachment() {
    ...
    if (!$post = get_post($id)) wp_send_json_error();
...
function wp_ajax_query_attachments() {
    ...
    if (current_user_can(get_post_type_object('attachment')->cap->read_private_posts)) $query['post_status'] .= ',private';
...
function wp_ajax_save_attachment() {
    if (!isset($_REQUEST['id']) || !isset($_REQUEST['changes'])) wp_send_json_error();
    ...
    if (isset($changes['parent'])) $post['post_parent'] = $changes['parent'];

    if (isset($changes['title'])) $post['post_title'] = $changes['title'];

    if (isset($changes['caption'])) $post['post_excerpt'] = $changes['caption'];

    if (isset($changes['description'])) $post['post_content'] = $changes['description'];

    if (MEDIA_TRASH && isset($changes['status'])) $post['post_status'] = $changes['status'];
...
function wp_ajax_save_attachment_compat() {
    ...
    if (empty($_REQUEST['attachments']) || empty($_REQUEST['attachments'][$id])) wp_send_json_error();
    ...
    foreach (get_attachment_taxonomies($post) as $taxonomy) {
        if (isset($attachment_data[$taxonomy])) wp_set_object_terms($id, array_map('trim', preg_split('/,+/', $attachment_data[$taxonomy])), $taxonomy, false);
    }
...
function wp_ajax_save_attachment_order() {
    ...
    if (!$post_id = absint($_REQUEST['post_id'])) wp_send_json_error();

    if (empty($_REQUEST['attachments'])) wp_send_json_error();
    ...
    foreach ($attachments as $attachment_id => $menu_order) {
        ...
        if (!$attachment = get_post($attachment_id)) continue;
...
function wp_ajax_send_attachment_to_editor() {
    ...
    if (!empty($attachment['url'])) {
        $url = $attachment['url'];
        if (strpos($url, 'attachment_id') || get_attachment_link($id) == $url) $rel = ' rel="attachment wp-att-' . $id . '"';
...
function wp_ajax_send_link_to_editor() {
    ...
    if (!$src = wp_unslash($_POST['src'])) wp_send_json_error();

    if (!strpos($src, '://')) $src = 'http://' . $src;

    if (!$src = esc_url_raw($src)) wp_send_json_error();

    if (!$link_text = trim(wp_unslash($_POST['link_text']))) $link_text = wp_basename($src);
    ...
    $type = 'file';
    if (($ext = preg_replace('/^.+?\.([^.]+)$/', '$1', $src)) && ($ext_type = wp_ext2type($ext)) && ('audio' == $ext_type || 'video' == $ext_type)) $type = $ext_type;
...
function wp_ajax_get_revision_diffs() {
    ...
    if (!$post = get_post((int)$_REQUEST['post_id'])) wp_send_json_error();

    if (!current_user_can('read_post', $post->ID)) wp_send_json_error();

    // Really just pre-loading the cache here.
    if (!$revisions = wp_get_post_revisions($post->ID, array('check_enabled' => false))) wp_send_json_error();
