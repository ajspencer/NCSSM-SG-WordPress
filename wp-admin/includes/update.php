...
function get_preferred_from_update_core() {
    ...
    if (empty($updates)) return (object)array('response' => 'latest');
    return $updates[0];
}
...
function get_core_updates($options = array()) {
    ...
    foreach ($updates as $update) {
        if ($update->response == 'autoupdate') continue;
...
function find_core_auto_update() {
    ...
    foreach ($updates->updates as $update) {
        if ('autoupdate' != $update->response) continue;

        if (!$upgrader->should_update('core', $update, ABSPATH)) continue;
...
function get_core_checksums($version, $locale) {
    ...
    if (!is_array($body) || !isset($body['checksums']) || !is_array($body['checksums'])) return false;
...
function find_core_update($version, $locale) {
    ...
    $updates = $from_api->updates;
    foreach ($updates as $update) {
        if ($update->current == $version && $update->locale == $locale) return $update;
    }
    return false;
}

function core_update_footer($msg = '') {
    if (!current_user_can('update_core')) return sprintf(__('Version %s'), get_bloginfo('version', 'display'));
    ...
    if (!isset($cur->url)) $cur->url = '';

    if (!isset($cur->response)) $cur->response = '';
...
function update_nag() {
    if (is_multisite() && !current_user_can('update_core')) return false;
    ...
    if (!isset($cur->response) || $cur->response != 'upgrade') return false;
...
function update_right_now_message() {
    ...
    if (current_user_can('update_core')) {
        ...
        if (isset($cur->response) && $cur->response == 'upgrade') $msg .= '<a href="' . network_admin_url('update-core.php') . '" class="button" aria-describedby="wp-version">' . sprintf(__('Update to %s'), $cur->current ? $cur->current : __('Latest')) . '</a> ';
    }
...
function get_theme_updates() {
    ...
    if (!isset($current->response)) return array();
...
function maintenance_nag() {
    ...
    if (!$nag) {
        ...
        if (version_compare($failed['attempted'], $wp_version, $comparison)) $nag = true;
    }

    if (!$nag) return false;

    if (current_user_can('update_core')) $msg = sprintf(__('An automated WordPress update has failed to complete - <a href="%s">please attempt the update again now</a>.'), 'update-core.php'); else
