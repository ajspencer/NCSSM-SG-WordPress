...
if (isset($_POST['savewidget']) || isset($_POST['removewidget'])) {
    ...
    foreach ((array)$wp_registered_widget_updates as $name => $control) {
        if ($name != $id_base || !is_callable($control['callback'])) continue;
    ...
    if (!isset($_POST['delete_widget'])) {
        foreach ($sidebars_widgets as $key => $sb) {
            if (is_array($sb)) $sidebars_widgets[$key] = array_diff($sb, array($widget_id));
        }
...
if (isset($_GET['editwidget']) && $_GET['editwidget']) {
    ...
    if (!isset($name)) $name = esc_html(strip_tags($control['name']));

    if (!isset($sidebar)) $sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'wp_inactive_widgets';

    if (!isset($multi_number)) $multi_number = isset($control['params'][0]['number']) ? $control['params'][0]['number'] : '';
        ...
        <div class="editwidget"<?php echo $width; ?>>
            ...
            <form action="widgets.php" method="post">
                <div class="widget-inside">
                    <?php
                    if (is_callable($control_callback)) call_user_func_array($control_callback, $control['params']); else
                        ...
                        foreach ($wp_registered_sidebars as $sbname => $sbvalue) {
                            ...
                            if ('wp_inactive_widgets' == $sbname || 'orphaned_widgets' == substr($sbname, 0, 16)) {
                                ...
                                if (!isset($sidebars_widgets[$sbname]) || !is_array($sidebars_widgets[$sbname])) {
                                    ...
                                    if (isset($_GET['addnew']) || !in_array($widget_id, $sidebars_widgets[$sbname], true)) $j++;
                                }
                                $selected = '';
                                echo "\t\t<select name='{$sbname}_position'>\n";
                                echo "\t\t<option value=''>" . __('&mdash; Select &mdash;') . "</option>\n";
                                for ($i = 1; $i <= $j; $i++) {
                                    if (in_array($widget_id, $sidebars_widgets[$sbname], true)) $selected = selected($i, $key + 1, false);
    ...
    <div class="wrap">
        <h2>
            ...
            if (current_user_can('customize')) {
                printf(' <a class="add-new-h2 hide-if-no-customize" href="%1$s">%2$s</a>', esc_url(add_query_arg(array(array('autofocus' => array('panel' => 'widgets')), 'return' => urlencode(wp_unslash($_SERVER['REQUEST_URI']))), admin_url('customize.php'))), __('Manage in Customizer'));
            }
            ?>
        </h2>
            ...
            <div id="widgets-left">
                ...
                foreach ($wp_registered_sidebars as $sidebar => $registered_sidebar) {
                    if (false !== strpos($registered_sidebar['class'], 'inactive-sidebar') || 'orphaned_widgets' == substr($sidebar, 0, 16)) {
                        ...
                        if (!empty($registered_sidebar['class'])) $wrap_class .= ' ' . $registered_sidebar['class'];
        ...
        <div class="widget-liquid-right">
            <div id="widgets-right"<?php echo $single_sidebar_class; ?>>
                <div class="sidebars-column-1">
                    ...
                    $wrap_class = 'widgets-holder-wrap';
                    if (!empty($registered_sidebar['class'])) $wrap_class .= ' sidebar-' . $registered_sidebar['class'];
