...
if (!current_user_can('manage_sites')) wp_die(__('You do not have sufficient permissions to add sites to this network.'));
...
if (isset($_REQUEST['action']) && 'add-site' == $_REQUEST['action']) {
    ...
    if (!$user_id) { // Create a new user with a random password
        ...
        if (false == $user_id) wp_die(__('There was an error creating the user.')); else
    ...
    if (!is_wp_error($id)) {
        ...
        $content_mail = sprintf(/* translators: 1: user login, 2: site url, 3: site name/title */
...
if (isset($_GET['update'])) {
    $messages = array();
    if ('added' == $_GET['update']) $messages[] = sprintf(/* translators: 1: dashboard url, 2: network admin edit url */
        __('Site added. <a href="%1$s">Visit Dashboard</a> or <a href="%2$s">Edit Site</a>'), esc_url(get_admin_url(absint($_GET['id']))), network_admin_url('site-info.php?id=' . absint($_GET['id'])));
}
    ...
    <div class="wrap">
        ...
        <form method="post" action="<?php echo network_admin_url('site-new.php?action=add-site'); ?>"
            ...
            <table class="form-table">
                    ...
                    <td>
                        <?php if (is_subdomain_install()) { ?>
                            <input name="blog[domain]" type="text" class="regular-text" id="site-address"
                                   aria-describedby="site-address-desc"/><span
                ...
                <tr class="form-field form-required">
                    <th scope="row"><label for="admin-email"><?php _e('Admin Email') ?></label>
                    </td>
