    ...
    switch ($action) {
        ...
        case 'adduser':
            ...
            if (!empty($_POST['newuser'])) {
                ...
                if ($user && $user->exists()) {
                    if (!is_user_member_of_blog($user->ID, $id)) add_user_to_blog($id, $user->ID, $_POST['new_role']); else
        ...
        case 'remove':
            if (!current_user_can('remove_users')) die(__('You can&#8217;t remove users.'));
        ...
        case 'promote':
            ...
            if (isset($_REQUEST['users'])) {
                ...
                foreach ($userids as $user_id) {
                    ...
                    if (!is_user_member_of_blog($user_id)) wp_die(__('Cheatin&#8217; uh?'), 403);
 ...
 */
if (!wp_is_large_network('users') && apply_filters('show_network_site_users_add_existing_form', true)) wp_enqueue_script('user-suggest');
