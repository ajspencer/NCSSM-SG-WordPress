...
if (!current_user_can('manage_sites')) wp_die(__('You do not have permission to access this page.'), 403);
...
if (isset($_GET['action'])) {
    ...
    if (in_array($_GET['action'], $manage_actions)) {
        $action = $_GET['action'];
        if ('allblogs' === $action) $action = 'bulk-sites';
    ...
    switch ($_GET['action']) {
        ...
        case 'deleteblog':
            if (!current_user_can('delete_sites')) wp_die(__('You do not have permission to access this page.'), '', array('response' => 403));
            ...
            if ((isset($_POST['action']) || isset($_POST['action2'])) && isset($_POST['allblogs'])) {
                ...
                foreach ((array)$_POST['allblogs'] as $key => $val) {
                    if ($val != '0' && $val != $current_site->blog_id) {
                        switch ($doaction) {
                            case 'delete':
                                if (!current_user_can('delete_site', $val)) wp_die(__('You are not allowed to delete the site.'));
    ...
    if (empty($updated_action) && in_array($_GET['action'], $manage_actions)) $updated_action = $_GET['action'];
...
if (isset($_GET['updated'])) {
    ...
    if (!empty($msg)) $msg = '<div class="updated" id="message notice is-dismissible"><p>' . $msg . '</p></div>';
}
