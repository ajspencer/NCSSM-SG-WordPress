...
if (!current_user_can('manage_sites')) wp_die(__('You do not have sufficient permissions to edit this site.'));
...
if (!$id) wp_die(__('Invalid site ID.'));

$details = get_blog_details($id);
if (!can_edit_network($details->site_id)) wp_die(__('You do not have permission to access this page.'), 403);
...
if (isset($_REQUEST['action']) && 'update-site' == $_REQUEST['action']) {
    ...
    foreach ($blog_data_checkboxes as $c) {
        if (!in_array($existing_details->$c, array(0, 1))) $blog_data[$c] = $existing_details->$c; else
...
if (isset($_GET['update'])) {
    $messages = array();
    if ('updated' == $_GET['update']) $messages[] = __('Site info updated.');
}
                ...
                <tr class="form-field form-required">
                    <?php if ($is_main_site) { ?>
                        ...
                        <td>
                            ...
                            <input type="checkbox" name="update_home_url" id="update_home_url"
                                   value="update" <?php if (get_option('siteurl') == untrailingslashit(get_blogaddress_by_id($id)) || get_option('home') == untrailingslashit(get_blogaddress_by_id($id))) echo 'checked="checked"'; ?> />
                            <label
                                for="update_home_url"><?php _e('Update <code>siteurl</code> and <code>home</code> as well.'); ?></label>
                        </td>
                        <?php
                        restore_current_blog();
                    } ?>
