...
class Custom_Image_Header {
    ...
    public function help() {
        ...
        get_current_screen()->set_help_sidebar('<p><strong>' . __('For more information:') . '</strong></p>' . '<p>' . __('<a href="https://codex.wordpress.org/Appearance_Header_Screen" target="_blank">Documentation on Custom Header</a>') . '</p>' . '<p>' . __('<a href="https://wordpress.org/support/" target="_blank">Support Forums</a>') . '</p>');
    }
    ...
    public function step() {
        if (!isset($_GET['step'])) return 1;

        $step = (int)$_GET['step'];
        if ($step < 1 || 3 < $step || (2 == $step && !wp_verify_nonce($_REQUEST['_wpnonce-custom-header-upload'], 'custom-header-upload')) || (3 == $step && !wp_verify_nonce($_REQUEST['_wpnonce'], 'custom-header-crop-image'))) return 1;
    ...
    public function js_includes() {
        ...
        if ((1 == $step || 3 == $step)) {
            ...
            if (current_theme_supports('custom-header', 'header-text')) wp_enqueue_script('wp-color-picker');
        } elseif (2 == $step) {
    ...
    public function css_includes() {
        ...
        if ((1 == $step || 3 == $step) && current_theme_supports('custom-header', 'header-text')) wp_enqueue_style('wp-color-picker'); elseif (2 == $step) wp_enqueue_style('imgareaselect');
    }
    ...
    public function take_action() {
        ...
        if (isset($_POST['text-color']) && !isset($_POST['display-header-text'])) {
            ...
            if (strlen($color) == 6 || strlen($color) == 3) set_theme_mod('header_textcolor', $color); elseif (!$color) set_theme_mod('header_textcolor', 'blank');
        }
    ...
    public function process_default_headers() {
        ...
        if (!isset($_wp_default_headers)) return;
    ...
    public function show_header_selector($type = 'default') {
        ...
        foreach ($headers as $header_key => $header) {
            ...
            $width = '';
            if (!empty($header['attachment_id'])) $width = ' width="230"';
    ...
    public function js() {
        $step = $this->step();
        if ((1 == $step || 3 == $step) && current_theme_supports('custom-header', 'header-text')) $this->js_1(); elseif (2 == $step) $this->js_2();
    }
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if ($this->admin_image_div_callback) {
                                ...
                                if ($custom_header->width) $header_image_style .= 'max-width:' . $custom_header->width . 'px;';
                                if ($custom_header->height) $header_image_style .= 'height:' . $custom_header->height . 'px;';
                                ?>
                                <div id="headimg" style="<?php echo $header_image_style; ?>">
                                    <?php
                                    if (display_header_text()) $style = ' style="color:#' . get_header_textcolor() . ';"'; else
                        ...
                        <td>
                            <p><?php _e('You can select an image to be shown at the top of your site by uploading from your computer or choosing from your media library. After selecting an image you will be able to crop it.'); ?>
                                ...
                                if (!current_theme_supports('custom-header', 'flex-height') && !current_theme_supports('custom-header', 'flex-width')) {
                                    ...
                                    if (!current_theme_supports('custom-header', 'flex-width')) printf(__('Images should be at least <strong>%1$d pixels</strong> wide.') . ' ', get_theme_support('custom-header', 'width'));
                                } elseif (current_theme_supports('custom-header', 'flex-width')) {
                                    if (!current_theme_supports('custom-header', 'flex-height')) printf(__('Images should be at least <strong>%1$d pixels</strong> tall.') . ' ', get_theme_support('custom-header', 'height'));
                                }
                                if (current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width')) {
                                    if (current_theme_supports('custom-header', 'width')) printf(__('Suggested width is <strong>%1$d pixels</strong>.') . ' ', get_theme_support('custom-header', 'width'));
                                    if (current_theme_supports('custom-header', 'height')) printf(__('Suggested height is <strong>%1$d pixels</strong>.') . ' ', get_theme_support('custom-header', 'height'));
                                }
                                ?></p>
    ...
    public function step_2() {
        ...
        if (!current_theme_supports('custom-header', 'uploads')) wp_die(__('Cheatin&#8217; uh?'), 403);
        ...
        if (current_theme_supports('custom-header', 'flex-width')) $max_width = 1500;

        if (current_theme_supports('custom-header', 'max-width')) $max_width = max($max_width, get_theme_support('custom-header', 'max-width'));
        ...
        if (!current_theme_supports('custom-header', 'flex-height') && !current_theme_supports('custom-header', 'flex-width') && $width == get_theme_support('custom-header', 'width') && $height == get_theme_support('custom-header', 'height')) {
            // Add the meta-data
            if (file_exists($file)) wp_update_attachment_metadata($attachment_id, wp_generate_attachment_metadata($attachment_id, $file));
            ...
            if (!$image || is_wp_error($image)) wp_die(__('Image could not be processed. Please go back and try again.'), __('Image Processing Error'));
        ...
        <div class="wrap">
            ...
            <form method="post" action="<?php echo esc_url(add_query_arg('step', 3)); ?>">
                ...
                <p class="submit">
                    ...
                    <?php
                    if (isset($oitar) && 1 == $oitar && (current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width'))) submit_button(__('Skip Cropping, Publish Image as Is'), 'secondary', 'skip-cropping', false);
                    ?>
                </p>
            </form>
        </div>
    ...
    public function step_2_manage_upload() {
        ...
        if (!wp_match_mime_types('image', $wp_filetype['type'])) wp_die(__('The uploaded file is not a valid image. Please try again.'));

        $file = wp_handle_upload($uploaded_file, $overrides);

        if (isset($file['error'])) wp_die($file['error'], __('Image Upload Error'));
    ...
    public function step_3() {
        ...
        if (!empty($_POST['skip-cropping']) && !(current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width'))) wp_die(__('Cheatin&#8217; uh?'), 403);
        ...
        if (empty($_POST['skip-cropping'])) $cropped = wp_crop_image($attachment_id, (int)$_POST['x1'], (int)$_POST['y1'], (int)$_POST['width'], (int)$_POST['height'], $width, $height); elseif (!empty($_POST['create-new-attachment'])) $cropped = _copy_image_file($attachment_id);
        else
            $cropped = get_attached_file($attachment_id);

        if (!$cropped || is_wp_error($cropped)) wp_die(__('Image could not be processed. Please go back and try again.'), __('Image Processing Error'));
        ...
        if (!empty($_POST['create-new-attachment'])) unset($object['ID']);
    ...
    public function admin_page() {
        if (!current_user_can('edit_theme_options')) wp_die(__('You do not have permission to customize headers.'));
        $step = $this->step();
        if (2 == $step) $this->step_2(); elseif (3 == $step) $this->step_3();
        else
            $this->step_1();
    }
    ...
    final public function set_header_image($choice) {
        if (is_array($choice) || is_object($choice)) {
            $choice = (array)$choice;
            if (!isset($choice['attachment_id']) || !isset($choice['url'])) return;
        ...
        if ($uploaded && isset($uploaded[$choice])) {
            ...
            $this->process_default_headers();
            if (isset($this->default_headers[$choice])) $header_image_data = $this->default_headers[$choice]; else
                return;
        }
    ...
    final public function reset_header_image() {
        ...
        if (!$default) return $this->remove_header_image();
    ...
    final public function create_attachment_object($cropped, $parent_attachment_id) {
        ...
        $object = array('ID' => $parent_attachment_id, 'post_title' => basename($cropped), 'post_content' => $url, 'post_mime_type' => $image_type, 'guid' => $url, 'context' => 'custom-header');
