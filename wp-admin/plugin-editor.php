...
if (!current_user_can('edit_plugins')) wp_die(__('You do not have sufficient permissions to edit plugins for this site.'));
...
if (empty($file)) $file = $plugin_files[0];
...
switch ($action) {
    ...
    case 'update':
        ...
        if (is_writeable($real_file)) {
            ...
            if (is_plugin_active($file) || isset($_POST['phperror'])) {
                ...
                if (!is_network_admin()) update_option('recently_activated', array($file => time()) + (array)get_option('recently_activated'));
    ...
    default:
        ...
        if (isset($_GET['liveupdate'])) {
            ...
            if (is_wp_error($error)) wp_die($error);

            if ((!empty($_GET['networkwide']) && !is_plugin_active_for_network($file)) || !is_plugin_active($file)) activate_plugin($file, "plugin-editor.php?file=$file&phperror=1", !empty($_GET['networkwide'])); // we'll override this later if the plugin can be included without fatal error
                ...
                <div class="alignleft">
                    <big><?php
                        if (is_plugin_active($plugin)) {
                            if (is_writeable($real_file)) echo sprintf(__('Editing <strong>%s</strong> (active)'), $file); else
                                echo sprintf(__('Browsing <strong>%s</strong> (active)'), $file);
                        } else {
                            if (is_writeable($real_file)) echo sprintf(__('Editing <strong>%s</strong> (inactive)'), $file); else
                    ...
                    <form action="plugin-editor.php" method="post">
                        ...
                        <select name="plugin" id="plugin">
                            ...
                            foreach ($plugins as $plugin_key => $a_plugin) {
                                ...
                                if ($plugin_key == $plugin) $selected = " selected='selected'"; else
                                    $selected = '';
                ...
                <ul>
                    ...
                    foreach ($plugin_files as $plugin_file) :
                        ...
                        if (preg_match('/\.([^.]+)$/', $plugin_file, $matches)) {
                            $ext = strtolower($matches[1]);
                            // If extension is not in the acceptable list, skip it
                            if (!in_array($ext, $editable_extensions)) continue;
                        } else {
                            // No extension found
                            continue;
                        }
