...
if (!current_user_can('read')) wp_die(__('You do not have sufficient permissions to view this page.'));
...
get_current_screen()->set_help_sidebar('<p><strong>' . __('For more information:') . '</strong></p>' . '<p>' . __('<a href="https://codex.wordpress.org/Dashboard_My_Sites_Screen" target="_blank">Documentation on My Sites</a>') . '</p>' . '<p>' . __('<a href="https://wordpress.org/support/" target="_blank">Support Forums</a>') . '</p>');
        ...
        if (empty($blogs)) :
            ...
            <form id="myblogs" method="post">
                ...
                <table class="widefat fixed striped">
                    ...
                    $num = count($blogs);
                    $cols = 1;
                    if ($num >= 20) $cols = 4; elseif ($num >= 10) $cols = 2;
